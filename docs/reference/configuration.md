...
|---|---|---|
| `NOMIFUN_DATA_DIR` | all hosts | Source of truth for the backend data directory when the host wants to honour it. The value is taken **literally** as the data root on every host — desktop shell, standalone web host, and the `nomicore` binary alike (the desktop shell no longer appends `/Nomi`). `FLOWY_DATA_DIR` is a compatibility alias for the same setting; when both are set, `FLOWY_DATA_DIR` wins, and `--data-dir` outranks both (flag > `FLOWY_DATA_DIR` > `NOMIFUN_DATA_DIR` > channel default, the same order on every host). With neither set the dir is the per-user app-data default (see [below](#data-directory-and-work-directory-semantics)). |
...
| `NOMIFUN_MANAGED_FETCH_MODE` | Desktop backend | Managed web extraction rollout mode. Unset or `evidence-backed` enables the evidence-backed PDF/JavaScript/empty-content MCP fallback; `off` is the emergency Local-only rollback. Unknown or blank values fail closed to Local-only and emit a structured warning. This setting is read at Desktop startup and is not a database or UI setting. |
| `NOMIFUN_ENABLE_FREE_MODELS` | all hosts | Optional operator recovery switch for the preserved `nomifun-free-model` supply. Unset or any value other than `1`, `true`, `yes`, or `on` keeps the feature disabled; set it to a truthy value and restart to restore its service, routes, and UI. It does not affect independent OpenCode ACP/Agent support. |
| `NOMIFUN_SSRF_ALLOW_CIDRS` | all outbound fetch paths (`nomifun-net::ssrf`) | Comma-separated CIDRs exempted from the SSRF block list, for VPN tunnels that map DNS answers into space the policy otherwise treats as internal (e.g. `fc00::/18` for sing-box IPv6 fake-IP). The common IPv4 fake-IP ranges `198.18.0.0/15` and `240.0.0.0/4` are already allowed by default and need no entry. Read once per process. |
