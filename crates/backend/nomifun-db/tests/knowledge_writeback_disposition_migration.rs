//! Migration 031 (drop `writeback_mode`, move the write-back disposition to
...
async fn seed_pre_031(pool: &sqlx::SqlitePool) {
    // Main already shipped migrations 025-030 after the shared baseline; seed
    // immediately before 031 while `writeback_mode` / policy `mode` still exist.
