...
const NON_REFERENCE_ID_COLUMNS: &[(&str, &str)] = &[
    ...
    // must not wipe the check-in accounting that aggregates these rows.
    ("learning_review_events", "event_id"),
    ...
    ("mcp_servers", "mcp_server_id"),
    ("meeting_segments", "segment_id"),
    ("meeting_segments", "speaker_id"),
    ("meeting_segments", "session_id"),
    ("meeting_sessions", "session_id"),
    ("meeting_speakers", "speaker_id"),
    ("meeting_speakers", "voiceprint_id"),
...
pub(crate) const LOGICAL_REFERENCES: &[LogicalReference] = &[
    ...
    text_ref!("meeting_sessions", "user_id" => "users", "user_id", false, "idx_meeting_sessions_owner", Cascade),
    text_ref!("meeting_sessions", "bound_conversation_id" => "conversations", "conversation_id", true, "idx_meeting_sessions_bound_conversation", SetNull),
    text_ref!("meeting_voiceprints", "user_id" => "users", "user_id", false, "idx_meeting_voiceprints_owner", Cascade),
    ...
    text_ref!("learning_question_tags", "question_id" => "learning_activities", "activity_id", false, "idx_learning_question_tags_activity_id", Cascade)
        .with_child_predicate("child.source = 'course'"),
    text_ref!("learning_question_tags", "question_id" => "learning_custom_questions", "custom_question_id", false, "idx_learning_question_tags_custom_question_id", Cascade)
        .with_child_predicate("child.source = 'custom'"),
    text_ref!("learning_question_tags", "tag_id" => "learning_tags", "tag_id", false, "idx_learning_question_tags_tag_id", Cascade),
    ...
    text_ref!("preset_mcp_servers", "preset_id" => "presets", "preset_id", false, "idx_preset_mcp_servers_preset_id", Cascade),
    text_ref!("preset_mcp_servers", "mcp_server_id" => "mcp_servers", "mcp_server_id", false, "idx_preset_mcp_servers_mcp_server_id", Restrict),
...
pub(crate) const JSON_LOGICAL_REFERENCES: &[JsonLogicalReference] = &[
    ...
    json_text_ref!(
        "client_preferences", "value", "$.provider_id",
        "SELECT json_extract(value, '$.provider_id') AS value FROM client_preferences WHERE (key = 'nomi.defaultModel' OR key = 'knowledge.autogenModel' OR key = 'tools.imageGenerationModel' OR key = 'tools.imageAnalysisModel' OR key = 'tools.speechToText' OR key = 'tools.textToSpeech' OR key LIKE 'channels.%.defaultModel') AND json_valid(value)" =>
        "providers", "provider_id", "idx_client_preferences_provider_key", SetNull, RequireParent
    ),
    json_text_ref!(
