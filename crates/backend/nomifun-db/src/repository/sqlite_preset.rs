...
async fn load_record(pool: &SqlitePool, preset_id: &str) -> Result<Option<PresetRecord>, DbError> {
    ...
    let knowledge_bases = sqlx::query_as::<_, PresetKnowledgeBaseRow>(
        ...
        .bind(preset_id).fetch_all(pool).await?;
    let mcp_servers = sqlx::query_as::<_, PresetMcpServerRow>(
        "SELECT preset_id, mcp_server_id, sort_order FROM preset_mcp_servers \
         WHERE preset_id = ? ORDER BY sort_order")
        .bind(preset_id).fetch_all(pool).await?;
    ...
    Ok(Some(PresetRecord {
        ...
        skill_bindings, knowledge_policy, knowledge_bases, mcp_servers, examples, tag_bindings,
        user_state,
    }))
}
...
fn catalog_record_matches(record: &PresetRecord, p: &PresetWriteParams) -> bool {
    ...
    let mcp_servers = record
        .mcp_servers
        .iter()
        .map(|row| (row.sort_order, row.mcp_server_id.clone()))
        .collect::<Vec<_>>();
    let expected_mcp_servers = p
        .mcp_servers
        .iter()
        .enumerate()
        .map(|(sort_order, mcp_server_id)| (sort_order as i64, mcp_server_id.clone()))
        .collect::<Vec<_>>();
...
async fn replace_bindings(
    ...
    let mut unique_mcp_ids = std::collections::HashSet::with_capacity(p.mcp_servers.len());
    for mcp_server_id in &p.mcp_servers {
        nomifun_common::validate_uuidv7(mcp_server_id).map_err(|error| {
            DbError::Conflict(format!(
                "Preset MCP server '{mcp_server_id}' is not a canonical UUIDv7: {error}"
            ))
        })?;
        if !unique_mcp_ids.insert(mcp_server_id.as_str()) {
            return Err(DbError::Conflict(format!(
                "Preset MCP server '{mcp_server_id}' appears more than once"
            )));
        }
        let parent = sqlx::query(
        ...
        if parent.rows_affected() == 0 {
            return Err(DbError::Conflict(format!(
                "Preset MCP server '{mcp_server_id}' does not exist or is deleted"
            )));
        }
    }
    ...
    for table in [
        ...
        "preset_knowledge_bases", "preset_mcp_servers", "preset_examples",
        "preset_tag_bindings",
    ] {
    ...
    for (sort_order, mcp_server_id) in p.mcp_servers.iter().enumerate() {
        sqlx::query(
            "INSERT INTO preset_mcp_servers (preset_id, mcp_server_id, sort_order) VALUES (?,?,?)",
        )
        .bind(&p.preset_id)
        .bind(mcp_server_id)
