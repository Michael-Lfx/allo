...
describe('decodeComposerBeforeInput', () => {
  ...
  test('only accepts a literal true composition marker', () => {
    expect(decodeComposerBeforeInput({ inputType: 'insertCompositionText', isComposing: true }).isComposing).toBe(
      true
    );
    expect(decodeComposerBeforeInput({ inputType: 'insertText', isComposing: 'true' }).isComposing).toBe(false);
  });
  ...
  test('preserves supported text, line break, deletion, and composition actions', () => {
    ...
    expect(
      resolveComposerBeforeInputAction(decodeComposerBeforeInput({ inputType: 'deleteContentBackward' }))
