...
export function CanvasCharacterReferenceNodeContent({ node }: { node: CanvasNodeData }) {
    ...
    return <div className="flex h-full w-full flex-col overflow-hidden bg-background">
        <div className="relative min-h-0 flex-1 overflow-hidden bg-foreground/[.045]">
            ...
            <span className="absolute left-3 top-3 rounded bg-black/60 px-2 py-1 text-[var(--fs-tiny)] font-medium text-white">{canvasT("videoCanvas.menu.characterRef", "角色引用")}</span>
        </div>
        <div className="shrink-0 border-t border-border/70 px-3 py-2.5">
            ...
            <div className="mt-2 flex min-w-0 gap-1.5">
                <State icon={<ImageIcon className="size-3" />} ready={visualReady} label={visualReady ? canvasT("videoCanvas.character.visualReady", "形象就绪") : canvasT("videoCanvas.character.visualPending", "形象待完善")} />
                <State icon={<Volume2 className="size-3" />} ready={voiceReady} label={voiceReady ? node.metadata?.characterVoiceName || canvasT("videoCanvas.character.voiceBound", "声音已绑定") : canvasT("videoCanvas.character.voiceUnbound", "声音未绑定")} />
            </div>
        </div>
    </div>;
}
