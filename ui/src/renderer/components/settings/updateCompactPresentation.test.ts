...
describe('compact update presentation', () => {
  test('opens update checks in a non-modal bottom-right card', () => {
    ...
    expect(modalSource.includes('update-compact-card-host')).toBe(true);
    expect(modalSource.includes('fixed')).toBe(true);
    expect(modalSource.includes('right-24px')).toBe(true);
    expect(modalSource.includes('bottom-24px')).toBe(true);
    // Mike uses UnoCSS host classes — do not require a revived UpdateModal.css.
    expect(modalSource.includes("import './UpdateModal.css'")).toBe(false);
  });
  ...
  test('view details reveals the retained expanded dialog without changing update data', () => {
    ...
    expect(modalSource.includes('update-modal__release-scroll custom-scrollbar')).toBe(true);
