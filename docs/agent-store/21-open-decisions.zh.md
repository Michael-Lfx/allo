...
| **D14** | MCP 声明文件的接入路径（Kimi 式 `mcp.json`） | ① 载体：A `~/.agent-store/mcp.json`（Kimi 同名同形）/ B `config.toml [mcp.servers]` / C 两者；② 落点与冲突：A 纯内存注入、同名文件优先 / B 投影成 `mcp_servers` 行 / C 先 A 后 B 可选；③ 项目级：A 只允许 http·sse / B 默认全允许 / C 整体不做 | ①**A** ②**C** ③**C** | 无（新增能力，不阻塞既有 R） | **①A ②C ③C** ✅（2026-09-12 用户拍板；规格见 `20` §7.9，落地见 `20` §9 第 8 行与 §9.3；**2026-09-13 补齐全部可选字段并收敛 `headers` 语义**，见 `20` §7.9.1–§7.9.3 与 §9.4、§9 第 9 行） |
| **D15** | skill 的 `disable` 语义（2026-09-15） | A 目录标记（无运行时效果）/ B 隐藏技能目录 / C 不做 disable | **A** | 无（`install/disable` 语义澄清） | **A** ✅（2026-09-15，见下 §D15；`05` §4.5 / §4.5.2 与 `07` §6.3 已按此写） |
| **D16** | 商店的「更新」控件（2026-09-15） | A 摘掉控件、改说明 + 服务端修「卸载再安装」/ B 保留控件 / C 保留但标「即将推出」 | **A** | 无（`16` §6 红线执行） | **A** ✅（2026-09-15，见下 §D16；服务端修法见 `18` §9.1 / `02` TC-INS-008） |
| **D17** | `~/.agent-store/mcp.json` 的读写面（2026-09-17 写 / 2026-09-18 补读） | A 维持只读（`05` §4.10 原判）/ B 两个写方法 + 一个读方法（全文读/写 + 逐条 `enabled` 切换）/ C 只做全文写 | **B** | 无（`16` §6 产品面要求；推翻 `05` §4.10 的只读条款） | **B** ✅（2026-09-17 写、2026-09-18 补 `config/get-mcp`，见下 §D17；规格 `05` §4.10、解析器侧最小编辑见 `mcp_declarations.rs`） |
...
## D15 · skill 的 `disable` 只是目录标记（2026-09-15）

**背景**：`install/disable` / `install/enable` 从「只写插件自己的 `disabled` 位」改成
**真的移动运行时状态**——connector 置 `mcp_servers.enabled`、agent/team 置 Preset
`enabled`。skill 没有可移动的运行时状态，于是它在这次改动里成了唯一一个**不能**
假装移动的 kind。

**决定（A）**：**skill 的 `disable` 是目录标记，无运行时效果**。

- **理由**：技能语料就是普通目录，宿主没有「技能启用态」这一概念——它只是被扫描
  发现。写在 `plugin_snapshot_components.disabled` 的位只能影响**目录呈现**，改不了
  运行时会不会加载它。既然移动不了，就**不假装**移动了。
- **可观察形态**：outcome 报 `action: "marked"` + 稳定码
  `code: "skill_disable_flag_only"`——调用方能明确区分「真的关了」与「只在目录里
  标了一下」；`disable` 的 `ok` 仍为 `true`（请求的**目录标记**达到了），但那个
  code 就是「这和别的 kind 不一样」的机器可读说法。
- **要把技能从运行时移除，唯一路径是 `uninstall`**（删物化目录
  `{skills_root}/agent-store/<snapshot_id>/<slug>/`）。`install/disable` 不是
  它的替代品，`skill/delete` 也不是（那个只动用户技能树里的用户技能，`05` §4.11）。
- **连带口径**：被 `disable` 的 skill 再次 `install/run` 时记录位会被清掉（安装是
  「让它可用」），与 connector / agent 的重新安装语义一致。

规格落点：`05` §4.5 规则块与 §4.5.2 的 code 闭集；SDK 侧透传不吞（`07` §6.3）。

---

## D16 · 商店的假「更新」控件：摘掉，而不是留着当死控件（2026-09-15）

**背景**：商店里有两处违反 `16` §6 红线（**绝不留「点了没反应」的空壳控件**）的
「更新」：

1. 卡片已安装态的角标是 `<button onClick={onInstall}>`——已安装条目再调
   `store/install-entry` 只会拿到 `reused: true` 的空返回，按下去什么也不发生；
2. 抽屉动作区另有一枚「更新／更新中」按钮，**同样**接 `onInstall`。它更糟：
   wire 上**根本没有更新动词**（不存在 `store/update-entry`），这枚按钮**不可能**
   完成它承诺的事。

**决定（A）**：**两处都改成说明性元素，不是控件。**

- 卡片已安装态 → `role="note"` 的 `<span>`，`title` 在「已安装」与「有新版本可用」
  之间切换；
- 抽屉那枚按钮 → 删掉，换成状态标签 + 一句真话（`catalog.storeUpdateNote`）：
  **「当前版本没有原地升级：先卸载再安装，即可取到市场里的新版本。」**
- 选项 B（保留控件）与 C（标「即将推出」）都被否决：保留一个按下去什么都不发生的
  按钮，比没有这个按钮更糟——它把「不可用」伪装成「可用」。
- **前置条件是服务端先修好**：这句话在 2026-09-15 之前**是假的**——`install_entry`
  无条件复用既有快照，「卸载再安装」忠实装回**旧**版本，而 `store/list` 同时广告
  `update_available: true`。所以顺序是**先**让「卸载再安装」真的取到新版本
  （安装器的版本感知，规格见 `18` §9.1、用例见 `02` TC-INS-008），**再**改文案；
  删按钮而不修服务端等于把用户手上唯一那条路也指向死胡同。
- **不发明动作**：wire 上至今**没有**更新动词，`update_available` 只是一个**标志**。
  SDK 的 `StoreClient.updateHint()` 返回 `"uninstall_reinstall"` 表达「唯一路径是
  卸载再安装」，`install()` 对已安装条目是 no-op——**store 层永远不会因为看到一个
  「有新版本」的标志就自行拼一个更新动作**（`07` §1 非目标）。

落地：`web/src/components/CatalogView.tsx`、`web/src/i18n/{zh-CN,en-US}.ts`
...
## D17 · `mcp.json` 从只读变可写（2026-09-17）

**背景**：`05` §4.10 原判「该文件**不在** `config/set` 的白名单里——它只能由用户手写，
页面上没有任何写控件」。这条判断建立在一个正确的顾虑上（**一条声明可以拉起本地命令**），
但它把「用设置开关表达声明」与「有写面」混为一谈：产品面需要的是**原文件编辑**，
而不是一个把声明抽象成开关的界面（参考实现的「配置 MCP」就是原文件编辑器）。

**决定（B）**：新增两个写方法 + 一个读方法，**都指向同一份文件**，都走与 `config/*` 完全一样
的口径（仅 WebSocket、无 HTTP 绑定、不进 SDK 包、`require_ready` owner 闸门、无路径参数）：

- `config/set-mcp` `{ source }` —— 写**全文**；
- `config/set-mcp-enabled` `{ name, enabled }` —— 只改一条目的 `enabled`；
- `config/get-mcp` `{}` —— **读回文件自己的文本**（2026-09-18 补，理由见下第 4 条）。

**为什么不是给 `config/set` 加一个 `mcp` 键**：三者都不同——目标**文件**不同、失败原因
需要带**行列号**、响应必须是 `mcp` 视图而不是 config.toml 视图。

**四条设计约束**（每条都有具体理由，不是风格偏好）：

1. **写面 fail-closed，读面保持 fail-open**。读面把坏文件照原样投影出来（`error` + 空列表）
   是为了让手写错误**可见**；写面若接受一个解析不过的文本，就等于亲手制造那个状态。
   所以写前用**同一个解析器**验一遍，不过就**一个字节都不写**，并把解析器原始原因回传。
2. **开关是文本级最小编辑**（`set_server_enabled_in_source`）：值就地替换；成员缺失且要禁用
   时按文件自己的换行与缩进插入；成员缺失且要启用是**零改动**。理由与 `config/set` 用
   `toml_edit` 相同——**这是用户手写的文件**，一个开关没有资格重新缩进、重排键、或删掉
   用户写的行。定位不明确时**拒绝**（`mcp_source_not_surgically_editable`），不悄悄重排版；
   无字节变化则**不写盘**（重复调用不动 mtime）。
3. **只切换被接受的条目**：`mcpServers` 的原始映射里也有被解析器**拒绝**的条目，对它们
   「切换成功」而运行时毫无变化是最坏的答复，故单列 `mcp_server_rejected`。
   条目的开关字段是 `enabled`（**不是** `disabled`——那是未知字段，会让整条被拒）。
4. **凭据边界的精确化（2026-09-18，本决策的第二段）**：原判「`env` / `headers` 的值永不上
   wire」**过于笼统**，它把两件事混成一件——**verdict 视图**（`config/get.mcp`：只报
   `name` / `transport` / `enabled` 与拒绝原因）与**文件原文**。前者继续不含任何取值；后者
   必须能到编辑器，否则「就地编辑」不可能成立（**编辑器无法编辑它看不见的文件**）。
   所以新增 `config/get-mcp`：**唯一**返回原文的读面、**只由编辑器按需调用**（不做成
   `config/get` 的字段，因为每个设置对话框一打开就会调它）。它读不动时报错而**不回空串**
   ——静默从 `""` 开始的编辑器会覆盖一个只是读不到的文件。
   **残余风险随之改写**：原文（含 `env` 的明文取值、`headers`）会进入前端内存，可能进截图
   或日志。接受它的理由是**边界没变**：只有回环连接、owner 闸门内、操作者编辑的是自己机器
   上自己那份文件；`secret:NAME` 引用（§7.9.3）也让常见情形里存的是名称而非凭据值。

**残余风险（明确登记）**：这条写面让 WebUI 的一个请求可以让宿主启动本地命令（`stdio`
声明）。防线是复用的（owner 闸门 / 仅回环 / 无路径参数 / 原子写），界面上 `adopted`
三态继续显式呈现。若将来 WebUI 暴露到回环之外，**这条写面与 `config/get-mcp` 的读回
都必须先重新评估**。

**未做**：项目级 `<workspace>/.agent-store/mcp.json` 仍不读不写（`21` D14 ③=C 不变）。
