...
use nomi_config::MediaGenConfig;
use nomi_types::{ToolError, ToolHandler};
...
impl ToolHandler for MediaWorkflowPlanHandler {
    async fn execute(&self, params: Value) -> Result<String, ToolError> {
        ...
        if segment_plan.segment_count() > 1 {
            nomi_config::spawn_background_install(vec![nomi_config::RuntimeDep::Ffmpeg]);
        }
        ...
        Ok(json!({
            ...
            "hint": if resumable_run_id.is_some() {
                ...
                "Call media_workflow_run with { \"plan\": <plan.plan object above> } — pass the plan object verbatim including workflow_id and inputs."
            },
    ...
    fn schema(&self) -> nomi_types::ToolSchema {
