...
import type { PendingConnectionCreate } from "@oc/components/canvas/canvas-workspace-overlays";
...
export function useCanvasConnectionController({
    ...
    const connectNodes = useCallback((current: ConnectionHandle, targetNodeId: string, targetHandleId?: string, targetAnchorRatio?: number) => {
        ...
        if (exists) {
            ...
            setConnections((currentConnections) => [...currentConnections, { id: `conn-${nanoid()}`, fromNodeId, toNodeId, fromHandleId, toHandleId, fromAnchorRatio, toAnchorRatio }]);
    ...
    const openBatchConnectionCreateMenu = useCallback((clientX: number, clientY: number) => {
        ...
        clearBatchConnection();
        return true;
    }, [clearBatchConnection, message, nodesRef, screenToCanvas, setMouseWorld]);
    ...
    }, [connectNodes, getConnectionDropTarget, screenToCanvas, setConnecting, setMouseWorld]);
    ...
    const handleConnectStart = useCallback((event: ReactPointerEvent, nodeId: string, handleType: "source" | "target", handleId?: string, anchorRatio?: number) => {
        ...
        setConnectionTargetAnchorRatio(undefined);
        setSelectedConnectionId(null);
    }, [clearBatchConnection, commitBatchConnection, screenToCanvas, setConnecting, setConnectionTargetAnchorRatio, setConnectionTargetNodeId, setMouseWorld, setSelectedConnectionId]);
    ...
    }, [clearBatchConnection, commitBatchConnection, finishBatchConnection, finishConnection, getBatchConnectionDropTarget, getConnectionDropTarget, openBatchConnectionCreateMenu, previewBatchConnection, screenToCanvas, setConnecting, setConnectionTargetAnchorRatio, setConnectionTargetNodeId, setMouseWorld]);
