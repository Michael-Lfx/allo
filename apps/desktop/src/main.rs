...
///    process relaunch.
/// 2. `FLOWY_DATA_DIR` / `NOMIFUN_DATA_DIR` env — explicit override; the shell
///    appends `/Nomi` (semantics unchanged since the Electron era). The sole
///    compatibility exception is a canonical path that already proves a complete
///    v3 data root, as exported by an older embedded backend before an update
///    relaunch.
...
///    Same-channel hosts share one state; non-stable channels stay isolated.
fn default_data_dir() -> PathBuf {
    resolve_desktop_data_dir(
        std::env::var_os(DESKTOP_EFFECTIVE_DATA_DIR_ENV),
        nomifun_common::storage_paths::resolve_data_dir_from_env().map(|p| p.into_os_string()),
    )
}
