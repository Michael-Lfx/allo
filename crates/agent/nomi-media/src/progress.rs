...
use nomi_types::tool_progress::report_tool_progress;
...
pub fn report_media_progress(message: impl Into<String>) {
    report_tool_progress(message.into());
}
...
pub fn long_video_planning(target_secs: u32, segment_count: usize, max_clip_secs: u32) -> String {
    format!(
        "目标时长约 {target_secs} 秒 — Seedance 单次最多 {max_clip_secs} 秒，将拆分为 {segment_count} 段并首尾帧衔接；Allo 会自动安装 ffmpeg 并完成拼接"
    )
}
