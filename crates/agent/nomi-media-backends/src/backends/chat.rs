...
use super::{FlowyMediaServices, MediaChat, map_model_err};
...
impl FlowyChat {
    pub fn new(services: FlowyMediaServices, model: Option<String>) -> Self {
        Self {
            services,
    ...
    async fn vision_model_candidates(&self) -> MediaBackendResult<Vec<String>> {
        let catalog = self
            .services
            .api
...
impl MediaChat for FlowyChat {
    async fn complete_text(&self, system: &str, user: &str) -> MediaBackendResult<String> {
        ...
        let system = format!(
            "{system}\n\n{}",
            crate::language_lock::language_lock_for_text(user)
        );
        self.services
            .api
    ...
    async fn complete_vision(
        ...
        let system = format!(
            "{system}\n\n{}",
            crate::language_lock::language_lock_for_text(user_text)
        );
        ...
        if candidates.is_empty() {
            return Err(MediaBackendError::Llm(
        ...
        let mut last_err: Option<MediaBackendError> = None;
        ...
        Err(last_err.unwrap_or_else(|| {
            MediaBackendError::Llm("all multimodal vision model candidates failed".into())
        }))
    }
}
