...
import { canvasNodeMaterialSummary, canvasNodeSearchContext, canvasNodeSearchTimes, searchCanvasNodes } from "@oc/lib/canvas/canvas-node-search";
import { getNodeListLabel } from "@oc/lib/canvas/node-registry";
...
export function CanvasNodeSearchModal({ open, nodes, onClose, onFocus }: { open: boolean; nodes: CanvasNodeData[]; onClose: () => void; onFocus: (nodeId: string) => void }) {
    ...
    return (
        <Modal
            title={canvasT("videoCanvas.search.title", "搜索画布节点")}
            open={open}
            footer={null}
        ...
        >
            <Input
                ...
                placeholder={canvasT("videoCanvas.search.placeholder", "搜索节点、章节、镜头、模型或标签")}
                aria-label={canvasT("videoCanvas.search.title", "搜索画布节点")}
            ...
            <div id={RESULT_LIST_ID} role="listbox" aria-label={canvasT("videoCanvas.search.resultsAria", "画布节点搜索结果")} className="thin-scrollbar max-h-[54vh] overflow-y-auto overscroll-contain py-1.5">
                {results.length ? results.map((node, index) => (
                    ...
                    />
                )) : <WorkspaceState icon="canvas" compact title={canvasT("videoCanvas.search.emptyTitle", "没有匹配节点")} description={canvasT("videoCanvas.search.emptyDesc", "换一个节点、章节、镜头、模型或标签继续搜索。")} />}
            </div>
        </Modal>
    );
}
...
function CanvasNodeSearchThumbnail({ node }: { node: CanvasNodeData }) {
    ...
    if (mediaSource && !failed && node.type === CanvasNodeType.Video) {
        return <video src={mediaSource} muted playsInline preload="metadata" aria-label={`${node.title} ${canvasT("videoCanvas.search.videoThumb", "视频缩略图")}`} className={commonClass} style={commonStyle} onError={() => setFailed(true)} />;
    }
...
function fullTime(value?: string) {
    if (!value) return canvasT("videoCanvas.search.unrecordedTime", "时间未记录");
