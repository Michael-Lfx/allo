...
import { getNodeIcon, getNodeListLabel } from "@oc/lib/canvas/node-registry";
...
export function CanvasNodeSearchModal({ open, nodes, onClose, onFocus }: { open: boolean; nodes: CanvasNodeData[]; onClose: () => void; onFocus: (nodeId: string) => void }) {
    ...
    const results = useMemo(() => {
        ...
        return nodes.filter((node) => [node.title, node.type, getNodeListLabel(node.type), node.metadata?.prompt, node.metadata?.composerContent, node.metadata?.model, node.metadata?.chapterTitle, node.metadata?.workflowTitle, node.metadata?.workflowDescription, typeof node.metadata?.shotIndex === "number" ? canvasT("videoCanvas.search.shot", "镜头 {{n}}", { n: node.metadata.shotIndex + 1 }) : "", ...(node.metadata?.assetTags || [])]
            .some((value) => typeof value === "string" && value.toLocaleLowerCase().includes(keyword))).slice(0, 80);
    }, [nodes, query]);

    return (
        <Modal title={canvasT("videoCanvas.search.title", "搜索画布节点")} open={open} footer={null} width="min(680px, 90vw)" onCancel={onClose} afterClose={() => setQuery("")} centered>
            <Input autoFocus allowClear value={query} onChange={(event) => setQuery(event.target.value)} prefix={<Search className="size-4 opacity-50" />} placeholder={canvasT("videoCanvas.search.placeholder", "搜索节点、章节、镜头、模型或标签")} />
            <div className="thin-scrollbar mt-3 max-h-[50vh] overflow-y-auto border-t pt-2">
                {results.length ? results.map((node) => (
                    ...
                    >
                        <span className="grid size-8 shrink-0 place-items-center rounded-md bg-black/5 dark:bg-white/10 [&>svg]:size-4">
...
function nodeContextLabel(node: CanvasNodeData) {
    const location = [node.metadata?.chapterTitle, typeof node.metadata?.shotIndex === "number" ? canvasT("videoCanvas.search.shot", "镜头 {{n}}", { n: node.metadata.shotIndex + 1 }) : ""].filter(Boolean).join(" · ");
