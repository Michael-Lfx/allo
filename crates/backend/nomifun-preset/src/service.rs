...
use nomifun_common::{AgentId, AppError, KnowledgeBaseId, McpServerId, PresetId, PresetTagId, ProviderId};
use nomifun_db::{
...
fn validate_request(
    ...
    for mcp_server_id in mcp_server_ids {
        McpServerId::parse(mcp_server_id).map_err(|error| {
            AppError::BadRequest(format!("invalid mcp_server_id: {error}"))
        })?;
    }
...
fn record_to_response(record: &PresetRecord) -> Result<PresetResponse, AppError> {
    ...
    let mcp_server_ids = record
        .mcp_servers
        .iter()
        .map(|row| {
            McpServerId::parse(&row.mcp_server_id)
                .map(McpServerId::into_string)
                .map_err(|error| {
                    AppError::Internal(format!(
                        "stored preset mcp_server_id '{}' is not canonical: {error}",
                        row.mcp_server_id
                    ))
                })
        })
...
fn write_from_response(r: PresetResponse) -> PresetWriteParams {
    ...
    PresetWriteParams { preset_id:r.preset_id.clone(),source_kind:"user".into(),source_key:None,name:r.name,description:r.description,routing_description:r.routing_description,instructions:r.instructions,avatar:r.avatar,fallback_allowed:r.fallback_allowed,
        localizations,targets:target_strings(&r.targets),agent_preferences:r.agent_preferences.into_iter().map(|v|(v.agent_id,v.required)).collect(),model_preferences:r.model_preferences.into_iter().map(|v|(v.provider_id,v.model,v.required)).collect(),
        skill_bindings:r.included_skills.into_iter().map(|v| { let binding = SkillBinding::from_persisted_skill_id(&v.skill_id, v.required); (binding.skill_id,"include".into(),binding.required) }).chain(r.excluded_auto_skills.into_iter().map(|v|(v,"exclude_auto".into(),false))).collect(),
        knowledge_policy:(r.knowledge_policy.enabled,r.knowledge_policy.writeback,r.knowledge_policy.eagerness,r.knowledge_policy.grounded),knowledge_bases:r.knowledge_bases.into_iter().map(|v|(v.knowledge_base_id.to_string(),v.required)).collect(),
...
fn merge_update(mut p: PresetResponse, r: UpdatePresetRequest) -> PresetResponse {
    ...
    if let Some(v)=r.knowledge_bases{p.knowledge_bases=v} if let Some(v)=r.mcp_server_ids{p.mcp_server_ids=v} if let Some(v)=r.examples{p.examples=v} if let Some(v)=r.examples_i18n{p.examples_i18n=v} if let Some(v)=r.audience_tag_ids{p.audience_tag_ids=v} if let Some(v)=r.scenario_tag_ids{p.scenario_tag_ids=v}
