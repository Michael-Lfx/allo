...
export function CanvasAudioPlayer({ node, theme }: CanvasAudioPlayerProps) {
    ...
    const label = node.title || canvasT("videoCanvas.audio.fallbackTitle", "音频");
    const isPlaying = snapshot.phase === "playing";
    const isLoading = snapshot.phase === "loading";
    const isError = snapshot.phase === "error";

    return (
        <div role="group" aria-label={canvasT("videoCanvas.audio.controlsAria", "{{label}}播放控件", { label })} className="flex h-full w-full min-w-0 flex-col justify-center gap-2.5 px-4" style={{ color: theme.node.text }}>
            <div className="flex min-w-0 items-center gap-3">
                <button
                    type="button"
                    aria-label={isPlaying ? canvasT("videoCanvas.audio.pauseAria", "暂停{{label}}", { label }) : canvasT("videoCanvas.audio.playAria", "播放{{label}}", { label })}
                ...
                <div className="relative min-w-0 flex-1">
                    ...
                    <input
                        ...
                        aria-label={canvasT("videoCanvas.audio.progressAria", "{{label}}播放进度", { label })}
            ...
            {isError ? (
                <div role="status" className="flex min-w-0 items-center gap-1.5 truncate text-[var(--fs-tiny)]" style={{ color: theme.accent.danger || theme.node.muted }}>
                    ...
                    <span className="truncate">{snapshot.error || canvasT("videoCanvas.audio.playFailed", "音频播放失败")}</span>
                </div>
            ) : null}
        </div>
    );
