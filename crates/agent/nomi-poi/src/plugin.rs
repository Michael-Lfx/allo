...
use crate::memory_plugin::MemoryProviderPlugin;
...
impl InterestMemoryPlugin {
    ...
    pub fn open(data_dir: &str, config: InterestConfig) -> Option<Arc<Self>> {
        if !config.enabled {
            return None;
        }
        let db_path = PathBuf::from(data_dir).join("interest.db");
    ...
    pub fn from_store(
        ...
        Arc::new(Self::new(
            store,
            config,
            PathBuf::from(data_dir).join("interest.db"),
        ))
    }
