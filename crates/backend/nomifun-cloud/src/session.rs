...
use crate::token_store::{FileTokenStore, OAuthCredential};
...
const SERVER_TOKEN_ENV: &str = "NOMIFUN_SERVER_TOKEN";
...
impl ServerSession {
    pub fn from_config(config: &ServerConfig, data_dir: impl AsRef<Path>) -> Self {
        let _ = config;
        let env_token = std::env::var(SERVER_TOKEN_ENV)
            ...
            .filter(|v| !v.is_empty());
        Self {
            token_store_path: data_dir.as_ref().join("auth").join("tokens.json"),
            env_token,
        }
    }
    ...
    async fn token_store(&self) -> Result<FileTokenStore, ServerClientError> {
        if let Some(parent) = self.token_store_path.parent() {
            tokio::fs::create_dir_all(parent).await.map_err(|e| {
                ServerClientError::Cloud(CloudError::Io(e.to_string()))
            })?;
        }
        FileTokenStore::new(&self.token_store_path)
            .await
            .map_err(ServerClientError::Cloud)
    }
    ...
    pub async fn save_tokens(&self, tokens: ServerTokens) -> Result<(), ServerClientError> {
        if self.env_token.is_some() {
            debug!("NOMIFUN_SERVER_TOKEN set; skipping file token store write");
            return Ok(());
        }
        let store = self.token_store().await?;
        store
...
impl std::fmt::Display for TokenSource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Environment => write!(f, "environment (NOMIFUN_SERVER_TOKEN)"),
