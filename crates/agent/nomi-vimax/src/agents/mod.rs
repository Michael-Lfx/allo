...
idx from 0; is_last true only on the final shot; cam_idx groups shots sharing a camera; visual_desc is a filmable beat (composition + the ONE visual event), not a costume/face encyclopedia — identity is locked by reference images. audio_desc is REQUIRED for EVERY shot — put spoken dialogue and/or SFX+BGM intent there (never null/empty). When a character speaks, prefix with identifier_in_scene (e.g. 李薇：「……」 / Alice: "…") and put the line in quotes so duration estimation ignores SFX/BGM. Do NOT invent vocal timbre/age/gender in audio_desc (no 低沉嗓音/尖细女声/沙哑男声) — voice identity is locked separately; only dialogue text, emotion intensity, SFX, BGM. Dialogue MUST finish inside the same shot's 5–12s Seedance clip: Chinese ~3.3 chars/sec or English ~2.3 words/sec, leave ~1s lead-in and ~1s tail after the last word, then land on a visible reaction/action beat (no empty hold). Action-only shots should usually be 5–8s (do not pad a sit/stand/walk to 12s). All shots in the scene MUST reuse the SAME continuous underscore motif/tempo/instrumentation in audio_desc — no new music style per cut. Purely visual beats still need ambient audio_desc (room tone + that same cinematic underscore). Match clip length to the beat; do not pad long takes or split one beat into micro-cuts. Natural-language values MUST match the user's input language (Chinese input → 简体中文)."#;
...
ff_*/lf_* are COMPACT static snapshots (shot size, who is where, facing) — not full appearance bibles. lf_desc only states what CHANGED; if composition is unchanged, one short sentence. motion_desc is the primary video instruction: camera + the one visual event; character names OK with at most one short visible tag; timed beats (0-3s / 3-8s) when the clip needs ~8s+. variation_type is large|medium|small. Prose fields MUST match the user's input language (Chinese input → 简体中文)."#;
