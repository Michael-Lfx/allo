...
documentation for **Flowy**. Normative current behavior lives under
`architecture/` and `contributing/`. Temporary implementation plans and
superseded design drafts are not kept under `docs/` — use Git history when
needed. The `continuity/` area retains historical decisions, handoff context,
and release audits; it does not override the current architecture or
