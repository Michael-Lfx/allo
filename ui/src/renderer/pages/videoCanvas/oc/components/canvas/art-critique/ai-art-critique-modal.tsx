...
} from "@oc/lib/art-critique/contracts";
import { ART_CRITIQUE_CATEGORY_COLORS, ART_CRITIQUE_SEVERITY_COLORS, layoutArtCritiqueLabels, repairIssueTarget, targetBounds } from "@oc/lib/art-critique/annotation";
import { runArtCritiquePipeline } from "@oc/lib/art-critique/pipeline";
...
export function AiArtCritiqueModal({ node, upstreamNodes, open, onClose, onUpdateState }: AiArtCritiqueModalProps) {
    ...
    const runReview = async () => {
        ...
        if (!selectedCritiqueModel) {
            setLocalError("未配置支持图片理解的文本模型（模型 extra.input 需包含 image）");
            return;
        }
        setView("overview");
        setSelectedIssueId(null);
        ...
        try {
            ...
            const errorMessage = formatCanvasUserError(error, canvasT("videoCanvas.artCritique.failedRetry", "AI 批改失败，请稍后重试"));
            const errorCode = error instanceof Error && /^[a-z][a-z0-9]*(?:_[a-z0-9]+)+$/.test(error.message) ? error.message : "art_critique_failed";
            setActiveStage("failed");
            setDraftReportVisible(false);
            setLocalError(errorMessage);
            onUpdateState(node.id, { ...baseState, status: "failed", analysisStage: "failed", errorCode, errorMessage, updatedAt: new Date().toISOString() });
        } finally {
                ...
                <div className="flex min-h-0 flex-1 flex-col lg:grid lg:grid-cols-[minmax(0,1.45fr)_minmax(360px,.85fr)]">
                    ...
                    <aside className="min-h-0 flex-1 overflow-y-auto" data-canvas-wheel-scroll>
                        <div className="flex min-h-full flex-col gap-4 p-4">
                            ...
                            {!selectedCritiqueModel && !visibleState.report ? <Alert type="warning" showIcon message="未配置视觉模型" description="请在设置中选择 extra.input 包含 image 的多模态文本模型，再运行分析。" /> : null}
                            {visibleState.status === "stale" ? <Alert type="warning" showIcon message="输入图片已经变化" description="当前报告针对旧图片生成，请重新批改。" /> : null}
                            {localError || visibleState.errorMessage ? <Alert type="error" showIcon message="批改失败" description={formatCanvasUserError(localError || visibleState.errorMessage, canvasT("videoCanvas.artCritique.failedRetry", "AI 批改失败，请稍后重试"))} /> : null}
                            ...
                            <div className="sticky bottom-0 z-10 -mx-4 mt-auto flex flex-wrap items-center gap-2 border-t px-4 py-2" style={{ borderColor: theme.node.edge, background: theme.canvas.background }}>
                                ...
                                <div className="ml-auto flex shrink-0 items-center gap-2">
                                    ...
                                    <Button
                                        ...
                                        loading={running}
                                        disabled={running || !input || !selectedCritiqueModel}
