...
<a href="https://www.nomifun.com">
  <img src="docs/images/readme-00-agent-collaboration-hero.png" alt="Flowy Agent 协作、角色沉淀与执行图" width="100%">
</a>

<h3>Flowy — 好看、流畅，真正把事做完的 AI 工作站。</h3>

<p>
  面向真实用户的商业桌面产品：Agent、自动化与知识库在同一套体验里完成。<br/>
  支持 Freemium，需要更高额度与进阶能力时可订阅升级。
</p>
...
> [!IMPORTANT]
> **产品与数据声明**：Flowy 是持续迭代中的商业产品。升级、迁移、体验实验功能或接入真实工作数据前请自行备份。迭代过程中的数据丢失风险由用户自行承担。

---

**Flowy** 是商业化的 AI 工作站：桌面伙伴、无人值守自动化、知识库、原生 computer / browser use，以及开放能力总线。目标是质感、流畅，并尽快交付可检查的成果。使用 Flowy 账号登录；免费档可起步，订阅解锁更高额度与进阶能力。工作区由你掌控；模型调用走你配置的供应商，或你选择启用的 Flowy 托管能力。
...
|  | |
|---|---|
| ✨ **商业级质感** | 按日常高频产品打磨：清晰层级、顺滑动效、更短的首胜路径。 |
| 💳 **Freemium + 订阅** | 免费起步；需要更高额度、优先模型或进阶自动化时再升级。 |
...
- **账号解锁产品。** 登录后偏好、套餐权益与设备连续性保持一致。
- **工作区由你掌控。** 项目与知识保留在你选择的表面上；模型流量走你配置的供应商，或你主动启用的 Flowy 托管端点。
- **Freemium。** 免费档覆盖核心工作流；订阅提升额度并解锁进阶自动化 / 媒体能力。
- **费用说明。** 即使使用免费档，第三方模型供应商的 token 费用仍可能单独计费。
...
<p>
  <img src="docs/images/readme-01-workbench-overview.png" alt="Flowy Agent 协作与执行图" width="100%">
...
<sub>均为 2026-07-01 从当前运行中的 Flowy 桌面应用截取，并保留 2560px 宽度版本。完整截图清单与采集方式见 <a href="docs/images/SCREENSHOTS.md">截图 manifest</a>。</sub>
...
从一条普通 Agent 会话开始。当任务确实值得并行时，Flowy 会委派部分工作，并展示实时执行图。
...
- **经 ACP 直连约 19 个外部智能体** —— Claude Code、Codex、Gemini、Qwen、Kimi、Cursor、Copilot、Goose、OpenCode、Droid 等，Flowy 为它们提供模型*以及*自家的原生能力（computer/browser/knowledge/gateway，经注入的 MCP 桥）。
- **处处可用** —— 这些原生能力对内置智能体、ACP 智能体、聊天界面**以及**终端一律可用。
- **多模态失败会优雅降级。** 如果当前模型/供应商不接受图片输入，Flowy 会自动剔除图片、在同一会话里重试，并给出一条可见提示，而不是直接把整段会话打断。
- **每模型上下文窗口可单独校准。** 当上游平台默认值不准、没报全，或你想精细控制路由与长上下文预算时，可以按模型单独覆写上下文窗口上限。

### 🔌 大模型供应商快捷入口

Flowy 不绑定任何一家模型厂商。你可以按地区、价格、额度、模型能力和数据政策选择供应商，注册后把 API Key 填到 Flowy 的 **模型 & Agent** 页面即可使用。以下均为第三方服务，费用、可用地区、速率限制与数据处理规则以各家官方说明为准。
...
在应用内 PTY 会话里运行各种 agent CLI（或独立的 `nomi` CLI）。Flowy 会把原生能力 —— 知识检索、需求完成、生命周期 hooks —— 经各 CLI *自己的*原生配置注入进去，从而保留完整保真度与 OAuth。
...
Flowy 非常需要你的加入来壮大 —— 代码贡献、社区运营、技术布道都热烈欢迎。如果你对这个项目有热情，请[联系我们](#-联系我们--社区)，与我们一起共建 Flowy 的生态。
...
Flowy 正按商业产品标准打磨：好看、流畅，并尽量在登录后 90 秒内交付可检查的首个成果。

许可细节见 [`LICENSE`](LICENSE)。免费档与订阅档由 Flowy 账号权益决定 —— 不再承诺「全部功能永久免费」。
...
| 📕 **小红书** | [Flowy](https://xhslink.com/m/4x6ti8n6cA1) |
| 📺 **哔哩哔哩** | [Flowy](https://b23.tv/0UhgKDh) · [演示视频](https://www.bilibili.com/video/BV1kwKZ6UE5X/) |
| 🎵 **抖音** | [Flowy](https://v.douyin.com/MDT5QVdYaJk/) |
| ▶️ **YouTube** | [@Flowy-o2y](https://www.youtube.com/@Flowy-o2y) · [演示视频](https://youtu.be/AsEToBDFR9s) |
