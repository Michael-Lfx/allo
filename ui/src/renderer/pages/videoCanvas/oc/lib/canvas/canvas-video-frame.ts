...
export async function captureVideoLastFrame(source: Blob | string) {
    const durationHint = source instanceof Blob ? undefined : undefined;
    const captured = await captureVideoFrames(source, [Number.MAX_SAFE_INTEGER]);
    const last = captured.frames[captured.frames.length - 1];
    if (!last) throw new Error("无法截取视频尾帧");
    void durationHint;
    return last.blob;
}
...
async function readVideoBlob(source: Blob | string) {
    ...
    try {
        const url = source.startsWith("/") ? resolveCanvasUrl(source) : source;
        if (!url) throw new Error("无法读取视频文件，请重新上传视频后再提取画面");
        const response = await fetch(url, { headers: buildBackendAuthHeaders("GET"), credentials: "omit" });
