...
impl AgentEngine {
    ...
    /// to inject dynamic context (knowledge RAG, memory, …) into the turn
    /// tail without the engine hard-coding the source. No-op effect on prompts
    ...
    async fn execute_turn_inner(
        ...
        *turn_started = true;
        ...
        loop {
            ...
            // Keep the system prompt byte-stable for prefix caching. Dynamic
            // per-turn context (date, plan mode, RAG/memory contributors) rides
            // the turn tail instead of mutating the system prompt.
