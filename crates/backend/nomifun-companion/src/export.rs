...
mod tests {
    ...
    #[tokio::test]
    async fn legacy_memory_bundle_validates_then_ignores_learn_history() {
        let dir = tempfile::TempDir::new().unwrap();
        let store = CompanionStore::open_memory().await.unwrap();
        let roster = scan_registry(dir.path(), "companions");
        ...
        // Learn history is install-local bookkeeping, not portable content: the
        // bundle's rows are validated (a malformed one still fails the import)
        // and then dropped rather than written into this install's timeline.
        let imported_runs: i64 =
            sqlx::query_scalar("SELECT count(*) FROM companion_learn_runs")
                .fetch_one(store.pool())
                .await
                .unwrap();
        assert_eq!(imported_runs, 0, "legacy run history must not be imported");
    }
