...
- `apps/desktop/tauri.conf.json` **已删除 `version` 字段** → Tauri 自动继承 workspace 版本（已验证：安装包名随之变 `Flowy_0.1.10_x64-setup.exe`）。
...
- Windows 那台已验证可出 `Flowy_0.1.10_x64-setup.exe`；mac/linux 包还没在对应系统上构建过。
...
bun run build:win --config apps/desktop/tauri.updater.conf.json   # 产 Flowy_<ver>_x64-setup.exe + .sig
