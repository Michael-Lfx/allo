...
// Stubs until nomi-skills exposes hub lock + content scanning for contribution.
struct HubLock {
    installed: Vec<HubInstallEntry>,
}
struct HubInstallEntry {
    name: String,
    install_path: String,
}
fn read_hub_lock(_skills_root: &Path) -> HubLock {
    HubLock {
        installed: Vec::new(),
    }
}
fn scan_content(_path: &str, _content: &str) -> Vec<ScanFinding> {
    Vec::new()
}
struct ScanFinding {
    severity: String,
