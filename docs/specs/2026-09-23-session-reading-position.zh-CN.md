...
export interface SessionScrollSnapshot {
  ...
  anchorMessageId?: string;     // 顶部可见消息 ID (备用锚点)
...
- **生命周期时序收拢**：
  - 将会话切换检测从 `useEffect` 移至 `useLayoutEffect`。当 `conversationId !== previousConversationIdRef.current` 时：
    1. 同步将离开会话的 `scrollTop` 与 `userScrolled` 状态写入 `sessionScrollRegistry`；
    2. 重置 `initialScrollDoneRef` 与当前会话的用户消息 ID 跟踪；
    3. 若当前会话历史状态为 `saved.userScrolled === true`，同步赋值 `scrollerEl.scrollTop = saved.scrollTop` 并激活历史锁定；
    4. 若当前会话为置底状态，同步重置 `showScrollButton` 与 `hasNewContentBelow` 为 `false`，并调度平滑置底。
- **流式输出防抖动**：
  - 依赖 `.message-list-end-spacer` 计算 sub-pixel 偏差并实时校准，关闭 Virtuoso 内部的 `followOutput`，避免思考过程折叠/工具调用展开时导致的视口跳变。
- **用户发消息置底优先级**：
  - 监听最新的 User Message ID。只要产生新的一轮提问，即刻解除锁定并置底。
...
- **测试命令**：
  ...
  ```
- **测试结果**：**31 项测试全部通过，149 个断言（expect calls）100% 达标**。
  - `sessionScrollRegistry.test.ts` (7/7 pass): 覆盖增删查改、清空、负数保护、LRU 200 容量上限淘汰与命中刷新。
  - `useAutoScroll.test.ts` (17/17 pass): 覆盖会话切换同步保存、历史视口精确恢复、发消息强制置底、悬浮按钮状态即时清理。
