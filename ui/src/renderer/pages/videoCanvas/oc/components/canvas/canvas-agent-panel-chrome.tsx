    ...
    return (
        <header className="shrink-0 px-3 pb-2 pt-3">
            ...
            <div className="mt-2 flex min-h-8 flex-wrap items-center gap-x-2.5 gap-y-1 px-0.5 text-[var(--fs-label)]" style={{ color: theme.node.muted }}>
                <span className="font-medium" style={{ color: theme.node.text }}>{canvasT("videoCanvas.agent.nodeCount", "{{count}} 个节点", { count: context.nodeCount })}</span>
                {context.selectedCount ? <span className="inline-flex items-center gap-1"><Focus className="size-3" />{canvasT("videoCanvas.agent.selectedCount", "选中 {{count}}", { count: context.selectedCount })}</span> : <span>{canvasT("videoCanvas.agent.noSelection", "未选择节点")}</span>}
                ...
                <div className="ml-auto flex shrink-0 items-center gap-1.5">
                    ...
                    <label className="flex h-7 cursor-pointer items-center gap-1.5 rounded-md px-1.5" style={{ color: theme.node.muted }}>
                        ...
                        <span className="whitespace-nowrap">{canvasT("videoCanvas.agent.confirmBefore", "执行前确认")}</span>
                    </label>
                </div>
            </div>
        </header>
            ...
            {(["online", "local"] as const).map((item) => {
                ...
                return (
                    <button key={item} type="button" className="inline-flex h-7 items-center gap-1 rounded-[var(--r-sm)] px-2 transition-colors" style={{ background: active ? theme.node.fill : "transparent", color: active ? theme.node.text : theme.node.muted, boxShadow: active ? `0 1px 5px ${theme.spatial.shadow}` : "none" }} onClick={() => onChange(item)} aria-pressed={active}>
                        ...
                        {item === "online" ? canvasT("videoCanvas.agent.modeOnline", "网站") : canvasT("videoCanvas.agent.modeLocal", "本机")}
                    </button>
                );
            })}
        </div>
...
const starterActions = [
    { labelKey: "starterPipeline", labelDefault: "搭建短剧工作流", icon: Clapperboard },
    { labelKey: "starterOrganize", labelDefault: "整理当前画布", icon: LayoutTemplate },
    { labelKey: "starterStoryboard", labelDefault: "生成镜头分镜", icon: PanelsTopLeft },
    { labelKey: "starterCheckLinks", labelDefault: "检查节点连线", icon: Workflow },
];
...
export function AgentChatEmptyState({ theme, nodeCount, onSelect }: { theme: CanvasTheme; nodeCount: number; onSelect: (value: string) => void }) {
    ...
    const visibleStarterActions = shortDramaEnabled ? starterActions : starterActions.filter((item) => item.labelKey !== "starterPipeline");
    return (
        <div className="flex h-full items-center px-5 py-8">
            <div className="mx-auto w-full max-w-[380px]">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[var(--fs-label)] font-medium" style={{ color: theme.node.muted }}>{canvasT("videoCanvas.agent.nodesReady", "{{count}} 个节点已就绪", { count: nodeCount })}</span>
                </div>
                <h2 className="mt-3 text-[var(--fs-heading-lg)] font-semibold leading-6" style={{ color: theme.node.text }}>{canvasT("videoCanvas.agent.startFromCanvas", "从当前画布开始")}</h2>
                <div className="mt-4 grid grid-cols-1 gap-1">
                    {visibleStarterActions.map(({ labelKey, labelDefault, icon: Icon }) => {
                        const label = canvasT(`videoCanvas.agent.${labelKey}`, labelDefault);
                        return (
