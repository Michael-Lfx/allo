...
import { resolveDirectorBoneRotation } from "@oc/lib/canvas/director/director-animation-semantics";
import { createDirectorTransaction, installDirectorTerminalListeners } from "@oc/lib/canvas/director/director-gesture-transaction";
import { emptyDirectorPlacementIntent, finiteDirectorGroundPoint, type DirectorGroundPoint, type DirectorPlacementIntent } from "@oc/lib/canvas/director/director-placement";
import { directorDiagnosticObjectKind } from "@oc/lib/canvas/director/director-diagnostics";
import { recordDirectorDiagnostic } from "@oc/lib/canvas/director/director-diagnostics-recorder";
import { directorCaptureInitial, directorCaptureUsable, directorLoadIdentity, directorLoadInitial, installDirectorContextListeners, reduceDirectorCapture, reduceDirectorLoad, releaseDirectorCapture, resolveDirectorDisplay, restoreDirectorCapture, upsertDirectorFailedLoad, type DirectorFailedLoads, type DirectorLoadSignal } from "@oc/lib/canvas/director/director-recovery";
import { disposeDirectorAdoptionFailure, disposeDirectorHelper, disposeDirectorMaterials, disposeDirectorModelResources, disposeDirectorObject3D, resolveDirectorLoadOwnership } from "@oc/lib/canvas/director/director-resources";
import { DIRECTOR_DEFAULT_ACTOR_URL, directorPoseBoneDeltas, directorTransformPathLength, finiteDirectorTransformKeyframes, interpolateDirectorTransform } from "@oc/lib/canvas/director/director-scene";
import { DIRECTOR_DEFAULT_VIEW_MODE, directorViewFramingKey, resolveDirectorEffectiveViewport, resolveDirectorOrthographicFraming, resolveDirectorOrthographicFrustum, resolveDirectorViewFraming, type DirectorOrthographicFraming, type DirectorViewFraming, type DirectorViewMode } from "@oc/lib/canvas/director/director-view-modes";
import { DirectorViewToolbar } from "@oc/components/canvas/director/director-view-toolbar";
import { resolveMediaUrl } from "@oc/services/file-storage";
import type { DirectorHumanoidBone, DirectorLight, DirectorObject, DirectorQuat, DirectorRenderMode, DirectorRig, DirectorScene, DirectorTransform, DirectorVec3 } from "@oc/types/director";
                ...
                loader.load(
                    ...
                    (gltf: { scene: Object3D; animations: AnimationClip[] }) => {
                        ...
                        try {
                            ...
                            clone = cloned;
                            normalizeModel(cloned, object.castShadow, object.receiveShadow);
                            const nextRig = inferDirectorRig(
                                cloned,
                                gltf.animations.map((clip: AnimationClip) => clip.name),
                            );
                            if (object.kind === "actor" || object.primitive === "character") applyActorReferenceMaterial(cloned, object.color);
                            mixer = new AnimationMixer(cloned);
                            mixerRef.current = mixer;
                            ownedRef.current = { model: cloned, mixer };
                            // 一次性写入带 identity 的展示记录，避免 model/rig/animations 之间出现中间态。
                            setLoaded({
                                identity,
                                value: { model: cloned, rig: nextRig, animations: gltf.animations, restRotations: readRigRestRotations(cloned, nextRig) },
                            });
                            setLoadPhase("ready");
