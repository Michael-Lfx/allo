...
import { modelProtocolCapability, normalizeModelProtocol, type ModelProtocol } from "@oc/lib/model-protocols";
import { normalizeVideoDuration, normalizeVideoResolution } from "@oc/lib/video-generation-options";
import type { ModelCapabilityConfig } from "@oc/lib/model-capabilities";
...
export function selectableModelsByCapability(config: AiConfig, capability?: ModelCapability) {
    ...
    // Prefer allo-synced capability lists when present (authoritative image/video split).
    const dedicated =
        capability === "image"
            ? config.imageModels
            : capability === "video"
