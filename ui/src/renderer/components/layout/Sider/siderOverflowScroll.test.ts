...
describe('application sider overflow handling', () => {
  ...
  test('keeps workspace and workpath headings in the same sticky stack', () => {
    expect(styles.includes('.workspaceSectionHeader')).toBe(true);
    expect(styles.includes('top: 0;')).toBe(true);
    expect(styles.includes('.scrollArea :global(.flowy-workpath-drawer-header)')).toBe(true);
    expect(styles.includes('top: var(--flowy-workspace-section-height);')).toBe(true);
    expect(styles.includes('scroll-padding-block-start')).toBe(true);
  });
});
