...
pub fn wants_stylized_non_photoreal(user_style: &str) -> bool {
    ...
    for needle in EN {
        if lower_raw.contains(needle) {
            let negated = format!("not {needle}")
                .chars()
                .collect::<String>();
            let negated2 = format!("no {needle}");
            if lower_raw.contains(&negated) || lower_raw.contains(&negated2) {
                continue;
            }
            return true;
        }
...
fn looks_like_child_character(identifier: &str, features: &str) -> bool {
...
pub fn sanitize_image_prompt(prompt: &str) -> String {
    ...
    if !vacant
        && looks_like_child_character("", &out)
