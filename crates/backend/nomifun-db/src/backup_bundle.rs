...
use nomifun_common::{
    ...
    now_ms, storage_paths, validate_uuidv7,
};
...
pub async fn restore_backup_bundle(
    ...
    if destination_database.file_name().and_then(|name| name.to_str())
        != Some(storage_paths::DATABASE_FILE)
            ...
            != Some("storage-generation")
    {
        return Err(BackupError::InvalidManifest(
            format!(
                "complete restore requires canonical {} and storage-generation paths",
                storage_paths::DATABASE_FILE
            ),
        ));
    }
...
pub async fn restore_backup_data_dir(
    ...
    let result = async {
        ...
        let restored_database = staging.join(storage_paths::DATABASE_FILE);
...
fn remove_migration_lock_file(
    ...
    let lock_path = database_path.with_file_name(format!(
        ...
        database_path
            .file_name()
            .and_then(|name| name.to_str())
            .unwrap_or(storage_paths::DATABASE_FILE)
    ));
...
fn restore_relative_path(bundle_path: &str) -> Result<PathBuf, BackupError> {
    match bundle_path {
        DATABASE_FILE => Ok(PathBuf::from(storage_paths::DATABASE_FILE)),
