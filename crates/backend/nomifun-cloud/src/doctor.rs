...
pub async fn run_doctor(
    ...
    checks.push(DoctorCheck {
        ...
        detail: if config.enabled {
            ...
            "server integration disabled (set server.enabled or NOMIFUN_SERVER_ENABLED)".to_string()
        },
    });
    ...
    checks.push(DoctorCheck {
        ...
        detail: if base_ok {
            ...
            "base_url empty — set server.base_url or NOMIFUN_SERVER_URL".to_string()
        },
    });
    ...
    match AuthManager::new(config.clone(), &data_dir) {
        Ok(manager) => {
                    ...
                    if status.is_logged_in() {
                        match manager.fetch_profile().await {
                            Ok(profile) => {
                                ...
                                match DeviceActivation::new(&data_dir)
            ...
            if !stored.is_empty() && !nomi_config::is_valid_wechat_open_app_id(stored) {
                checks.push(DoctorCheck {
                    ...
                    detail: format!(
                        "stored wechat_app_id '{stored}' is invalid — \
                         ...
                         Update `server.channel` in config.yaml (current channel: {}).",
                        config.effective_wechat_app_id(),
                        config.channel
                    ),
                });
    ...
    let source = if std::env::var("NOMIFUN_SERVER_TOKEN")
        .ok()
