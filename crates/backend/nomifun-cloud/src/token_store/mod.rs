...
const TOKEN_STORE_KEY_ENV: &str = "NOMIFUN_TOKEN_STORE_KEY_B64";
...
impl FileTokenStore {
    pub async fn new(path: impl AsRef<Path>) -> Result<Self, CloudError> {
    ...
    pub async fn upsert(&self, credential: OAuthCredential) -> Result<(), CloudError> {
        self.cache
            .write()
            .await
    ...
    pub async fn remove(&self, provider: &str) -> Result<(), CloudError> {
    ...
    async fn flush(&self) -> Result<(), CloudError> {
        let content = {
            ...
            serde_json::to_vec_pretty(&envelope).map_err(|e| CloudError::Config(e.to_string()))?
        };
...
async fn load_token_cache(
...
) -> Result<(TokenCache, bool), CloudError> {
    ...
    let legacy: TokenCache =
        serde_json::from_str(&raw).map_err(|e| CloudError::Config(e.to_string()))?;
    tracing::warn!(
        path = %path.display(),
...
fn encrypt_token_cache(
    key: &[u8; TOKEN_STORE_KEY_BYTES],
    cache: &TokenCache,
) -> Result<TokenStoreEnvelope, CloudError> {
    let plaintext = serde_json::to_vec(cache).map_err(|e| CloudError::Config(e.to_string()))?;
    let cipher = Aes256Gcm::new_from_slice(key).map_err(|e| CloudError::Config(e.to_string()))?;
    let mut nonce_bytes = [0u8; TOKEN_STORE_NONCE_BYTES];
    getrandom(&mut nonce_bytes).map_err(|e| CloudError::Config(e.to_string()))?;
    let nonce = Nonce::from_slice(&nonce_bytes);
    let ciphertext = cipher
        .encrypt(nonce, plaintext.as_ref())
        .map_err(|_| CloudError::AuthFailed("failed to encrypt token store".into()))?;
...
fn decrypt_token_cache(
    ...
    let nonce = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|e| CloudError::Config(format!("invalid token store nonce: {}", e)))?;
    ...
    let ciphertext = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|e| CloudError::Config(format!("invalid token store payload: {}", e)))?;
    let cipher = Aes256Gcm::new_from_slice(key).map_err(|e| CloudError::Config(e.to_string()))?;
    let plaintext = cipher
        .decrypt(Nonce::from_slice(&nonce), ciphertext.as_ref())
        .map_err(|_| CloudError::AuthFailed("failed to decrypt token store".into()))?;
    serde_json::from_slice::<TokenCache>(&plaintext).map_err(|e| CloudError::Config(e.to_string()))
}

async fn load_or_create_store_key(path: &Path) -> Result<[u8; TOKEN_STORE_KEY_BYTES], CloudError> {
    ...
    getrandom(&mut key).map_err(|e| CloudError::Config(e.to_string()))?;
    let encoded = base64::engine::general_purpose::STANDARD.encode(key);
    write_file_private(path, encoded.as_bytes()).await?;
    Ok(key)
}

fn decode_store_key(encoded: &str) -> Result<[u8; TOKEN_STORE_KEY_BYTES], CloudError> {
    let decoded = base64::engine::general_purpose::STANDARD
        .decode(encoded)
        .map_err(|e| CloudError::Config(format!("invalid {}: {}", TOKEN_STORE_KEY_ENV, e)))?;
...
async fn write_file_private(path: &Path, content: &[u8]) -> Result<(), CloudError> {
...
async fn set_private_permissions(path: &Path) -> Result<(), CloudError> {
    #[cfg(unix)]
    {
...
pub type RefreshHandler = Arc<
    dyn Fn(
            String,
            String,
        ) -> futures::future::BoxFuture<'static, Result<OAuthCredential, CloudError>>
        + Send
        + Sync,
>;
...
impl AuthManager {
    ...
    pub async fn save_credential(&self, credential: OAuthCredential) -> Result<(), CloudError> {
        self.store.upsert(credential).await
    }

    pub async fn get_access_token(&self, provider: &str) -> Result<Option<String>, CloudError> {
        ...
        if credential.is_expired(30) {
            ...
            let handler = self
                ...
                .cloned()
                .ok_or_else(|| {
                    CloudError::AuthFailed(format!("no refresh handler for {}", provider))
                })?;
...
pub fn build_authorization_url(
    ...
    let mut url = Url::parse(&endpoints.authorize_url)
        .map_err(|e| CloudError::Config(format!("invalid authorize_url: {}", e)))?;
    {
...
pub async fn exchange_authorization_code(
    ...
    code: &str,
    code_verifier: &str,
) -> Result<OAuthCredential, CloudError> {
