...
/// (`nomifun-channel/src/message_service.rs`). The conversation path used to
/// ignore this, which is how a truncated turn that wrote nothing to disk was
/// recorded as `result_ok = 1`.
...
/// subset.
///
/// `committed_artifact_count` is the turn-scoped total of committed artifacts.
/// Local main does not yet accumulate this across continuation resends; callers
/// pass `0` until that tracking lands. Non-zero counts remain first-class
/// success evidence for native image turns that suppress provider prose.
...
mod tests {
    ...
    fn terminal(terminal: RelayTerminal, text: Option<&str>) -> RelayOutcome {
        RelayOutcome {
            terminal,
            stop_reason: None,
            final_text: text.map(str::to_owned),
    ...
    fn finish_with_text_is_success() {
        let outcome = finish(Some(TurnStopReason::EndTurn), Some("ok"));
    ...
    fn finish_without_a_stop_reason_is_success() {
        assert_eq!(
            map_turn_failure(&finish(None, Some("ok")), 0),
            None
        );
    }
    ...
    fn finish_with_empty_text_is_empty_final_text_not_retryable() {
        assert_eq!(
            map_turn_failure(&finish(Some(TurnStopReason::EndTurn), Some("  ")), 0),
    ...
    fn finish_without_text_is_empty_final_text_not_retryable() {
        assert_eq!(
            map_turn_failure(&finish(Some(TurnStopReason::EndTurn), None), 0),
            Some(("empty_final_text".into(), false))
        );
    }

    #[test]
    fn finish_at_output_ceiling_is_output_truncated_even_with_prose() {
        let prose = "Here is the complete implementation: ".repeat(64);
        let outcome = finish(Some(TurnStopReason::MaxTokens), Some(prose.as_str()));
        assert!(!turn_succeeded(&outcome, 0));
        assert_eq!(
    ...
    #[test]
    fn finish_at_output_ceiling_with_artifacts_is_still_truncated() {
        assert!(!turn_succeeded(
            &finish(Some(TurnStopReason::MaxTokens), None),
            3
        ));
    }

    #[test]
    fn finish_at_request_budget_is_turn_requests_exhausted() {
        assert_eq!(
            map_turn_failure(
                &finish(Some(TurnStopReason::MaxTurnRequests), Some("partial")),
                0
            ),
    ...
    #[test]
    fn finish_on_refusal_is_model_refused_not_retryable() {
        assert_eq!(
            map_turn_failure(
                &finish(Some(TurnStopReason::Refusal), Some("I can't help")),
                0
            ),
    ...
    #[test]
    fn finish_cancelled_maps_to_turn_cancelled() {
        assert_eq!(
            map_turn_failure(
                &finish(Some(TurnStopReason::Cancelled), Some("partial")),
                0
            ),
    ...
    fn channel_closed_is_retryable() {
        assert_eq!(
            map_turn_failure(&terminal(RelayTerminal::ChannelClosed, None), 0),
    ...
    fn every_incomplete_stop_reason_has_a_contracted_code() {
        for reason in [
            ...
            let code = incomplete_stop_code(Some(reason))
                .expect("incomplete stop reason must map to a fixed code");
            let (mapped, retryable) = map_turn_failure(&finish(Some(reason), Some("text")), 0)
                .expect("incomplete finish must be a failure");
