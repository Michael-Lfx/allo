...
/// The service resolves every path it hands out, so an expected path built from
/// the shared formula has to go through the same resolution before it can be
/// compared. Only the spelling differs, and only on some platforms: Windows
/// canonicalization adds the `\\?\` verbatim prefix and expands 8.3 short names,
/// and a macOS temp dir is reached through the `/var` → `/private/var` symlink.
...
async fn provisioning_the_workspace_returns_an_absolute_source_path_and_is_idempotent() {
    ...
    assert!(
        source.starts_with(canonical(nomifun_miniapp::miniapps_root(&services.work_dir))),
