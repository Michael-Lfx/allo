...
[progress] Plan fully completed. Prefer finishing: summarize results and stop unless verification \
is still required for a non-trivial change.\n";
...
impl Tool for UpdatePlanTool {
    ...
    async fn execute(&self, input: Value) -> ToolResult {
        ...
        if all_completed {
            prefix.push_str(COMPLETED_PLAN_NOTE);
        } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn execute_incomplete_plan_reminds_model_to_keep_progress() {
        let r = UpdatePlanTool::new()
            .execute(json!({
                "plan": [
        ...
        assert!(
            r.content.contains("incomplete step") || r.content.contains("Continue the next")
        );
        let start = r.content.find('{').unwrap();
        let v: serde_json::Value = serde_json::from_str(&r.content[start..]).unwrap();
    ...
    #[tokio::test]
    async fn execute_completed_plan_notes_finish() {
        let r = UpdatePlanTool::new()
            .execute(json!({
                "plan": [
                    { "step": "Inspect", "status": "completed" },
        ...
        assert!(!r.is_error);
        assert!(
            r.content.contains("fully completed") || r.content.contains("Prefer finishing")
        );
        let start = r.content.find('{').unwrap();
        let v: serde_json::Value = serde_json::from_str(&r.content[start..]).unwrap();
        assert_eq!(v["kind"], "plan_update");
