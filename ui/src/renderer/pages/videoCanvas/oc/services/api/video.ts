...
import { getMediaBlob, uploadMediaFile, type UploadedFile } from "@oc/services/file-storage";
import { getResourceOSSUrl } from "@oc/services/api/resources";
import { channelRequest } from "@oc/services/api/custom-channel-relay";
import { imageToDataUrl } from "@oc/services/image-storage";
import { modelCapabilityConfigFor, videoDurationAllowed } from "@oc/lib/model-capabilities";
import { boolConfig, buildSeedancePromptText, isArkPlanBaseUrl, isSeedanceVideoConfig, normalizeSeedanceDuration, normalizeSeedanceRatio, normalizeSeedanceResolution, seedanceVideoReferenceError, SEEDANCE_REFERENCE_LIMITS } from "@oc/lib/seedance-video";
import { buildApiUrl, isSystemProxyBaseUrl, modelOptionName, resolveModelRequestConfig, type AiConfig } from "@oc/stores/use-config-store";
...
function assertVideoCapability(profile: NonNullable<ReturnType<typeof modelCapabilityConfigFor>["video"]>, prompt: string, references: ReferenceImage[], videoReferences: ReferenceVideo[], audioReferences: ReferenceAudio[], seconds: string) {
    ...
    for (const audio of audioReferences) {
        if (profile.references.maxAudioBytes > 0 && ((audio as { bytes?: number }).bytes || 0) > profile.references.maxAudioBytes) throw new Error("参考音频文件超过当前模型大小限制");
