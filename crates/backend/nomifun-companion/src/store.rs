...
fn validate_suggestion_action(
    ...
    struct CreateSkillAction {
        ...
        /// Accepted so `deny_unknown_fields` does not reject a learner-written
        /// action; the service reads it back off the raw JSON on accept.
        #[serde(default)]
        #[allow(dead_code)]
...
impl CompanionStore {
    ...
    /// Grant the same XP to every listed companion. Accepting a suggestion is a
    /// shared achievement: the card is install-wide, so the whole roster grows.
    ...
    pub async fn find_recent_decided_similar(
        ...
        let norm_body = body.trim().to_lowercase();
        let rows = sqlx::query(
            "SELECT suggestion_id, title, body FROM companion_suggestions WHERE kind = ? AND status IN ('accepted','dismissed') AND decided_at >= ?",
        )
        .bind(kind)
        .bind(cutoff)
        .fetch_all(&self.pool)
...
impl CompanionStore {
    ...
    /// How many of a companion's skills are currently usable — the 专精 badge's
    /// number. Counted at read time rather than tracked as a counter so a skill
    /// archived by the evolution loop drops out of the badge on the next poll.
    pub async fn count_active_skills(&self, companion_id: &str) -> Result<i64, AppError> {
        validate_companion_id(companion_id, "skill companion_id")?;
        let n: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM companion_skills WHERE companion_id = ? AND status = 'active'",
        )
        .bind(companion_id)
        .fetch_one(&self.pool)
        .await
...
"#;
    ...
    #[tokio::test]
    async fn v3_upgrade_preserves_suggestions_learning_and_evolution_state() {
        let root = tempfile::tempdir().unwrap();
        let database_path = root.path().join("memory.db");
        ...
        let store = CompanionStore::open(root.path(), None).await.unwrap();
        // Suggestions and learn-run history stay: the 建议 cards, the 学习
        // timeline and the local analytics roll-up all still read them, so an
        // upgrade must carry their rows across, not drop the tables.
        assert_eq!(store.list_learn_runs(10).await.unwrap().len(), 1);
        assert_eq!(store.list_suggestions(None, 10).await.unwrap().len(), 1);
        assert_eq!(
