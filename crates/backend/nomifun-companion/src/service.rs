...
use crate::learner::{CompanionCompleter, Learner, SuggestionGate};
...
impl CompanionService {
    ...
    pub async fn learning_graph(
        ...
        let skills = self.store.list_skills(companion_id, true).await?;
        for skill in &skills {
...
mod tests {
    ...
    async fn consent_applies_once_and_never_reenables_after_disable() {
        let dir = tempfile::tempdir().unwrap();
        let svc = service(dir.path()).await;
        // fresh: work-source collection stays off until consent; learn already
        // defaults on, evolve stays off until consent flips it.
        assert!(!svc.config.read().await.collect.tool_calls);
        assert!(svc.config.read().await.learn.enabled);
        assert!(!svc.config.read().await.evolve.enabled);
