//! Background runtime dependency installation and tool-time waiting.
...
use crate::dep_gate::{self, NotifyFn};
...
impl Coordinator {
    ...
    fn start_install(&self, dep: RuntimeDep) {
        ...
        if !auto_ensure_enabled() {
            debug!(%dep, "NOMIFUN_AUTO_ENSURE_DEPS disabled; not starting background install");
...
/// Register dep-gate hooks so agent media tools can background-install and wait at tool time.
