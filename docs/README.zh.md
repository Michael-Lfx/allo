...
本目录保存 **Flowy** 当前的技术文档、运维文档与贡献者文档。当前规范性内容
位于 `architecture/` 与 `contributing/`。临时实施计划和已被替代的设计草稿
不再保留在 `docs/` 中；确需追溯时使用 Git 历史。`continuity/` 保留历史决策、
交接背景和发布审计，不得覆盖当前架构或贡献者规范。
