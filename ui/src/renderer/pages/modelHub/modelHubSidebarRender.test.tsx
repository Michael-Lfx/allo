...
describe('model hub sidebar renders', () => {
  ...
  test('对话 is selected by default, and the retired key resolves to its heir', () => {
    const fresh = render('/models');
    expect(fresh.includes('aria-labelledby="model-hub-tab-chat"')).toBe(true);

    // `?section=speech` was the voice host; 语音识别 inherits it.
    const legacy = render('/models?section=speech');
    expect(legacy.includes('aria-labelledby="model-hub-tab-asr"')).toBe(true);

    // `?section=creation` is still a real section — the cross-capability
    // overview was kept, not replaced by the per-capability split.
    const creation = render('/models?section=creation');
    expect(creation.includes('aria-labelledby="model-hub-tab-creation"')).toBe(true);
  });
});
