...
import { getNodeDefinition, getNodeGenerationMode, getNodeInputKind, listCreatableNodeDefinitions } from "@oc/lib/canvas/node-registry";
import { resolveAddNodeMenuCommands, type AddNodeMenuContext } from "@oc/lib/canvas/tool-registry";
import { ART_CRITIQUE_NODE_TYPE, ART_CRITIQUE_RUBRIC_VERSION } from "@oc/lib/art-critique/contracts";
import { ART_CRITIQUE_SEVERITY_COLORS, isRenderableArtCritiqueTarget, referenceTargetForIssue, repairIssueTarget, targetBounds, targetPolygon } from "@oc/lib/art-critique/annotation";
import { artCritiqueMessages, parseArtCritiqueResponse } from "@oc/lib/art-critique/review";
import {
    applyGrounding,
    applyEditPrompts,
...
} from "@oc/lib/art-critique/pipeline";
import { ART_CRITIQUE_RUBRIC, buildArtCritiqueRubricPrompt } from "@oc/lib/art-critique/rubrics";
import type { CanvasAgentTurnResult } from "@oc/lib/canvas/canvas-agent-llm";
import { CanvasNodeType } from "@oc/types/canvas";

function response(value: unknown): CanvasAgentTurnResult {
    return {
        content: "",
...
describe("AI 审美批改内置节点", () => {
    test("不作为生成参考素材，且没有 generationMode", () => {
        expect(getNodeInputKind(CanvasNodeType.ArtCritique)).toBeUndefined();
        expect(getNodeGenerationMode({
            id: "critique",
            type: CanvasNodeType.ArtCritique,
            title: "AI 审美批改",
            position: { x: 0, y: 0 },
            width: 560,
            height: 420,
        })).toBeNull();
        expect(getNodeDefinition(CanvasNodeType.ArtCritique)?.showInCreateMenu).toBe(true);
        expect(listCreatableNodeDefinitions().some((def) => def.type === CanvasNodeType.ArtCritique)).toBe(true);
    });
    ...
    test("添加节点菜单包含审美批改", () => {
        ...
        expect(commands.some((command) => command.id === ART_CRITIQUE_NODE_TYPE)).toBe(true);
    });
});
...
describe("AI 审美批改报告解析", () => {
    test("解析 AI 生成的局部编辑提示词并按问题 ID 应用", () => {
        ...
        } satisfies import("@oc/lib/art-critique/contracts").ArtCritiqueIssue;
...
describe("AI 审美批改多阶段管线", () => {
    ...
    test("跳过畸形候选，并把数字字符串收成置信度", () => {
        const result = parseSceneReviewPayload({
            ...
            candidates: [
                { id: "coerced", checkId: "visual-balance", category: "composition", title: "有效问题", observation: "有证据", reason: "影响表达", evidence: ["左侧重量明显"], severity: "0.8", confidence: "0.9", targetDescription: "左侧" },
                "not-an-object",
                { id: "broken", checkId: "visual-balance", category: "composition" },
            ],
        });
        expect(result.candidates.map((candidate) => candidate.id)).toEqual(["coerced"]);
        expect(result.candidates[0].confidence).toBe(0.9);
        expect(result.candidates[0].severity).toBe(0.8);
    });
