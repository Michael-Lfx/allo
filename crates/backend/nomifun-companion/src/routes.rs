...
#[derive(Deserialize)]
struct ListLearnRunsQuery {
    limit: Option<i64>,
}

/// Persisted learn-run history, install-wide (the 学习 tab timeline and the
/// local analytics roll-up both read it).
async fn list_learn_runs(
    State(state): State<CompanionRouterState>,
    Extension(_user): Extension<CurrentUser>,
    Query(query): Query<ListLearnRunsQuery>,
) -> Result<Json<ApiResponse<Vec<crate::store::CompanionLearnRun>>>, AppError> {
    Ok(Json(ApiResponse::ok(
        state.service.list_learn_runs(query.limit.unwrap_or(50)).await?,
    )))
}
...
) -> Result<Json<ApiResponse<crate::store::SuggestionPage>>, AppError> {
    Ok(Json(ApiResponse::ok(
        state
            .service
...
) -> Result<Json<ApiResponse<crate::store::CompanionSuggestion>>, AppError> {
    let Json(req) = body.map_err(|e| AppError::BadRequest(e.to_string()))?;
    Ok(Json(ApiResponse::ok(
        state.service.decide_suggestion(&suggestion_id, req.accept).await?,
    )))
}
...
async fn source_recommendations(
    ...
    Ok(Json(ApiResponse::ok(
        state.service.collector_source_stats().await?,
    )))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn learn_endpoint_is_companion_scoped_and_history_stays_install_wide() {
        ...
        // Run history stays install-wide and readable: the 学习 timeline and the
        // analytics roll-up both read it.
        let history = app
            .oneshot(
