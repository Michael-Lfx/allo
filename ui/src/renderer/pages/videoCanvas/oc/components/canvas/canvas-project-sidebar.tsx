...
export function CanvasProjectSidebar({ projectId, detail, onAddChapter, onLocateStyle, onOpenStyle, onOpenAssets }: CanvasProjectSidebarProps) {
    ...
    const visibleUnits = useMemo(() => {
        ...
        return orderedUnits.reduce<Array<{ unit: ProjectUnit; index: number }>>((result, unit, index) => {
            ...
            const matchesTitle = unit.title.toLocaleLowerCase().includes(deferredQuery);
    ...
    return (
        <aside className="relative z-[var(--z-panel)] hidden w-[var(--canvas-sidebar-width)] shrink-0 flex-col border-r border-border bg-background/94 backdrop-blur-xl lg:flex">
            <header className="flex h-12 shrink-0 items-center justify-between gap-2 border-b border-border px-2.5">
                ...
                <button type="button" className="grid size-7 shrink-0 place-items-center rounded-md text-foreground/45 hover:bg-foreground/[.06]" title={canvasT("videoCanvas.sidebar.collapse", "收起项目侧栏")} aria-label={canvasT("videoCanvas.sidebar.collapse", "收起项目侧栏")} onClick={() => setCollapsed(true)}>
            ...
            <section className="shrink-0 border-b border-border/70 p-2">
                ...
                >
                    ...
                    <span className="min-w-0 flex-1">
                        <span className="block truncate text-xs font-medium">{style?.title || canvasT("videoCanvas.sidebar.styleNotSet", "尚未设置画风")}</span>
                        <span className="mt-0.5 block truncate text-[var(--fs-micro)] text-foreground/38">{style ? canvasT("videoCanvas.sidebar.styleSynced", "随项目设置自动同步") : canvasT("videoCanvas.sidebar.styleGoSettings", "前往项目设置选择")}</span>
                    </span>
            ...
            <section className="flex min-h-0 flex-1 flex-col">
                <div className="flex h-9 shrink-0 items-center justify-between px-3">
                    <span className="flex items-center gap-1.5 text-[var(--fs-label)] font-medium text-foreground/48">
                        ...
                        {canvasT("videoCanvas.sidebar.chapters", "剧情章节")} <span className="tabular-nums text-foreground/32">{orderedUnits.length.toLocaleString()}</span>
                    </span>
                    {selectedId ? (
                        <button type="button" onClick={revealSelectedChapter} className="grid size-6 place-items-center rounded text-foreground/35 hover:bg-foreground/[.06] hover:text-foreground" title={canvasT("videoCanvas.sidebar.revealChapter", "回到当前章节")} aria-label={canvasT("videoCanvas.sidebar.revealChapter", "回到当前章节")}>
                ...
                <div className="shrink-0 px-2 pb-2">
                    <label className="flex h-8 items-center gap-1.5 rounded-md border border-border/75 bg-foreground/[.025] px-2 focus-within:border-[var(--workspace-accent)] focus-within:ring-2 focus-within:ring-[var(--workspace-accent-soft)]">
                        ...
                        <input
                            ...
                            placeholder={canvasT("videoCanvas.sidebar.searchPlaceholder", "搜索标题或输入章节序号")}
                            aria-label={canvasT("videoCanvas.sidebar.searchAria", "搜索项目章节")}
                        />
                        {query ? (
                            <button type="button" onClick={() => setQuery("")} className="grid size-5 shrink-0 place-items-center rounded text-foreground/32 hover:bg-foreground/[.06] hover:text-foreground" aria-label={canvasT("videoCanvas.sidebar.clearSearch", "清空章节搜索")}>
                                <X className="size-3" />
                            </button>
                        ) : null}
                    </label>
                    {deferredQuery ? <div className="mt-1 px-0.5 text-[var(--fs-micro)] tabular-nums text-foreground/35">{canvasT("videoCanvas.sidebar.foundChapters", "找到 {{count}} 章", { count: visibleUnits.length.toLocaleString() })}</div> : null}
                </div>

                <div ref={scrollRef} className="thin-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-contain px-1.5" aria-label={canvasT("videoCanvas.sidebar.chapterListAria", "项目章节列表")}>
                    {visibleUnits.length ? (
                                    ...
                                    <div key={unit.id} className="absolute left-0 top-0 w-full" style={{ height: virtualItem.size, transform: `translateY(${virtualItem.start}px)` }}>
                                        ...
                                        >
                                            ...
                                            <button
                                                ...
                                                aria-label={canvasT("videoCanvas.sidebar.previewChapterAria", "预览第 {{n}} 章 {{title}}", { n: index + 1, title: unit.title })}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={canvasT("videoCanvas.sidebar.addToCanvasCenter", "添加到画布中央")}
                                                aria-label={canvasT("videoCanvas.sidebar.addChapterAria", "添加{{title}}到画布", { title: unit.title })}
                        ...
                        <div className="px-2 py-8 text-center text-xs text-foreground/40">{orderedUnits.length ? canvasT("videoCanvas.sidebar.noMatch", "没有匹配的章节") : canvasT("videoCanvas.sidebar.noChapters", "暂无章节")}</div>
                    )}
                </div>
            </section>
            ...
            <section className="shrink-0 border-t border-border/70 p-2">
                <button type="button" onClick={onOpenAssets} className="flex h-9 w-full items-center gap-2 rounded-md border border-border/80 px-2 text-xs text-foreground/65 hover:border-[var(--workspace-accent)] hover:bg-[var(--workspace-accent-soft)]">
                    ...
                    <span className="flex-1 text-left">{canvasT("videoCanvas.sidebar.refAssets", "引用项目资产")}</span>
                    <span className="rounded bg-foreground/[.06] px-1.5 py-0.5 text-[var(--fs-micro)] tabular-nums text-foreground/45">{mediaAssetCount.toLocaleString()}</span>
                </button>
            </section>
...
function ChapterPreview({
    ...
    return (
        <section
            ...
            aria-label={canvasT("videoCanvas.sidebar.chapterPreviewAria", "第 {{n}} 章预览", { n: chapterNumber })}
        >
            <header className="flex items-start gap-2 border-b border-border/70 px-3 py-2.5">
                <div className="min-w-0 flex-1">
                    <div className="text-[var(--fs-tiny)] tabular-nums text-foreground/38">{canvasT("videoCanvas.sidebar.chapterN", "第 {{n}} 章", { n: chapterNumber.toLocaleString() })}</div>
                    <h2 className="mt-0.5 truncate text-sm font-semibold">{unit.title}</h2>
                </div>
                <button type="button" onClick={onClose} className="grid size-6 shrink-0 place-items-center rounded text-foreground/38 hover:bg-foreground/[.06] hover:text-foreground" aria-label={canvasT("videoCanvas.sidebar.closePreview", "关闭章节预览")}>
                    <X className="size-3.5" />
                </button>
            </header>
            <div className="thin-scrollbar min-h-0 flex-1 overflow-y-auto px-3 py-3">
                <p className="whitespace-pre-wrap text-xs leading-5 text-foreground/58">{loading ? canvasT("videoCanvas.sidebar.loadingBody", "正在读取本章正文…") : loadError ? canvasT("videoCanvas.sidebar.loadBodyFailed", "正文读取失败，请稍后重试") : text || canvasT("videoCanvas.sidebar.noBody", "本章还没有正文")}</p>
                {!loading && !loadError ? <div className="mt-2 text-[var(--fs-tiny)] tabular-nums text-foreground/35">{canvasT("videoCanvas.sidebar.charCount", "{{count}} 字", { count: text.length.toLocaleString() })}</div> : null}
            </div>
