...
export function CanvasProjectStatusDialogs({ theme, task, taskLogs, taskLoading, previewNode, clearConfirmOpen, onCloseTask, onClosePreview, onCancelClear, onConfirmClear }: CanvasProjectStatusDialogsProps) {
    ...
    const logText = taskLoading
        ? canvasT("videoCanvas.dialog.loading", "加载中...")
        : taskLogs.length
          ? taskLogs.map((log) => `[${new Date(log.createdAt).toLocaleString()}] ${log.level.toUpperCase()} ${log.message}`).join("\n")
          : canvasT("videoCanvas.dialog.noLogs", "暂无日志");
    return (
        <>
            <CanvasSheet open={Boolean(task)} theme={theme} width={760} title={canvasT("videoCanvas.dialog.taskDetails", "任务详情")} onClose={onCloseTask}>
                {task ? (
                    <div className="space-y-4 text-sm">
                        ...
                        <div>
                            ...
                            <pre className="max-h-64 overflow-auto rounded-lg p-3 text-[var(--fs-label)] leading-5" style={{ background: theme.node.fill, color: theme.node.text }}>{logText}</pre>
                        </div>
                    </div>
                ) : null}
            </CanvasSheet>
            ...
            <CanvasSheet
                open={Boolean(previewNode?.type === CanvasNodeType.Video && canvasNodeDisplayUrl(previewNode))}
                theme={theme}
                width="min(1200px, calc(100vw - 32px))"
                title={canvasT("videoCanvas.dialog.videoPreview", "视频预览")}
            ...
            <CanvasSheet
                ...
                footer={
                    <>
                        <CanvasSheetButton theme={theme} className="ml-auto" onClick={onCancelClear}>{canvasT("videoCanvas.dialog.cancel", "取消")}</CanvasSheetButton>
                        <CanvasSheetButton theme={theme} variant="danger" onClick={onConfirmClear}>{canvasT("videoCanvas.dialog.clear", "清空")}</CanvasSheetButton>
                    </>
                }
            >
                <p className="m-0 text-sm" style={{ color: theme.node.muted }}>{canvasT("videoCanvas.dialog.clearCanvasBody", "这会删除当前画布上的所有节点和连线。")}</p>
            </CanvasSheet>
        </>
    );
}
