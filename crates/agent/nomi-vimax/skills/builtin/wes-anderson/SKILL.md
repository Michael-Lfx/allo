---
name: wes-anderson
display-name: 韦斯·安德森美学
description: >-
  Directs Wes Anderson–inspired films with bilateral symmetry, pastel palettes,
  planar staging, and wry sparse narration. Use when the user mentions Wes
  Anderson, symmetry, pastel tableaux, whimsical production design, or formal
  playful aesthetics.
category: aesthetic
version: "1.1.0"
tags: [wes-anderson, symmetry, whimsical, aesthetic, pastel]
requirement-overlay: |
  Direct with Wes Anderson–inspired formal playfulness.
  Prioritize: centered symmetry, planar staging, deadpan performances,
  chapter-like scene cards, meticulous prop worlds, and witty sparse narration.
  Keep camera mostly locked-off or lateral; favor tableau over handheld chaos.
  Avoid gritty realism, shaky action, and dark horror tones unless requested.
  QA: frames should feel designed as posters; color and props must feel curated.
style-overlay: |
  Wes Anderson aesthetic, perfect bilateral symmetry, pastel color palette,
  planar composition, centered subjects, miniature-like sets, whimsical props,
  flat even lighting, retro production design, tableau staging
---

# 韦斯·安德森美学 Playbook

## When to apply
- Stylized comedy / fable tones with strong production design
- Any request for symmetry-first, poster-like frames

## 结构
1. 标题卡 / 章节感开场
2. 对称场景建立人物关系
3. 怪诞细节推动情节
4. 冷静旁观的情感落点

## 镜头语法
- 正中构图、水平移动、固定机位
- 角色走位像舞台调度
- 道具与服装成为叙事符号

## 资产规范
- 马卡龙或复古限定色板
- 场景高度人工、整洁、符号化
