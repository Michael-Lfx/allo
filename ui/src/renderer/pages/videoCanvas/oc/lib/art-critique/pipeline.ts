import { requestCanvasAgentTurn, type ResponseFunctionTool, type ResponseInputMessage } from "@oc/lib/canvas/canvas-agent-llm";
import { resolveCanvasVisionModel } from "@oc/lib/canvas/canvas-vision-model";
import type { AiConfig } from "@oc/stores/use-config-store";

import { reviewArtCritiqueImage, type ArtCritiqueReviewInput } from "./review";
import {
...
export async function runArtCritiquePipeline(config: AiConfig, input: ArtCritiqueReviewInput, options: ArtCritiquePipelineOptions = {}) {
    const visionModel = resolveCanvasVisionModel(config);
    if (!visionModel) throw new Error("未配置支持图片理解的文本模型（模型 extra.input 需包含 image）");
    const critiqueConfig = { ...config, model: visionModel, textModel: visionModel };
    ...
    if (!reviewerSucceeded) {
        const firstCause = reviewerResults.find((result) => result.status === "rejected")?.reason;
        rethrowIfAborted(firstCause, options.signal);
        warnings.push("多通道分析未完成，已改用综合批改。");
        try {
            const fallbackReport = await reviewArtCritiqueImage(critiqueConfig, input, options);
            return {
                ...fallbackReport,
                scene,
            ...
            throw firstCause instanceof Error ? firstCause : error instanceof Error ? error : new Error("art_critique_pipeline_failed");
        }
    }
...
async function requestPipelineStage<T>(config: AiConfig, messages: ResponseInputMessage[], tool: ResponseFunctionTool, toolName: string, parse: (value: unknown) => T, options: ArtCritiquePipelineOptions) {
    ...
    const response = await requestCanvasAgentTurn(config, messages, [tool], { type: "function", function: { name: toolName } }, { signal: options.signal, stream: false });
    const named = response.toolCalls.find((candidate) => candidate.function.name === toolName);
    const raw = named?.function.arguments || response.toolCalls[0]?.function.arguments || response.content;
...
function numeric(value: unknown) {
    ...
    if (typeof value === "string" && value.trim()) {
        const named: Record<string, number> = { high: 0.9, medium: 0.6, low: 0.3, 高: 0.9, 中: 0.6, 低: 0.3 };
