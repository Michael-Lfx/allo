    ...
    const renderComposerHeader = (expanded: boolean) => (
        <div className="flex min-w-0 items-center gap-1 px-0.5">
            {isPortraitTexture ? (
                ...
                <div className={`flex h-6 min-w-0 items-center gap-1 rounded-md px-1.5 transition-colors ${darkSurface ? "hover:bg-white/[.04]" : "hover:bg-black/[.04]"}`} style={{ background: controlSurface }}>
        ...
        >
            ...
            {mode === "video" && !simpleMode ? (
                <div className="mt-1.5 overflow-hidden rounded-md border" style={{ background: controlSurface, borderColor: insetBorder }}>
                    <button
                        type="button"
                        className={`flex w-full items-center gap-1.5 px-2 py-1 text-[var(--fs-micro)] font-medium transition-colors ${darkSurface ? "hover:bg-white/[.04]" : "hover:bg-black/[.04]"}`}
