...
/// Which half of a request URL owns the API version segment.
///
/// On the nomi redesign this lives on protocol endpoint descriptors in
/// omifun-api-types\. Mike has not absorbed that registry yet, so the enum
/// is defined here with the algebra that consumes it.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum EndpointRootShape {
    /// Connection root carries the version (\https://host/v1\); endpoint
    /// template is version-free (\/chat/completions\).
    VersionedRoot,
    /// Connection root is version-free (\https://host\); endpoint template
    /// carries the version (\/v1/messages\, \/v1beta/...\).
    OriginRoot,
}
