...
describe('mini-app composer entry wiring', () => {
  test('entry strip exposes the entry, the dismissible active token, and its i18n keys', () => {
    ...
    // IconPark: plain named import, never aliased or namespaced. An aliased
    // import survives tsc but the build-time icon rewrite turns it into illegal
    // syntax, so the module 500s at runtime.
    const iconImportLine = entryStrip.split('\n').find((line) => line.includes('@icon-park/react')) ?? '';
    expect(iconImportLine.includes('ApplicationOne')).toBe(true);
