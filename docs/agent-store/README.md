...
> 最后核对：2026-09-10（Team 触发方式修订，见 `agent-store-v1-roadmap.md` §10 决策 3）
> 用途：本目录文档的地图、权威顺序与状态图例。新会话/新成员先读本文，再按需深入。
> 维护约定：新增文档编号顺延（当前到 `20`）；状态变更时同步更新本索引与文档头部“更新”行；公共契约变更先写 `agent-store-v1-roadmap.md` §10 决策记录，再改基线文档。
...
| `mcp-oauth-runtime-evidence.zh.md` | OAuth 登录 → 注入 → 401 刷新（真实 MCP 服务，引擎层） |
| `oauth-runtime-evidence.zh.md` | WP-3 P0-C/D OAuth 协议面证据（TC-OAUTH-001/002/004，SDK 公共面 26/26 PASS） |
