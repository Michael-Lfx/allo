...
use super::{FlowyMediaServices, MediaVideo, map_model_err, map_server_err};
...
impl FlowyVideo {
    ...
    async fn resolve_model(&self) -> MediaBackendResult<String> {
        ...
        catalog
            ...
            .map(|m| m.id.clone())
            .filter(|s| !s.is_empty())
            .ok_or_else(|| MediaBackendError::Video("no Flowy video model in catalog".into()))
    }

    /// Read a local frame, optionally shrink it, upload via OSS presign PUT, return `publicUrl`.
    async fn frame_public_url(&self, path: &Path, role: &str) -> MediaBackendResult<String> {
...
#[async_trait]
impl MediaVideo for FlowyVideo {
    async fn generate(
        &self,
        prompt: &str,
        ...
        if self.is_cancelled() {
            return Err(MediaBackendError::Cancelled);
        }
        ...
        if uses_frame_roles {
            ...
            let mut slots: Vec<Option<(String, MediaBackendResult<String>)>> = (0..n)
                .map(|_| None)
                .collect();
            while let Some(joined) = set.join_next().await {
                let (i, stem, url) =
                    joined.map_err(|e| MediaBackendError::Video(format!("ref upload join: {e}")))?;
        ...
        let duration = duration_secs.clamp(
            crate::video_quality::MIN_CLIP_DURATION_SECS,
            crate::video_quality::MAX_CLIP_DURATION_SECS,
        );
        ...
        let _permit = global_video_gate()
            .acquire()
            .await
            .map_err(|_| MediaBackendError::Video("video rate-limit gate closed".into()))?;
                ...
                match second {
                    ...
                    Err(e2) if is_seedance_caption_empty_err(&e2) => {
                        ...
                        self.services
                            ...
                            .map_err(|e3| {
                                if self.is_cancelled() {
                                    return MediaBackendError::Cancelled;
                                }
                                map_model_err(
                                    "video",
        ...
        let url = record.video_url().ok_or_else(|| {
            if record.is_success() {
                MediaBackendError::Video("video task succeeded but no video_url".into())
            } else {
                // Surface the upstream failure reason (e.g. InputTextSensitiveContentDetected)
                // instead of a misleading success message.
                MediaBackendError::Video(video_task_failure_message(&record))
            }
        })?;
...
async fn download_video(url: &str, out_path: &Path) -> MediaBackendResult<()> {
    let resp = reqwest::Client::new()
        .get(url)
        .send()
        .await
        .map_err(|e| MediaBackendError::Video(e.to_string()))?;
...
async fn download_image_still(url: &str, out_path: &Path) -> MediaBackendResult<()> {
    let resp = reqwest::Client::new()
        .get(url)
        .send()
        .await
