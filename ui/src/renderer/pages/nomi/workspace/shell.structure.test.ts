...
/**
 * The files this redesign actually wrote: the shell, the roster panel and the tab
 * tree. Surfaces carried over from the previous page (数据分析, 学习图谱, 建议,
 * 知识库, 凭据, the figure library) are reachable but were not rewritten here, so
 * holding them to this redesign's house style would only mean touching every one
 * of them for no behavioural gain. Deletion invariants still judge the whole page.
 */
const REDESIGNED_PREFIXES = ['workspace', 'CompanionSidebar', 'index.tsx', 'useNomi.ts', 'CompanionModelControl.tsx'];
const isRedesigned = ({ name }: Source): boolean =>
  REDESIGNED_PREFIXES.some((prefix) => name === prefix || name.startsWith(`${prefix}\\`) || name.startsWith(`${prefix}/`));

const redesignedOffenders = (predicate: (src: Source) => boolean): string[] =>
  corpus.filter((src) => isRedesigned(src) && predicate(src)).map((src) => src.name);

describe('workspace tab registry', () => {
  test('is the eight tabs, in reading order', () => {
...
describe('deleted features stay deleted', () => {
  test('建议 is confined to the 洞察 tab', () => {
    // Suggestions survived the redesign (the queue is still generated and still
    // needs deciding), but only as one section of 洞察 — the tab that shows what the
    // companion worked out. Any OTHER workspace surface reading suggestions would be
    // a second, competing inbox. The memory MERGE assistant is an unrelated live
    // feature whose endpoint is literally named merge-suggestions.
    expect(
      redesignedOffenders(
        ({ name, code }) =>
          !name.includes('InsightsTab') &&
...
describe('house style', () => {
  ...
  test('colour ramps go through the project rule, not an arbitrary rgb()', () => {
    ...
    expect(
      redesignedOffenders(({ raw }) =>
        /\b(?:text|bg|border)-\[rgb\(var\(--(?:primary|danger|success|warning)-[1-9]\)\)\]/.test(raw)
      )
    ).toEqual([]);
  });

  test('icons come from @icon-park/react only', () => {
    expect(redesignedOffenders(({ raw }) => raw.includes('@arco-design/web-react/icon'))).toEqual([]);
  });
  ...
  test('every source file carries the license header', () => {
    expect(redesignedOffenders(({ raw }) => !raw.startsWith('/**\n * @license'))).toEqual([]);
  });
});
