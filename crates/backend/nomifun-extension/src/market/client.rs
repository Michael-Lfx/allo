...
use reqwest::header::{ACCEPT, ACCEPT_LANGUAGE, RETRY_AFTER, HeaderMap, HeaderValue};
...
/// response body must restart its own consumption (and any partially written
/// output file) per attempt.
///
...
/// exhausted transport error as the mapped [`AppError`].
