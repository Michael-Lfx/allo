...
Fields: idx (int from 0), identifier_in_scene, is_visible (camera presence only — false for phone/VO/off-screen speakers, who MUST still be listed), static_features (MUST state sex/age band then appearance/physique, e.g. 成年女性、长发), dynamic_features (clothing/accessories, optional), voice_profile (REQUIRED film-stable speaking voice bible reused across every shot — timbre must be a concrete acoustic fingerprint with age/gender/resonance/texture; volume quiet|normal|loud; pitch low|mid-low|mid|mid-high|high; speaking_style = pace+diction+emotional baseline only; caption_clause optional). Natural-language field values MUST match the user's input language (Chinese → 简体中文; English → English)."#;
...
One entry per input character (same idx / identifier_in_scene). voice_profile must be film-stable and distinctive: timbre = concrete acoustic fingerprint matching the character's sex/age (not vague 女声/男声, never default everyone to 男中音); volume quiet|normal|loud; pitch low|mid-low|mid|mid-high|high; speaking_style = pace+diction+baseline only (no per-shot crying/shouting baked into timbre). caption_clause may be null (pipeline rebuilds FIXED SPEAKER VOICE inject). Do not emit tts_voice. Prose MUST match the user's input language (Chinese → 简体中文; English → English)."#;
...
One entry per input image. category MUST be exactly one of: character, environment, prop, style. photo_id MUST match the provided id. summary = one short visual description. suggested_label may be empty. Prose MUST match the user's label language (Chinese → 简体中文; English → English)."#;
...
environments: one entry per distinct location+time of day from scene headings (do not collapse the film to a global top-5). props: few named plot-critical objects only. description/name prose MUST match the user's input language (Chinese → 简体中文; English → English; slugline may stay INT./EXT. style)."#;
}
