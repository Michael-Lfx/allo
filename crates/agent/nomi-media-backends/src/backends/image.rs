...
use super::{FlowyMediaServices, MediaImage, map_model_err, map_server_err};
...
impl FlowyImage {
    ...
    async fn resolve_model(&self) -> MediaBackendResult<String> {
        ...
        catalog
            ...
            .map(|m| m.id.clone())
            .filter(|s| !s.is_empty())
            .ok_or_else(|| MediaBackendError::Image("no Flowy image model in catalog".into()))
    }
    ...
    async fn resolve_reference_urls(
        ...
        .await
        .map_err(|e| MediaBackendError::Image(format!("compose ref strip join: {e}")))??;
        Ok(vec![path_to_data_url(&composed_path).await?])
    }
}

#[async_trait]
impl MediaImage for FlowyImage {
    async fn generate(
        &self,
        prompt: &str,
        ref_image_paths: &[&Path],
...
async fn path_to_data_url(path: &Path) -> MediaBackendResult<String> {
