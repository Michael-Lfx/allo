import { requestCanvasAgentTurn, type CanvasAgentTurnResult, type ResponseFunctionTool, type ResponseInputMessage } from "@oc/lib/canvas/canvas-agent-llm";
...
export async function reviewArtCritiqueImage(config: AiConfig, input: ArtCritiqueReviewInput, options?: ArtCritiquePipelineOptions) {
    ...
    const response = await requestCanvasAgentTurn(
        config,
        artCritiqueMessages(input),
        [artCritiqueTool],
        { type: "function", function: { name: ART_CRITIQUE_TOOL_NAME } },
        { signal: options?.signal, stream: false },
    );
...
export function parseArtCritiqueResponse(response: CanvasAgentTurnResult, input: ArtCritiqueReviewInput): ArtCritiqueReport {
    const call = response.toolCalls.find((candidate) => candidate.function.name === ART_CRITIQUE_TOOL_NAME) || response.toolCalls[0];
    ...
    const issues = prioritizeIssues(
        value.issues.slice(0, 8).flatMap((item, index) => {
            try {
                return [parseIssue(item, index)];
            } catch {
                return [];
            }
        }),
...
function parseIssue(value: unknown, index: number): ArtCritiqueIssue {
    ...
    const namedSeverity = enumValue(value.severity, ["low", "medium", "high"] as const);
    const confidence = numeric(value.confidence);
    const severity = namedSeverity || severityFromNumeric(numeric(value.severity));
...
function severityFromNumeric(value: number | null): "low" | "medium" | "high" | null {
