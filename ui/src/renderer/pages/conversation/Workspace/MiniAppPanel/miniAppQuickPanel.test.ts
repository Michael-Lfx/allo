...
describe('the rail entry sits below the terminal icon, exactly once', () => {
  ...
  test('it reuses the mini-app glyph and its existing nav label', () => {
    ...
    expect(descriptor.includes("title: t('miniApps.nav.entry')")).toBe(true);
    expect(descriptor.includes('<ApplicationOne size={18} />')).toBe(true);
    expect(descriptor.includes('content: <MiniAppPanel />')).toBe(true);
  });
  ...
  test('the entry is unconditional, so a stored selection is never overwritten', () => {
    ...
    // choice. The descriptor itself therefore has no gate of its own.
    expect(descriptor.includes('return [];')).toBe(false);
    expect(/useConversationMiniAppTab\(\): WorkspaceExtraTab \{/.test(descriptor)).toBe(true);
  });
});
...
describe('the panel avoids retired collaboration vocabulary', () => {
  test('no retired terms in the panel or the registration site', () => {
    for (const source of [panel, descriptor]) {
