...
- 范围：一个面板统一承载——① `/` 命令（内置动作 + 目录 `quick_prompts` / `defaultInitPrompt`）；② `@` 提及（专家 / 技能 / 连接器，前缀过滤，写入结构化 `mentions`）；③ 会话动作（新建 / 重命名 / 删除 / 归档）；④ 模型与思考等级切换；⑤ 键盘导航（↑↓ / Enter / Esc）与 IME 兼容。
- 验收：输入 `/` 或 `@` 或按 `Cmd+K` 都能唤起；选择后 `mentions` 与草稿文本一致（删除 token 同步移除结构化项）；不与 `+` 目录菜单抢占。
...
| 清单项 | 实际状态 | 归属 |
| --- | --- | --- |
| §1.1 正式认证与令牌管理 | ❌ 未做 | 未纳入（主计划 E 延后） |
| §1.2 Origin / CSP / CSRF | ❌ 未做 | 未纳入 |
| §1.3 WS 断线重连与多标签协调 | ❌ 未做 | W8 |
| §2.1 消息重试 / 编辑 / 重新生成 | ❌ 未做 | W7 |
| §2.2 归档 / 回收站 | ❌ 未做 | 未纳入（后续立项） |
| §2.3 批量操作 | ❌ 未做 | 未纳入 |
| §2.4 历史分页 / 虚拟列表 | 🟡 虚拟列表已用 `@tanstack/react-virtual`；游标分页有 `history-cursor.ts` | 已基本满足 |
| §3.1 Request ID / 错误上报 | ❌ 未做 | 未纳入 |
| §3.2 健康检查 | ❌ 未做 | 未纳入 |
| §3.3 配额 / 限流 | ❌ 未做 | 未纳入 |
| §4.1 只读 / 不可访问标识 | ❌ 未做 | 未纳入 |
| §4.2 重命名注册 | 🟡 会话重命名有；工作区重命名无 | 后续立项 |
