...
use nomifun_insights::local_analytics::{
    ConversationAnalytics, LearningAnalytics, LocalAnalytics, MemoryAnalytics, MemoryKindCounts,
    SkillAnalytics, SkillSourceCounts, SkillStatusCounts, SkillUsageEntry,
};
use serde::Serialize;
...
use crate::learner::{CompanionCompleter, CompanionLearnResult, Learner, SuggestionGate};
...
pub struct CompanionStatus {
    ...
    /// Undecided proactive suggestions. Install-wide: the suggestion queue has no
    /// owner column, so every companion's card reports the same backlog.
    pub suggestions_new: i64,
    /// This companion's active (usable) skills — drives the "专精 N 技能" badge.
    pub skills_active: i64,
    pub model_configured: bool,
    pub collect_any_enabled: bool,
    /// Most recent learn run, whichever companion produced it.
...
impl CompanionService {
    ...
    pub async fn start_with_provider_lifecycle(
        ...
        let learner = Arc::new(Learner {
            ...
            gate: SuggestionGate::production(),
    ...
    pub async fn status(&self) -> Result<CompanionStatus, AppError> {
        ...
        Ok(CompanionStatus {
            ...
            suggestions_new: self.store.count_suggestions("new").await?,
            // No companion exists to own a skill.
            skills_active: 0,
    ...
    pub async fn list_learn_runs(
        &self,
        limit: i64,
    ) -> Result<Vec<crate::store::CompanionLearnRun>, AppError> {
    ...
    pub async fn collector_source_stats(
        &self,
    ) -> Result<Vec<crate::collector::SourceRecommendation>, AppError> {
        crate::collector::compute_source_recommendations(&self.shared_dir)
    }

    pub async fn learning_graph(
        &self,
        companion_id: &str,
    ) -> Result<LearningGraph, AppError> {
        if self.registry.get(companion_id).await.is_none() {
            return Err(AppError::NotFound(format!(
                "companion '{companion_id}' not found"
            )));
        }
        let mut nodes = Vec::new();
        let mut edges = Vec::new();

        let skills = self.store.list_skills(companion_id).await?;
        for skill in &skills {
            nodes.push(LearningGraphNode {
                id: skill.companion_skill_id.clone(),
                label: skill.skill_name.clone(),
                node_type: "skill".into(),
                status: skill.status.clone(),
                kind: skill.source.clone(),
                strength: skill.strength,
                usage_count: skill.usage_count,
                importance: skill.confidence,
            });
            for event_id in &skill.provenance_event_ids {
                edges.push(LearningGraphEdge {
                    source: event_id.clone(),
                    target: skill.companion_skill_id.clone(),
        ...
        for memory in &memories {
            nodes.push(LearningGraphNode {
                ...
                label: memory.content.chars().take(60).collect(),
        ...
        let suggestions = self.store.list_suggestions(None, 50).await?;
        for suggestion in &suggestions {
            let suggestion_id = format!("suggestion:{}", suggestion.suggestion_id);
            nodes.push(LearningGraphNode {
            ...
            if suggestion.kind == "create_skill"
                && let Some(companion_skill_id) = suggestion
                    .action
                    .as_ref()
                    .and_then(|action| action.get("companion_skill_id"))
                    .and_then(|value| value.as_str())
            {
                edges.push(LearningGraphEdge {
                    source: suggestion_id,
                    target: companion_skill_id.to_owned(),
    ...
    pub async fn local_analytics(&self) -> Result<LocalAnalytics, AppError> {
        let all_skills = self.store.list_all_skills().await?;
        let active_skills = all_skills
            .iter()
        ...
        let avg_strength = average(
            active_skills.iter().map(|skill| skill.strength),
            active_skills.len(),
        );
        let avg_confidence = average(
            active_skills.iter().map(|skill| skill.confidence),
            active_skills.len(),
        );
        let mut top_by_usage = all_skills
            .iter()
            .map(|skill| SkillUsageEntry {
                name: skill.skill_name.clone(),
                usage_count: skill.usage_count,
                strength: skill.strength,
            })
            .collect::<Vec<_>>();
        top_by_usage.sort_by(|left, right| right.usage_count.cmp(&left.usage_count));
        ...
        let avg_importance = average(
            memories.iter().map(|memory| memory.importance),
            memories.len(),
        );
        let avg_memory_strength = average(
            memories.iter().map(|memory| memory.strength),
            memories.len(),
        );
        ...
        Ok(LocalAnalytics {
            conversations: ConversationAnalytics::default(),
            ...
            learning: LearningAnalytics {
                ...
                runs_7d: runs
                    .iter()
                    .filter(|run| run.started_at >= seven_days_ago)
                    .count() as u64,
                total_memories_added: runs
                    .iter()
                    .map(|run| run.memories_added.max(0) as u64)
                    .sum(),
                total_suggestions_added: runs
                    .iter()
                    .map(|run| run.suggestions_added.max(0) as u64)
                    .sum(),
            },
    ...
    async fn materialize_create_skill(&self, action: &serde_json::Value) -> Result<(), AppError> {
        ...
        if let Some(kb_content) = action.get("knowledge_base").and_then(|v| v.as_str()) {
            if !kb_content.trim().is_empty() {
                if let Ok(Some(skill)) = self.store.get_skill(companion_skill_id).await {
                    let scope = if companion_id.as_str().is_empty() {
                        nomifun_extension::skill_service::SkillScope::Shared
                    } else {
                        nomifun_extension::skill_service::SkillScope::Companion(
                    ...
                    if let Ok(skill_dir) = nomifun_extension::skill_service::skill_dir_for(
                        ...
                        if let Err(e) = tokio::fs::write(&kb_path, kb_content).await {
                            tracing::warn!(
                                ...
                                "failed to write knowledge_base reference"
                            );
                        } else {
                            tracing::info!(
                                skill = %skill.skill_name,
                                path = %kb_path.display(),
                                "knowledge_base reference written"
                            );
                        }
                    }
                }
    ...
    async fn materialize_proposed_memory(&self, action: &serde_json::Value) -> Result<(), AppError> {
        ...
        // 共享记忆已删除：一条被采纳的提议必须落在提出它的那个伙伴名下。
        let Some(owner) = action
            .get("companion_id")
            .and_then(|v| v.as_str())
            .and_then(|id| CompanionId::try_from(id).ok())
        else {
        ...
        if self
            .store
            .find_similar_active(kind, content, &owner)
            .await?
            .is_some()
        {
            return Ok(());
        ...
        let memory = self
            .store
            .insert_memory_scoped(kind, content, &[], 0.8, "summon", Some(&owner))
            .await?;
    ...
    pub async fn archive_companion_skill(
        &self,
        companion_id: &str,
        name: &str,
    ) -> Result<CompanionSkill, AppError> {
        let skill = self
            .store
            .find_owned_skill_by_name(companion_id, name)
            .await?
        ...
        self.store
            .set_skill_status(&skill.companion_skill_id, "archived")
            .await?;
        ...
        Ok(self
            .store
            .get_owned_skill(companion_id, &skill.companion_skill_id)
            .await?
            .unwrap_or(skill))
    }
...
mod tests {
    ...
    async fn consent_applies_once_and_never_reenables_after_disable() {
        ...
        // first-launch consent → default-on collect applied + flag set
