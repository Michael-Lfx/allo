...
| 层 | 改动 |
| --- | --- |
| 迁移 | `crates/backend/nomifun-db/migrations/058_app_server_context_usage_last_turn.sql`（**新**）：`ALTER TABLE app_server_context_usage ADD COLUMN last_turn_input_tokens INTEGER CHECK (last_turn_input_tokens >= 0)` + 同形 `last_turn_output_tokens`；**可空、无默认值**；头注释写死三条边界（只承载最近一轮 / 金额不入库 / 不是逐轮审计）与「未上报 = SQL NULL ≠ 0」。**未改 001–057**。 |
