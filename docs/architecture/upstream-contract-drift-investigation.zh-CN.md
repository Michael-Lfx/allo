...
09-02 已出现同样的 `you schema_mismatch`（`2026-09-02.nomicore.log:6304-6307`），
说明该问题至少存在两周。
...
- 用户记忆中"qwen3.8-flash + web_search 报错"来自备份环境的 09-04 测试，其直接原因是
  搜索 provider 链整体超时（含 DDG）；与 09-15 本仓库环境看到的 `you schema_mismatch`
  是同一系统的两个层面：**发现契约不可恢复 + 兜底预算**；
