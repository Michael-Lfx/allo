...
use crate::error::{MediaBackendError, MediaBackendResult};
...
pub fn write_image_bytes_as_png(bytes: &[u8], out_path: &Path) -> MediaBackendResult<()> {
    if let Some(parent) = out_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| MediaBackendError::Media(e.to_string()))?;
    }
    if image_magic_kind(bytes).is_none() {
        let head = String::from_utf8_lossy(&bytes[..bytes.len().min(80)]);
        return Err(MediaBackendError::Media(format!(
            "downloaded image is not PNG/JPEG/WEBP (head={head:?})"
        )));
    }
    let img = image::load_from_memory(bytes).map_err(|e| {
        MediaBackendError::Media(format!("decode image for {}: {e}", out_path.display()))
    })?;
    img.save_with_format(out_path, image::ImageFormat::Png)
        .map_err(|e| MediaBackendError::Media(format!("save png {}: {e}", out_path.display())))?;
    Ok(())
}
...
pub fn write_image_bytes_atomic(bytes: &[u8], out_path: &Path) -> MediaBackendResult<()> {
    if let Some(parent) = out_path.parent() {
        std::fs::create_dir_all(parent).map_err(|e| MediaBackendError::Media(e.to_string()))?;
    }
...
pub fn copy_image_file_atomic(src: &Path, out_path: &Path) -> MediaBackendResult<()> {
    if !src.is_file() {
        return Err(MediaBackendError::Media(format!(
            "copy source missing: {}",
            src.display()
    ...
    std::fs::copy(src, &part).map_err(|e| {
        MediaBackendError::Media(format!(
            "copy {} → {}: {e}",
            src.display(),
            part.display()
        ))
...
fn replace_file_atomic(part: &Path, out_path: &Path) -> MediaBackendResult<()> {
    if !part.is_file() {
        return Err(MediaBackendError::Media(format!(
    ...
    match std::fs::rename(part, out_path) {
        ...
        Err(rename_err) => match std::fs::copy(part, out_path) {
            ...
            Err(copy_err) => {
                let _ = std::fs::remove_file(part);
                Err(MediaBackendError::Media(format!(
...
pub fn scrub_unusable_image(path: &Path) -> MediaBackendResult<()> {
...
pub fn compose_reference_strip(paths: &[&Path], out_path: &Path) -> MediaBackendResult<()> {
    if paths.is_empty() {
        return Err(MediaBackendError::Media("compose_reference_strip: no images".into()));
    }
    if paths.len() == 1 {
        // Normalize JPEG-as-.png (and similar) into a real PNG for downstream APIs.
        let bytes = std::fs::read(paths[0]).map_err(|e| MediaBackendError::Media(e.to_string()))?;
        if image_magic_kind(&bytes) == Some("png") {
            std::fs::write(out_path, &bytes).map_err(|e| MediaBackendError::Media(e.to_string()))?;
        } else {
    ...
    for p in paths.iter().take(MAX_PANELS) {
        let bytes = std::fs::read(p)
            .map_err(|e| MediaBackendError::Media(format!("read ref {}: {e}", p.display())))?;
    ...
    if panels.is_empty() {
        return Err(MediaBackendError::Media("compose_reference_strip: all panels empty".into()));
    }
    ...
    DynamicImage::ImageRgba8(canvas)
        .save(out_path)
        .map_err(|e| MediaBackendError::Media(format!("save strip {}: {e}", out_path.display())))?;
    Ok(())
}

fn ffmpeg_missing_error(detail: Option<String>) -> MediaBackendError {
    match detail {
        Some(d) => MediaBackendError::Media(format!(
            "ffmpeg not found and auto-install failed: {d}. \
             Install ffmpeg on PATH or retry when network mirrors are reachable."
        )),
        None => MediaBackendError::Media(
...
async fn ensure_ffmpeg_ready() -> MediaBackendResult<PathBuf> {
...
pub async fn concat_videos(clip_paths: &[&Path], out_path: &Path) -> MediaBackendResult<()> {
    if clip_paths.is_empty() {
        return Err(MediaBackendError::Media("no clips to concatenate".into()));
    }
    ...
    let mut normalized: Vec<PathBuf> = Vec::with_capacity(clip_paths.len());
    {
        ...
        for (i, clip) in clip_paths.iter().enumerate() {
            ...
            set.spawn(async move {
                let _permit = permit.acquire_owned().await.map_err(|_| {
                    MediaBackendError::Media("normalize semaphore closed".into())
                })?;
        ...
        while let Some(joined) = set.join_next().await {
            let i = joined
                .map_err(|e| MediaBackendError::Media(format!("normalize join: {e}")))??;
...
/// the caller's job.
async fn try_remux_concat(ffmpeg: &Path, clips: &[PathBuf], out_path: &Path) -> MediaBackendResult<bool> {
...
async fn run_ffmpeg(ffmpeg: &Path, args: &[&str]) -> MediaBackendResult<std::process::ExitStatus> {
    ffmpeg_command(ffmpeg)
        ...
        .status()
        .await
        .map_err(|e| MediaBackendError::Media(format!("ffmpeg spawn: {e}")))
}

async fn run_ffmpeg_owned(
    ffmpeg: &Path,
    args: &[String],
) -> MediaBackendResult<std::process::ExitStatus> {
    ffmpeg_command(ffmpeg)
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
...
async fn run_ffmpeg_owned_capture(
    ffmpeg: &Path,
    args: &[String],
) -> MediaBackendResult<(bool, String)> {
    let output = ffmpeg_command(ffmpeg)
        .args(args)
        .stdin(Stdio::null())
        .stdout(Stdio::null())
...
pub async fn extract_last_frame(video_path: &Path, out_path: &Path) -> MediaBackendResult<()> {
    let ffmpeg = ensure_ffmpeg_ready().await?;
...
pub async fn extract_new_camera_frame(video_path: &Path, out_path: &Path) -> MediaBackendResult<()> {
    let ffmpeg = ensure_ffmpeg_ready().await?;
    if !video_path.is_file() {
        return Err(MediaBackendError::Media(format!(
            "video missing: {}",
...
pub async fn write_video_bytes_atomic(out_path: &Path, bytes: &[u8]) -> MediaBackendResult<()> {
    if let Some(parent) = out_path.parent() {
        tokio::fs::create_dir_all(parent).await?;
    }
    if (bytes.len() as u64) < MIN_USABLE_VIDEO_BYTES {
        return Err(MediaBackendError::Video(format!(
    ...
    tokio::fs::rename(&part, out_path).await.map_err(|e| {
        MediaBackendError::Video(format!(
...
pub async fn scrub_unusable_video(path: &Path) -> MediaBackendResult<()> {
