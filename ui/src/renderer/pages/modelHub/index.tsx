...
/**
 * Retired section keys kept resolvable so old bookmarks and links land
 * somewhere sensible. `speech` was a single host for the whole voice story; it
 * now has one section per direction, so an old link resolves to the first.
 */
 ...
 * is the source of every model, so 接入 leads; then one section per model
...
const SECTION_GROUPS: SectionGroup[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      {
        key: 'creation',
        labelKey: 'settings.modelHub.sectionCreation',
        icon: <Platte theme='outline' size='16' strokeWidth={3} />,
      },
      {
        key: 'global',
 ...
 *
 * One capability sidebar entry = one model capability, so nothing hides behind
 * a page-level filter or a second row of tabs. 创作能力 and 全局模型设置 keep
 * their own cross-capability views under 进阶: the first is the capability
 * OVERVIEW (image ∪ video in one filterable list), the second owns IDMM
 * defaults, the failover queue, and decision activity — none of which is a
 * per-capability concern.
 *
 * The sidebar width is drag-resizable and persisted. On mobile the sidebar
 * collapses to a horizontal segmented bar above the content (flat — the groups
 * are a desktop affordance).
 *
 * The level syncs to `?section=`; the retired `speech` key still resolves so
 * old bookmarks work.
 */
