...
use crate::store::{CompanionLearnRun, CompanionStore, MOOD_KEY, MemoryFilter};
...
pub struct CompanionLearnResult {
    ...
    /// Suggestions this pass raised. Suggestion cards remain install-wide, but
    /// the pass that raises them belongs to one companion.
...
impl Learner {
    ...
    pub async fn run_for(&self, companion_id: &str) -> Result<CompanionLearnResult, AppError> {
        ...
        // A companion that never picked a dedicated learn model still distills,
        // using the model it already chats with.
        let Some(model) = profile.learn.model.clone().or_else(|| profile.model.clone()) else {
        ...
        let Some(output) = parsed else {
            ...
            self.record_run(started_at, &run).await;
        ...
        let prior_active = self.store.count_memories("active", Some(&owner)).await.unwrap_or(0);
        ...
        if prior_active == 0 && run.memories_added > 0 {
            ...
            self.emitter.emit_suggestion_created(&owner, &milestone);
        }
        ...
        for s in output.suggestions.iter().take(MAX_SUGGESTIONS_PER_RUN) {
            ...
            // service layer can write a references/ page on accept.
            let action = if let Some(kb) = s.knowledge_base.as_ref().filter(|v| !v.trim().is_empty()) {
                let mut action = s.action.clone().unwrap_or_else(|| serde_json::json!({}));
                if let Some(obj) = action.as_object_mut() {
                    obj.insert(
                        "knowledge_base".to_string(),
                        serde_json::Value::String(kb.clone()),
                    );
                }
                Some(action)
            } else {
    ...
    /// Persist the run so the 学习 timeline, the diary list and the local
    /// analytics roll-up have history to read. Best-effort: a failed bookkeeping
    /// write must never fail a pass whose memories are already committed.
    async fn record_run(&self, started_at: i64, run: &CompanionLearnResult) {
        let record = CompanionLearnRun {
            learn_run_id: nomifun_common::CompanionLearnRunId::new().into_string(),
            started_at,
            finished_at: Some(now_ms()),
            status: run.status.clone(),
            events_processed: run.events_processed,
            memories_added: run.memories_added,
            suggestions_added: run.suggestions_added,
            error: run.error.clone(),
            summary: run.summary.clone(),
        };
        if let Err(error) = self.store.insert_learn_run(&record).await {
            tracing::warn!(%error, "companion learn run history write failed");
        }
    }
}
...
mod tests {
    ...
    async fn run_once_throttles_then_fires_on_accumulated_evidence() {
        ...
        let run2 = learner.run_for(&id).await.unwrap();
    ...
    #[tokio::test]
    async fn learn_model_falls_back_to_the_companions_own_chat_model() {
        let dir = tempfile::tempdir().unwrap();
        seed_event(dir.path());
        ...
        assert!(companion.learn.model.is_none());
        ...
        let run = learner.run_for(&companion.companion_id).await.unwrap();
        assert_eq!(run.status, "ok");
    }
