...
class CanvasAudioPlaybackController {
    ...
    private async begin(source: CanvasAudioSource, autoplay = true, seekTimeMs?: number) {
        ...
        if (!url) {
            this.update(source.nodeId, { phase: "error", error: canvasT("videoCanvas.audio.unavailable", "音频资源不可用") });
            return;
        }
    ...
    private ensureAudio() {
        ...
        if (typeof Audio === "undefined") throw new Error(canvasT("videoCanvas.audio.unsupported", "当前环境不支持音频播放"));
        ...
        audio.addEventListener("error", () => {
            const active = this.activeSource;
            if (active) this.update(active.nodeId, { phase: "error", error: canvasT("videoCanvas.audio.loadFailed", "音频加载失败") });
        });
        this.audio = audio;
        return audio;
    }
...
function mediaErrorMessage(error: unknown) {
    ...
    return canvasT("videoCanvas.audio.playFailed", "音频播放失败");
}
