...
> **2026-09-11 C 档复核（卡点三分法）**：C 档原以「维持不动」统一处置 5 条，复核后判定**混杂了三种不同性质**，改按性质分别处置——① **已获批、未排期**（R23 / R24：`21` D6=A 已批准按规范实现阻断，卡的不是决策而是排期）→ 移出「等外部条件」，标「已批准待排期」；**后续状态（2026-09-11）**：两条**均已落地**（R23 批 8 + 批 10 / R24 批 9，现均属 ✅ 组，见 §5.2 与 §5.3 落地记录）；② **技术依赖**（R20 / R15：一条挂死两条，依赖粒度太粗）→ 拆条，把不依赖的部分先做；③ **触发条件**（R6① / R33：R33 的唯一承诺动作「开立项页」当时并未兑现）→ 补度量并立刻做零风险项。逐条依据见 §5.3「卡点处置四档」的 C 档重写与「卡点决策」表。
...
> - **D6 落地口径随之翻转**：由「逃生口默认关（保兼容）」重新判为「**严格依赖检查默认关（`[import].strict_dependencies` 默认 `false`），保留现状行为**」**（原写「阻断默认开」已被 2026-09-11 用户决定订正，见 §5.2 R23 / R24 行与 §5.3 落地记录）。
...
| # | 事实 | 证据 | 影响 |
| --- | --- | --- | --- |
| F1 | 下载目录**只有 2 个产物**，均为 Windows x86_64 | `curl http://111.170.173.22:10014/downloads/` → `flowy-agent-store-latest-windows-x86_64.zip`、`flowy-agent-store-v1.0.11-windows-x86_64.zip` | 非 Windows 访客无产物。✅ **已退役（2026-09-15）**：旧 VPS 下载目录不再作为分发入口（两个旧包由运维清理下网——它们停在 2026-09-04、命名体系还是 `v1.0.11` / `latest`，与现行版本已脱节），二进制一律走 GitHub Releases（`content/release.json` 的 `repo`，当前唯一 Release 为 `v0.1.0-beta.3`）。站点域名**不托管下载产物**：`/downloads/*` 不存在，也不做重定向，对外措辞统一指向 Releases（见站点仓 `README.md` §内容来源、`content/docs/{zh-CN,en-US}/compatibility.md`） |
...
| F5 | 站点部署口径漂移 | `README.md` 称 GitHub Pages；`deploy-site.yml` 的 `push:` 触发**已注释**，仅剩 `workflow_dispatch` | 文档与真实部署不一致；线上是 VPS 裸 IP + HTTP。✅ **订正（2026-09-15）**：线上已是官网站点 `https://agent-store.flowyaipc.cn`（腾讯云 EdgeOne Makers，Git 直连本仓库构建），裸 IP + HTTP 那条路径整体退役——市场镜像 `:10072` 由官网站点 `/source/…` 取代（见 `18` §11 D1），下载目录 `:10014/downloads/` 停用（见本表 F1） |
...
| F20 | **产物无任何展示** | `web/src` 搜 `output_files` / `artifact` 无命中；`TurnResult.output_files` 已聚合、`artifact.created` 已发 | 跑完看不到产出文件，「使用」环节缺一环 —— ✅ **已修（2026-09-10，T12）**：按 D-W5-1 收敛为**宿主面文件服务**（`/api/fs/list`、`/api/fs/read`、`/api/fs/metadata`），`ArtifactPanel` + Topbar 入口交付「列表 / 预览 / 下载 / 评论入草稿」；**残留（已全部消解，2026-09-11）**：列表项 size / type / mtime 已补（批 3 R20）· 「按 Run 归属」已交付（R20a，客户端纯投影）· 「接受 / 回退」已交付（R20b，宿主 git 基线快照服务 `/api/fs/snapshot/*`）；Artifact 协议仍延后（见 D-W5-1，该偏差已消解） |
...
| T12 | W5 产物面板（⚠️ 按 D-W5-1 收敛为宿主面文件服务：会话 workspace 的列表 / 预览 / 下载 / 评论入下一条消息；**无**「按 Run 归属」与「接受 / 回退」；**后续 2026-09-11**：两项均已在同一宿主面方案内补齐——R20a 客户端纯投影、R20b 宿主快照服务，仍未引入 Artifact 协议） | 2026-09-10 |
...
| R14 | B | W9 模型能力与用量 | 2026-09-11（批 6 + R14 轮） | 费率 / 窗口 / 视觉投影 + 发送前校验 + 接近上限建议 + 按 turn 的 token 与估算金额（费率与 token 都在才显示）；**逐轮 usage 已持久化**（迁移 `058`），重载后仍显示上一轮 token 与金额 |
| R15 | B | W10 附件 / 图片输入 | 2026-09-11 | 协议 additive `attachments`（**路径引用**）+ 宿主会话工作区准入 + 客户端前置校验 + 附件选择面板 + **拖拽 / 粘贴本地文件**（字节经 `/api/fs/upload` 的 `workspace` 落点写进**会话工作区**再拿回路径引用；见 §5.3「R15 落地记录」） |
| R17 | B | W12 技能 / 专家管理 | 2026-09-11 批 4 | 后端 `skill/create\|update\|delete` + `skill/copy`（目录级复制原语 `copy_skill_directory`：新名 + 链接拒绝 + 目标占用判定）+ `origin`/`writable` 归属与卸载语义 + WebUI 管理面（列表按来源区分 / 新建 / 编辑 / 复制 / 删除，只读来源给原因，双语文案 `skills.*`）；原三项缺口全部闭合：编辑面改 `skill/update` 服务端字段级合并（不需 `skill/source`）、`skill/copy` 目录级复制原语落地、UI 已接（代码核实：`web/src/components/skills/SkillWriteSurface.tsx`、`web/src/store/skillAdmin.ts`、`crates/backend/nomifun-extension/src/skill_service.rs`；见 §5.3「R17 落地记录」） |
...
| R20 | B | W5 余项 | 2026-09-11 | 三段全部落地：list 项 size / type / mtime（`/api/fs/metadata`）· **R20a 按 Run 归属**（客户端纯投影 `web/src/lib/artifact-owners.ts`）· **R20b 接受 / 回退**（宿主 git 基线快照服务 `/api/fs/snapshot/*`：`compare` 出待处理 / 已接受、`stage` 接受、`discard` 回退、`unstage` 撤销接受；面板「变更」页）——**未引入 Artifact 协议**（见 §5.3「R20a 落地记录」「R20b 落地记录」） |
| R21 | B | W13 余项 | 2026-09-10 批 1 | 移除前受影响快照改由服务端前置投影（`market/get` 的 `installed_count`） |
| R22 | C | `17` P1 凭据明文（安全） | 2026-09-11 | 值不入快照 / 不落库：导入期敏感 `env` 改写为 `secret:<KEY>`，MCP 装配期（连接测试 + nomi / acp）按引用注入；`[credentials]` 手填、不入 `config/get` / `config/set`（见 §5.3「R22 落地记录」） |
| R23 | C | `17` P3 依赖阻断 | 2026-09-11 批 8 + 批 10 | extension 层（加载期）+ 导入层（导入期 SemVer 解析与阻断）均落地，逃生口 `[import].strict_dependencies` **默认关**（用户拍板）；原「阻断默认开」已订正（见 §5.3「R23 落地记录」与「R23 导入层半边 · 落地记录」） |
| R24 | C | `17` P4 市场条目 `strict` 阻断 | 2026-09-11 批 9 | 条目级 `strict=true` 阻断（条目可见 + 带原因 + 导入 `blocked` + 不落库），三处 additive 透传；`strict=false` 的「条目代替清单」拆为 `17` §10 **P5**（需单独立项，不属本项验收范围）（见 §5.3「R24 落地记录」） |
...
##### 🟡 部分完成（3 项：主体已落地，缺口登记在案）

| # | 类别 | 任务 | ✅ 已完成 | 🟡 未完成（缺口本身也算未做） | 解锁条件 / 下一步 |
| --- | --- | --- | --- | --- | --- |
| R6 | A | C4 站点机制 | 中英同步校验脚本（`check-docs-sync`）· changelog 页（批 4） | ① **站内搜索**——❌ **不做（2026-09-11 用户决定）**；② **市场数据刷新纳入发布流程——⏸ 已延后（2026-09-11 用户决定：站点部署卡点相关问题延后）** | ① **订正（2026-09-11 实测）**：原「会引入构建期索引这一新守卫面」**不成立**——`site/app/lib/docs.ts:10-14` 已用 `import.meta.glob(..., { query: "?raw", eager: true })` 把 docs **全文内联进 bundle**，客户端本来就拿得到全文，故搜索可以是**纯客户端、零构建步骤、零新守卫面**；剩下的只是「做不做」的产品取舍（范围建议仅 docs，市场侧已有自己的客户端过滤 `site/app/pages/Market.tsx:66-71`）；② **不再挂计划**；重开条件＝用户提出站点部署 / 发布流程需求 |
| R16 | B | W11 设置 Dialog | provider 分区（`config/get` / `config/set`）+ nav 八→二（批 4）+ **`agent` 分区（记忆蒸馏开关，2026-09-11，见 §5.3「本轮落地记录」A 档 ②）** + **`mcp` 分区（`mcp.json` 只读投影，2026-09-13，见 §5.3）** | **其余五分区不渲染**（account / plugin / advanced / lab / archived）——判定为「没有可自证的真实面」，非缺数据 | 逐分区解锁条件见「卡点决策」R16 行；`agent` 已解锁（宿主**已消费** `[memory].distill_enabled`：`apps/agent-store/src/main.rs:318`） |
| R33 | E | 方向四 + WP-5 协议词汇与概念对齐 | ✅ 立项页已建（`22-webui-productionization.zh.md`，2026-09-11）；**A 组（安全类）已动工（2026-09-11，用户指令）**——起点 A2「正式认证与令牌管理」已落地（范围 / 验收 / 决策见 `22` §7.1） | **A1 / A3**（依赖 A2）与 **B / C / D 组**仍未排期 | 按 `22` §7 顺序推进：A2 ✅ → A1 → A3；B/C/D 组动工前先在 `22` 登记 |

##### ⏸ 未完成（2 项：一行未动，含有意不做 / 等外部条件；R23 / R24 / R22 / R20 / R15 已转 ✅ 组、R33 已转 🟡 组）
...
> **计数**：✅ 29 项 · 🟡 3 项 · ⏸ 2 项 = **34 项**（R1–R34，无遗漏、无重复；**R20 于 2026-09-11 拆为 R20a / R20b 两个子项，不新增顶层编号，故总数不变**）。**待拍板项：无**——Q2 已于 2026-09-11 按用户决定「站点部署卡点相关问题延后」（连带的 R7 / R32 / R6② 一并延后、不再挂计划）；其余决策（Q1 / Q3–Q7、D1–D12）均已定，见 `21-open-decisions.zh.md`。
>
> **2026-09-11 C 档复核后的归属变化（不改变上表计数，只改「为什么没做」的定性）**：
> - **R23**：**✅ 已落地（2026-09-11，批 8）**——extension 层阻断 + `[import].strict_dependencies`（**默认关**，用户拍板）已实现并验证，转 ✅ 组。**R24**：**✅ 已落地（2026-09-11，批 9）**——市场条目 `strict=true` 的阻断打通（可见 + 带原因 + 导入 `blocked` + 不落库），转 ✅ 组；其 `strict=false`（条目代替清单）半边**拆出**为 `17` §10 **P5**（未实现需单独立项，不属本项验收范围）；**R23 的导入层半边亦已落地（2026-09-11，批 10）**——SemVer 范围解析 + 与目录投影比对 + 可开启阻断，读作 `17` §10 P3 收口（见 §5.3「R23 导入层半边 · 落地记录」）。
> - **R20**：**✅ 已落地（2026-09-11）**——拆条后三段全部完成：list 项 size / type / mtime（批 3）· **R20a 归属**（客户端纯投影，`web/src/lib/artifact-owners.ts`）· **R20b 接受 / 回退**（宿主快照服务 `/api/fs/snapshot/*`，`stage` / `discard`；**未引入** Artifact 协议），转 ✅ 组。原「等 Artifact Phase」判断被订正——宿主已有的 git 基线快照原语就是「接受 / 回退」的落点。见 §5.3「R20a 落地记录」「R20b 落地记录」。
> - **R15**：**✅ 已落地（2026-09-11）**——载体＝**路径引用**；用户拍板开字节通道后补齐最后一块「拖拽 / 粘贴本地文件」：宿主 `/api/fs/upload` 增加可选 `workspace` 落点（字节写进**会话工作区**、复用同一套 allowed-roots 准入，再作为路径引用附件），转 ✅ 组。原「落宿主 tmp + 第二条路径权威」的顾虑被落点设计消解。见 §5.3「R15 落地记录」。
> - **R33**：立项页已建（`22-webui-productionization.zh.md`），C 档唯一承诺动作已兑现；**2026-09-11 用户指令「22 A 组」→ A 组（安全类）已动工**，起点 A2 已落地（连接令牌 TTL + 过期/无效码区分 + 登出即吊销，见 `22` §7.1），故 R33 由 ⏸ 转 🟡；A1 / A3 与 B / C / D 组仍未排期。
> - **R6①**：阻塞理由订正——docs 全文已内联进 bundle（`site/app/lib/docs.ts:10-14`），不存在「构建期索引守卫面」；是否做改为纯产品取舍。
> - **R22**：**✅ 已落地（2026-09-11）**——`17` P1 凭据明文收口：值不入快照 / 不落库（导入期改写为 `secret:<KEY>` 引用，MCP 装配期按引用注入），转 ✅ 组。原「缺引用语法 + 解析注入点 + 填写入口」三项**同批落地**，不再是安全阻塞。见 §5.3「R22 落地记录」。
...
| R20 | B | W5 余项 | 🟡 **list 项 size / type / mtime 已补（2026-09-11）**。**2026-09-11 拆条**：**R20a「按 Run 归属」＝✅ 已完成（2026-09-11）**——归属数据**已存在**（`run/plan` 投影 attempt 的 `output_files`，`nomifun-agent-execution/src/runtime_adapter.rs:141`），**零协议增量**；落地**没有**走宿主面文件服务，改为**客户端纯投影**（理由与实现见下方「R20a 落地记录（2026-09-11）」）；**R20b「接受 / 回退」＝✅ 已完成（2026-09-11）**——**未**引入 Artifact 协议，改用**宿主 git 基线快照服务**（`nomifun-file` 的 `SnapshotService`，经 `/api/fs/snapshot/{compare,stage,stage-all,unstage,discard}` 暴露）：面板新增「变更」页，`compare` 分「待处理 / 已接受」两组，每行接受 / 回退 / 撤销接受 + 全部接受；`capabilities.artifacts` 仍硬编码 `false`（`nomifun-app-server/src/lib.rs:401`），**不需要**它。**实现**：`/api/fs/list` 只回名字与路径，元数据在宿主 `POST /api/fs/metadata`——store 新增 `artifactsMeta`（按绝对路径索引，`null`＝查不到，**缺键**才是未查询，故失败也记账以杜绝重复请求）+ `loadArtifactMetadata()`（并发上限 4、单条失败降级为 `null`、**不弹错不重试**、切会话丢弃过期结果），`refreshArtifacts` 拿到列表后自动补齐；`ArtifactPanel` 列表行与预览头渲染真实 `size · MIME · mtime`，**查不到就整段不渲染**；预览头在无元数据时才退回按文本长度估算（并在注释里写明那只是字符数、不是文件字节数）。验证：`appStore.artifact-meta.test.ts` **5 passed**（每路径只查一次 / 失败记 null 且不重发 / 显式路径只查这些 / 无 client 与无路径 no-op / 无工作区时不查）。 | ✅ 三段全部完成（size/type/mtime + R20a 归属 + R20b 接受/回退）；见 §5.3「R20a 落地记录」「R20b 落地记录」 |
...
| R23 | C | `17` P3 | 依赖 SemVer 范围解析与「不可满足即阻断」 | ✅ **已完成（2026-09-11，批 8 + 批 10，两层）**：extension 层（加载期）+ 导入层（导入期 SemVer 解析与阻断）均落地，逃生口**默认关**（用户拍板）。**二次订正（2026-09-11）**：**（a）决策不是卡点**——`21` D6=A 已批准按规范实现阻断 + 配置逃生口；**（b）解析 + 拓扑排序已接线、不是「调用点全在单测」**：`registry_helpers.rs:46-61`（`load_and_validate` 在 `:56` 调 `validate_dependencies`、`:57` 用 `load_order` 真排序）← `registry.rs:111` / `:166`；`dependency.rs:112-129`。**（c）层级要分开**——「全仓无 semver 解析」指的是**导入层** `nomifun-importer/src/import.rs:664-681`（只登记不决策），与 extension 层是两个消费面 | ✅ 已落地（见 §5.3「R23 落地记录」）。**逃生口默认关**（`[import].strict_dependencies` 默认 `false`，完全保留现状行为）——原写「阻断默认开」已按用户 2026-09-11 决定订正。**导入层半边已于批 10 补齐**（见 §5.3「R23 导入层半边 · 落地记录」） |
| R24 | C | `17` P4 | 市场条目 `strict` 阻断（`strict=true` 且来源缺 `plugin.json`） | ✅ **已完成（2026-09-11，批 9）**。**归属订正（保留）**：这是**条目发现 / 条目导入**层语义，`strict` 的宿主是 `.codebuddy-plugin/marketplace.json` 的 `plugins[]` 行（`02` §8），原「同 R23」把两层混为一谈（`17` §3 表位置亦已订正）。**实现**：`strict_entry_block`（纯判定）＋ 探测**保留**被阻断条目（不再静默跳过）＋ 导入前闸门（`blocked`、**不落库**）；`MarketplaceEntry` / `AppServerMarketplaceEntry` / `AppServerStoreItem` 三处 additive 透传；Web 端标「不可安装」并撤掉安装动作 | ✅ 已落地（见 §5.3「R24 落地记录」）。**余项**：`strict=false` 的「条目代替清单」拆为 `17` §10 **P5**（未实现，需单独立项，不属本项验收范围） |
...
| R23 | C | `17` P3 依赖阻断（**两层均已落地**） | 2026-09-11 批 8 + 批 10 | **extension 层（批 8）**：`registry_helpers::blocked_dependents`（只取 `Missing` / `VersionMismatch` 的**依赖方**，环仍按 API 规范尽力加载）＋ `ExtensionRegistry::with_strict_dependencies`，初始化与热重载同口径。**导入层（批 10）**：新增 `nomifun-importer/src/dependency.rs` 的 `check_dependencies`（SemVer 范围解析 + 与目录投影比对）＋ `ImporterService::with_strict_dependencies`，闸门在幂等判定后、持久化前，命中即 `blocked` 且**不落库**。**两侧同一个逃生口**＝`[import].strict_dependencies`，**默认关**（用户 2026-09-11 拍板） |
| R24 | C | `17` P4 市场条目 `strict` 阻断（**条目发现 / 导入层**） | 2026-09-11 批 9 | 新增 `strict_entry_block`（纯判定）＋ 探测**保留**被阻断条目（不再静默跳过）＋ 导入前闸门（返回 `blocked`、**不落库**）；`strict`/`blocked_reason` 三处 additive 透传（DB 行 JSON → `market/get` → `store/list`）；Web 端标「不可安装」且不给安装动作。**余项**：`strict=false` 的「条目代替清单」拆为 `17` §10 **P5**（未实现，需单独立项） |
...
| D-W5-1 | **W5 的「artifact 协议已就绪」前提不成立** | `19` §3 / §7 原称 `artifact/list` / `artifact/get` 已定义、`artifact.created` 已发；实测 `05`:152 与 `TC-AS-008` 明确 Artifact 能力**延后实现**（`capabilities.artifacts` 恒 `false`，且规定不允许任意路径读取），TS 协议包与 `packages/client` 均无对应类型与子客户端，`artifact.created` 事件不存在 | W5 无法按「按 Run 的产物 + 接受/回退」交付；在「第 3 批不碰协议」的边界下，AC-5 的「按 Run 归属」与「回退」两项不可达 | ✅ **已定（2026-09-10）：收敛为宿主面文件服务方案**——按会话 workspace 交付列表 / 预览 / 下载（`/api/fs/list` + `/api/fs/read`，服务端已有），「按 Run 归属」与「接受 / 回退」留待 Artifact Phase；`19` §3 / §7 已按实测修正。**后续（2026-09-11）——本偏差正式消解**：「按 Run 归属」由 R20a 以**客户端纯投影**交付（`web/src/lib/artifact-owners.ts`，数据来自 `run/plan` 的 `attempt.output_files`）；「接受 / 回退」由 R20b 以**宿主 git 基线快照服务**交付（`/api/fs/snapshot/*`：`compare` / `stage` / `discard` / `unstage`）——两项**均未引入** Artifact 协议（`capabilities.artifacts` 仍恒 `false`），`19` 已同步 |
...
| # | 项 | 决定 | 理由与解锁条件 |
| --- | --- | --- | --- |
| R22 | `17` P1 MCP 连接器 `env` 明文写入快照 | **✅ 已落地（2026-09-11，D 档同批）：值不入快照 / 不落库，导入期敏感 `env` 改写为 `secret:<KEY>` 引用，MCP 装配期（连接测试 + nomi / acp）按引用注入；`[credentials]` 手填、不入 `config/get` / `config/set**（见 §5.3「R22 落地记录」） | 这是**安全**项：快照里落明文 env 的修法是「值不入快照、只留键名 + 注入引用」。**订正（2026-09-11 实测）**：原写「前提是宿主有凭据存储」过重——① 加密原语**已有**（`nomifun-common/src/crypto.rs` 的 `encrypt_string`/`decrypt_string` + workspace `aes-gcm`）；② 值侧存放位置**已拍板**（`21` **D5 = C：config.toml + env，不落库**）；③ 缺的是**引用语法 + MCP spawn 解析注入点 + 导入侧改写 + 填写入口**（实测 `nomifun-mcp` 无 env 展开 / `secret_ref`）。只改一半（值删了但无处填）＝导入后连接器**静默不可用**，故两步须同批，见 §5.3 D 档。 |
| R16 | W11 设置 Dialog 7 分区 | **provider 分区已落地（2026-09-11，批 4）；其余五分区不渲染（nav 八→二；`agent` 分区记忆蒸馏开关已随 A 档②落地；`mcp` 分区（`mcp.json` 只读投影）2026-09-13 加入，nav 三→四）** | 逐条判定（每条都是「没有可自证的真实面」，不是「暂时缺数据」）：**agent** —— **订正（2026-09-11 实测）：宿主已消费**。`apps/agent-store/src/main.rs:318` 启动时调用 `nomifun_ai_agent::manager::nomi::distill::set_distill_host_override(Some(enabled))`，读的就是 `[memory].distill_enabled`（D-STREAM-2 的接线已落地）→ 因此「写它就是假开关」**不再成立**；缺的只有两处：`AppServerConfigView` 未投影 `[memory]`、`config/set` 白名单只有 `default_model`。**处置：A 档执行**（补 1 个 additive 字段 + 1 个白名单键，见 §5.3 A 档）。可镜像的 models / efforts 事实仍由 composer 的 ModelPicker **单一持有**，设置里不再放第二份口径。**account** —— 本 App Server 没有账户面（唯一的登录态是连接 token；连接器 OAuth 行在市场目录页）。解锁：宿主账户 API 出现。**plugin** —— 真实面（市场源 / 安装态 / `auto_update`）**已在应用商店页（W13）**，设置里再放只读副本不新增能力；`[default_marketplaces]` / `[marketplace] cadence` 虽有真实配置，但做成写面＝再开一组白名单字段。解锁：插件分区要有**自己的**宿主级设置语义（先设计写面）。**advanced / lab / archived** —— 无宿主设置、无 feature flag、无归档 API（协议里没有归档/回收站，`19` R19 行同样按现状收敛）。**执行口径**：一律从 nav 移除，**不做禁用项、不留「即将推出」占位**（§6 原文允许「标即将推出」，但空壳正是本轮要消灭的形态：禁用项与占位同样点了没反应）。 |
| R17 | W12 技能 / 专家 CRUD | **✅ 已全部落地（2026-09-11，批 4 + A/B 档）：后端三动词写面 + `skill/copy` 目录级复制原语 + WebUI 管理面（列表 / 新建 / 编辑 / 复制 / 删除）** | 原「建议单独立项」已被执行：归属 / 覆盖 / 卸载语义按源码布局逐条定死（`user_skills_dir/{name}/` 是**唯一可写**位置；内置 / 市场产物 / 共享 / 伙伴 / 草稿一律只读），`create` 同名 `conflict`、卸载仍归 `install/uninstall`。**原「仍不做」三项已闭合（代码已核实）**：① 编辑面改为 `skill/update` **服务端字段级合并**（正文按「替换」语义，故不需放开 `skill/get` 全量回读、不需新增 `skill/source`）；② `skill/copy` 由新增的目录级原语 `copy_skill_directory`（新名 + 链接拒绝 + 目标占用判定）实现，满足「复制为新名 + 重写 frontmatter name」；③ WebUI 管理面已接（`web/src/components/skills/SkillWriteSurface.tsx` + `web/src/store/skillAdmin.ts`，含 `skills.*` 双语文案）。详见 §5.3「R17 落地记录」与任务总表 R17 行（现归 ✅ 组）。 |
| R23 / R24 | `17` P3 依赖 SemVer 范围解析与「不可满足即阻断」；P4 `strict` | **R23 ✅ 两层均已落地（2026-09-11 批 8 extension 层 + 批 10 导入层）；R24 ✅ 已落地（2026-09-11 批 9，条目发现 / 导入层）** | **原判定把排期误写成决策阻塞**：`21` 速览 D6 = A「按规范实现阻断 + 配置逃生口，作为 v1.1 的破坏性变更写进 changelog」——**已批准**，故「解锁条件＝v1.1 解冻」不成立（D6 已满足）。**归属与工作量二次订正**：① extension 层的**解析 + 拓扑排序已接线**（`registry_helpers.rs:46-61` ← `registry.rs:111` / `:166`），缺的只有**阻断**（`registry.rs:136-138` 只 `warn!`，`valid` 未当门）；②「全仓无 semver 解析」指**导入层**（`nomifun-importer/src/import.rs:664-681`），是**另一个消费面**；③ `strict` 属**导入期**语义，挂 importer 层、与 `PluginManifest` 同级，不是 extension 层字段。**处置**：按逃生口**默认关**形式落地（`[import] strict_dependencies` 默认 `false`，完全保留现状行为），语义 + 反向用例先落地；等「第一个外部发布者挑战」这一触发出现时，只翻默认值 + 发 changelog 公告——避免「已批准却没做」永久悬空。**R23 已于 2026-09-11 按此形式落地（两侧）**：批 8 extension 层（加载期阻断 + 默认关逃生口）、批 10 导入层（SemVer 解析 + 目录投影比对 + 可开启阻断），见下方「R23 落地记录」与「R23 导入层半边 · 落地记录」。**R24 亦已于同日落地**（批 9：条目级 `strict=true` 阻断——可见 + 带原因 + 导入 `blocked` + 不落库；见下方「R24 落地记录（2026-09-11，批 9）」）；其 `strict=false` 的「条目代替清单」半边**拆出**为 `17` §10 P5（未实现）。 |
...
| R6 | 站内搜索 / 市场数据刷新纳入发布流程（changelog 页本轮已完成；**② 已于 2026-09-11 按用户决定延后**） | **① 站内搜索：不做（2026-09-11 用户决定；理由为产品取舍——技术无障碍，见右列订正）**；② 市场数据刷新纳入发布流程：⏸ 延后（2026-09-11，用户决定：站点部署卡点相关问题延后） | **站内搜索理由订正**：原写「属站点新功能——要引入索引与构建期数据，会把『索引与文档内容一致性』这一道新的守卫面拉进来」**不成立**。实测 `site/app/lib/docs.ts:10-14` 已用 `import.meta.glob("../../content/docs/**/*.md", { query: "?raw", eager: true })` 把 **9 页 × 2 语言全文内联进 bundle**，即客户端**本来就持有全文**——因此搜索可以是**纯客户端对已内联内容做索引 / 子串扫描**，零构建期产物、零新增守卫面、零协议改动。结论：这**不是技术阻塞，是产品取舍**（建议范围仅 docs；市场侧已有自己的客户端过滤 `site/app/pages/Market.tsx:66-71`）。若仍决定不做，理由应写成「价值不足」，不是「有守卫面风险」。**市场数据刷新纳入发布流程**：⏸ **已按用户决定延后（2026-09-11）**——原卡点是「部署访问权 + D2 未定」，现整条「站点部署」轴延后（与 R7 / R32 同批），不再作为待推进项；本机无法自证端到端效果的前提不变，重开条件＝用户提出站点部署 / 发布流程需求。 |
| D-W9-1 | W9 按 turn 费用 | **取①（2026-09-11，批 6，按用户指令执行）：后端在会话流补逐轮 `usage`（additive）——已落地，见 R14 行 ⑥；持久化（重载后仍显示）已于同日 R14 轮落地，见 R14 行 ⑦** | 原②「不做费用、只显示费率与窗口」的前提是「补 usage ＝ 新增一条 token 上报链路，超出 D12=A『用 models.dev 定价』的原意」；源码级侦察推翻了该前提——**运行时早已上报、中继早已转发，只是 App Server 投影丢掉了这份数据**，所以补的是**一层投影**、不是新链路，D12=A 的取价口径不变。金额仍严守「费率与 token 都在才算、任一缺席整段不渲染」。 |

> 上述决定均为**记录**；`16` 的状态列如实标注：已落地的（R22 / R23 / R24 / R15 / R30 / R33 等）标 ✅，未落地的仍标 🟡 / ⏸ / 🔴（逐行见分组视图与逐项明细）。（**2026-09-11 补充（站点部署轴延后）**：按用户决定「站点部署卡点相关问题延后」，R7 / R32 / R6② 三项从「等拍板 / 挂起」改为**延后、不再挂计划**；Q2 与之保持延后。分组视图的状态列已同步标注，重开条件逐行写明。）（**2026-09-11 批 5 补充**：其中 R30 一条已在批 5 兑现，见下。）（**2026-09-11 批 4 补充**：R6 行的「changelog 页」已完成——见上表 R6 行与「R6 剩余项落地记录」；同行的站内搜索与发布流程两项**仍不做**，理由与解锁条件不变。**2026-09-11 C 档复核订正**：其中「站内搜索」的**理由已改判**——不存在「构建期索引守卫面」（docs 全文已内联进 bundle），改为**产品取舍**；「市场数据刷新纳入发布流程」的理由（部署访问权）不变，仍随站点部署轴延后。）（**2026-09-11 批 6 补充**：D-W9-1 的 ① 已落地——逐轮 `usage` 走 App Server 投影 additive 上 wire，见 R14 行 ⑥ 与上表 D-W9-1 行。）（**2026-09-11 R14 轮补充**：D-W9-1 的**持久化边界已闭合**——迁移 `058` 给 `app_server_context_usage` 加两列 + 读回经 `ContextUsageView` additive 上 wire + 前端重载回填（金额仍现算），见 R14 行 ⑦；**R14 由 🟡 转 ✅**（唯一遗留「逐轮历史回放」明确不属本项）。）
...
>
> **本轮未做（登记 + 解锁条件，不用半条链路凑）** —— ⚠️ **本条已过时（2026-09-11 复核，代码已核实）**：下方三项均已在批 4 + A/B 档实际落地，R17 现归任务总表 ✅ 组，保留仅作溯源。① ~~**WebUI 技能管理面**~~ ✅ **已完成**：编辑走 `skill/update` 服务端字段级合并（不需 `skill/get` 全量回读、未新增 `skill/source`）；② ~~**`skill/copy`（复制为新名字）**~~ ✅ **已完成**：新增目录级原语 `copy_skill_directory`（新名 + 链接拒绝 + 目标占用判定）；③ ~~前端写面 helper 故意不加~~ ✅ **已完成**：`web/src/store/skillAdmin.ts` 的 `create/update/remove/copy` 已接 `SkillWriteSurface.tsx`。读面 `origin`/`writable` 与后端三动词 + 归属语义均已可用且有测试（见 R17 行与上方「R17 落地记录」）。
...
| # | 性质 | 改判后的判定 | 依据（二次实测） |
| --- | --- | --- | --- |
| R23 / R24 | **已获批（R23 / R24 均已落地）** | **R23 ✅ 两层均已落地（2026-09-11）**：批 8 extension 层（加载期阻断）＋ 批 10 导入层（SemVer 解析 + 目录投影比对 + 导入期阻断），逃生口**默认关**（`[import].strict_dependencies` 默认 `false`，完全保留现状行为，**一个键管两侧**）；**R24 ✅ 亦已落地（2026-09-11，批 9）**：条目级 `strict=true` 阻断（条目可见 + 带原因 + 导入返回 `blocked` + 不落库）。`strict=false` 的「条目代替清单」拆为 `17` §10 P5，需单独立项 | `21` D6=A **已批准**按规范实现阻断 + 逃生口 → 原「解锁条件＝v1.1 解冻」不成立。**两处口径订正**：① 「调用点全在自身单测」**不成立**——`registry_helpers.rs:46-61`（`:56` 调 `validate_dependencies`、`:57` 用 `load_order` **真排序**）← `registry.rs:111` / `:166`，`dependency.rs:112-129` 有用例；**真缺的只有阻断**（`registry.rs:136-138` 只 `warn!`，`valid` 未当门）。② 「全仓无 semver 解析」指**导入层**（`nomifun-importer/src/import.rs:664-681` 只登记不决策），是**另一消费面**；`strict` 属导入期语义，挂 importer 层（`PluginManifest` 同级），不是 extension 层字段 |
...
| R15 | **技术依赖（与 词汇对齐 混同）** | **✅ 已落地（2026-09-11）**：载体＝**路径引用**（用户拍板）；协议 additive `attachments` + 宿主侧会话工作区准入 + 客户端前置校验 + 附件选择面板。**剩**：拖拽/粘贴本地文件（待「要不要开字节通道」的决策，见 R15 落地记录） | 桌面 API 已有 `files: Vec<String>`（`nomifun-api-types/src/conversation.rs:109-133`），app-server 写死 `files: Vec::new()`（`nomifun-app-server/src/lib.rs:3677`）→「引擎侧已通、协议侧空白」，`attachments` 可 additive。词汇对齐 的 `thread/turn/item` 重命名**只影响字段命名**，不影响载体语义 → 「现在做、词汇对齐 改一次名」与「等」可比较 |
...
| R6① | **缺触发度量（阻塞理由不成立）** | **✅ 不做（2026-09-11 用户决定）**；理由为产品取舍 | 原理由「会引入构建期索引守卫面」**不成立**：`site/app/lib/docs.ts:10-14` 已 `import.meta.glob(..., { eager: true, query: "?raw" })` 把 docs **全文内联进 bundle** → 搜索可纯客户端实现，**零构建期产物、零新守卫面**。建议范围仅 docs（市场侧已有客户端过滤，`site/app/pages/Market.tsx:66-71`）；若不做，理由是「9 页 × 2 语言价值不足」，不是「有风险」 |
...
- **不做假开关**：设置分区、市场开关等一律要有真实数据源与回写路径；未实现的：能移除的从入口移除（R16 执行口径，不留「即将推出」占位），确需保留入口的才标「即将推出」——但**绝不留「点了没反应」的空壳控件**（§6 红线）。
...
**R16 追加 · `mcp` 分区（只读投影）→ 已完成（2026-09-13）**

- **后端零改动**：`config/get` 的 `mcp { exists, servers[{name,transport,enabled}], rejected[{name,reason}], error? }` 早在 `20` §9.3 就上了 wire（协议指纹 `2026-09-13` 已覆盖它），本项只补 TS 类型与渲染。
...
- **只读是刻意的**：`config/set` 没有该文件的键（`05` §4.10）——声明可以启动本地命令（stdio），所以它保持「宿主操作者手写」的形态，设置页不提供编辑；视图里也没有任何写控件（渲染测试断言不含 `<input>` / `<select>` / 「保存」）。**⚠️ 订正（2026-09-19）**：本条只描述**当时**上线的只读投影；写面在 2026-09-17 随 `21` D17 落地，见本文 §8.3「MCP 声明文件从只读变可读可写」与 `05` §4.10——设置里的 `mcp` 分区现有 `role="switch"` 开关与「配置 MCP」正文编辑器，因此本句的「设置页不提供编辑」与「视图里没有任何写控件」**均不再成立**；仍然成立的是「没有『新增 server』表单」，声明只能写在那份文件里（2026-09-19 同步修掉 `settings.mcpFileDesc` 的文案，它此前把「无表单」误写成了「无编辑」）。
...
**R16 追加 · `mcp.adopted`（补上「宿主是否真的在用」）→ 已完成（2026-09-14）**

- 上一条只读分区交付时登记的产品边界**已收口**：`servers` / `rejected` / `error` 描述的都是**文件**，而 `mcp_declaration_view` 是**无条件读盘**的，于是「宿主根本不读这份文件」（`nomifun-web` 默认位）与「宿主把每条都注入了会话」渲染完全相同。读面新增 `adopted`（宿主事实），设置页第一张卡片多一行「本宿主是否使用它」；`mcpFileDesc` 里那句「不代表宿主是否采用它」随之删除——不再是能力边界，已能被回答。
- **三态**：`true` / `false` / 缺席（宿主未上报，比该字段更旧的构建）。缺席单独一句文案，绝不渲染成「未使用」：`apps/agent-store` 在该字段存在之前就已采用声明，折叠成 `false` 会把「不知道」写成否定答案。
- 协议指纹 `2026-09-13` → **`2026-09-14`**（10 处，见 §7 决策 4 的清单；仍无方法增删，`46 / 65` 计数守卫未动）。规格、三处判断与验证读数见 `20` §9.5。

**安装面五动词「做真事」· 本轮落地记录（2026-09-15，本会话）**

> 真实读数之外的文字均为**语义说明**；本批**未运行测试套件**，因此不在此处登记任何自跑读数（下表只记「新增/改了什么、定义在哪」）。本记录覆盖 `05` §4.5 / §4.5.2、`02` TC-INS-001/002/004 + 新增 TC-INS-008、`18` §9.1/§9.2、`07` §6.3、`21` 两条新决策、`09` §9 的同步。

| # | 改动 | 落点 |
| --- | --- | --- |
| ① | **`install/run` 可重入**：同 `snapshot_id` 再跑不产生第二个 Preset（复用组件 `runtime_ref`/`preset_id` 记下的 id；**不按显示名认领**；记录里的 Preset 被手工删掉则重建）；对曾被 disable 的组件重新安装会**同时**把运行时侧重新启用（否则记录会声称「已启用」而运行时仍是关的） | `app_server_installer.rs` |
| ② | **`uninstall` 真正释放产物**：skill 删物化目录 `{skills_root}/agent-store/<snapshot_id>/<slug>/`（最后一个 skill 时剪掉快照根）、agent/team 删 Preset、connector 按**记录 id**删 `mcp_servers` 行；顺序＝**先释放后清记录**，且只清真的释放成功的；已不在算**成功**（可重入）；失败则**保留 `installed=1`** 且不清记录（可重试、指针不丢）；`component_ids` 空列表 → `invalid_request`；路径按单段校验 + 拒符号链接 | `app_server_installer.rs`、`nomifun-importer/src/install.rs` |
| ③ | **`disable`/`enable` 真正移动运行时状态**：connector **置**（非 toggle）`mcp_servers.enabled`；agent/team 置 Preset `enabled`（`PresetService::resolve` 本就把 disabled 挡在 run 之外，无需改运行路径）；**skill 例外**＝目录标记（`action: "marked"` + `code: "skill_disable_flag_only"`）；标志位**只对运行时真的动了的组件**翻转 | `app_server_installer.rs`、`nomifun-mcp/src/service.rs` |
| ④ | **结构化 `outcomes`**：`AppServerInstallOutcome { component_id, kind, action, ok, code?, message? }`，`action` 八值，`ok:false` 只在请求状态未达到时；`code` 为**闭集**（服务端 15 个 + SDK `store` 子客户端侧 2 个 = 17）；`install/run` 的 `errors` 不再恒为 `vec![]`；`install/uninstall\|disable\|enable` 的 `InstallStatus` 与 `store/install-entry` 的 `StoreInstallResult` 均带上它（后者由安装器**转发**） | `nomifun-api-types/src/app_server.rs`、`web/packages/protocol/src/protocol.ts`、`05` §4.5.2 |
| ⑤ | **运行路径点名禁用**：`agent/run` 返回稳定码 `preset_disabled`；`team/run` 返回 `agent_disabled`（**点名**成员），检查在 `resolve_team_members` 里**每次运行、先于 Connector 栅栏**做（**不**埋在 `ensure_team_template`——那里复用既有模板，只会第一次触发），同一遍还报 `agent_not_installed` | `nomifun-app-server/src/team_run.rs`、`lib.rs`、`05` §5.2 |
| ⑥ | **`store/install-entry` 版本感知**：wire 上**没有**更新动词，所以「卸载再安装」是唯一升级路径——原实现无条件复用既有快照，忠实重装**旧**版本，而 `store/list` 同时广告 `update_available: true`（一个永远不会发生的升级）。现在：已安装 → no-op（`reused=true`，不偷偷升级）；未安装且同版本 → 装该快照；未安装且版本漂移（前进/回滚）→ 经 `market/entry-import` **重新导入**并装新快照（旧快照保持不可变）。版本推导收敛为**一个共享 helper**（`entry_live_version`），`store/list` 与 `install_entry` 共用，目录与安装器不可能互相矛盾 | `app_server_store.rs`、`18` §9.1、`02` TC-INS-008 |
| ⑦ | **`.store` 子客户端（20 例如新建测试）**：`launchClient` 返回的 client 现带 `readonly store: StoreClient`，把五个扁平动词收敛成一条状态机（`list`/`search`/`installed`/`checkUpdates`/`updateHint`/`install`/`uninstall`/`setEnabled`），**不新增任何 wire 方法**；两条自我约束＝绝不上报服务端没上报的状态（`components` 逐字透传 + `ok` 与 `ready` 分离）、绝不掩盖已文档化的不对称（connector 以 disabled 注册，故先 `enable` 再探测；需授权立即返回 `authorization_required` 不烧超时）。`ready_timeout` 连同**成功的安装**一起返回，慢 connector 不等于安装失败 | `web/packages/client/src/store.ts`、`store.test.ts`（**20 例**）、`07` §6.3 |
| ⑧ | **宿主管理面类型搬进 `protocol`**：六个方法（`config/get\|set`、`skill/create\|update\|delete\|copy`）的**类型**移入 `@flowy-agent-store/protocol`，`client` **刻意不给** typed method——这是**可发现性/稳定性边界，不是访问边界**（transport 公开、服务端按磁盘 origin 判定可写）；原写法把「不该给」写成了假理由，本次改成真理由 | `web/packages/protocol/src/protocol.ts`、`07` §1 非目标 |
| ⑨ | **摘掉商店的假「更新」控件**：卡片已安装态由 `<button onClick={onInstall}>`（再点只会拿 `reused:true` 空返回）改成 `role="note"` 说明元素；抽屉里那枚接了 `onInstall` 的「更新/更新中」按钮删掉，换成状态标签 + 一句真话（`catalog.storeUpdateNote`：**当前版本没有原地升级：先卸载再安装，即可取到市场里的新版本**）——这句话现在成立，正是因为 ⑥ 修掉了「卸载再安装仍拿回旧版本」。i18n：`storeUpdate` 改述状态、新增 `storeUpdateNote`、删除孤儿键 `storeUpdating` | `web/src/components/CatalogView.tsx`、`web/src/i18n/{zh-CN,en-US}.ts`、§6 红线 |
| ⑩ | **契约指纹 `2026-09-14` → `2026-09-15`**：现有 DTO 加字段即 wire 变更；仍**无方法增删**，`46 / 65` 计数守卫未动；落点仍是那 8 处代码/夹具 + 2 处站点文档 | 见 §7 决策 4 |

**本批未做 / 不声称**（如实登记）：① 本记录**不包含**任何自跑测试读数——上表 ⑦ 的「20 例」是新建文件 `store.test.ts` 的用例数（静态计数），不是运行结果；② `web/packages/client/src/client.ts` 里 `disableInstall` 的 JSDoc 仍写着 "runtime artifacts stay"，与 ③ 的新语义不符——**文档已按真实语义写**（`05` §4.5：disable 移动运行时状态、产物保留），该注释属 `web/` 侧，交给并行写入者；③ P0 门禁未因此推进（见 `09` §9）。
...
> **2026-09-11（R14 轮）更新**：下表的 **B 档 ③ R14 已收口**（原列为「未做（设计已定）」，现改为 ✅ 保留该行以对照原判定）；**当前真正未做的只剩 D 档 R22 一项**。R14 的完整落地记录见本节末尾「R14 逐轮 usage 持久化 · 落地记录」。

| 项 | 状态 | 原因与解锁条件 |
| --- | --- | --- |
| **B 档 ③ R14 逐轮 usage 持久化** | ✅ **已完成（2026-09-11，R14 轮）** | 原设计按序落地：**新迁移 `058_*`** 给 `app_server_context_usage` 加 `last_turn_input_tokens` / `last_turn_output_tokens`（INTEGER、**可空、无默认值**；**未改 001–057**）→ DB 模型 + 仓储 trait / sqlite 两处 → 写点取 `TurnCompleted` 的 `input_tokens` / `output_tokens`（**未上报写 NULL，绝不写 0**，也不拿占用顶替）→ `ContextUsageView` additive 两字段（`skip_serializing_if`：缺席即不上 wire）→ 前端 `persistedTurnUsage` + `restoreTurnUsage` 回填（金额仍走批 6 的 `turnCostUsd` / `costText`）。**剩余边界（不属本项，已写进迁移注释）**：该表是按会话唯一行，只承载**最近一轮**；逐轮历史回放需另立表且不在本次范围。 |
...
#### 基线红测试收口记录（2026-09-11 · 三个既有红用例）

> 真实读数，命令与计数照抄。这三例在 R14 轮被**证伪归属**（与本项无关）后登记为「未修，交归属方」；本轮收口。**原则：只改测试与一处可见性导出，不动任何迁移。**

| # | 用例 | 根因 | 修法 |
| --- | --- | --- | --- |
| ① | `id_schema_contract::every_product_table_has_one_integer_autoincrement_row_primary_key`（`:131`） | 期望 `tables.len() == 110`，实际 **114**（`052`/`053`/`055` 共追加 4 张表，期望没跟着走） | 期望改 **114**，并写明「这个数字是**刻意显式**的契约：新增表时应当被刻意改一次」，而不是从迁移里推导 |
| ② | `id_schema_contract::all_nontechnical_id_columns_are_text_and_only_id_is_a_technical_key`（`:257`） | 用例自带一份「哪些 `*_id` 可以是非 TEXT」的副本，与契约登记表 `NON_REFERENCE_ID_COLUMNS` 漂移；迁移 `052` 给 `oauth_tokens` 加了 `registration_id INTEGER`（逻辑链接）后立刻打架 | `NON_REFERENCE_ID_COLUMNS` 由私有改 `pub` 并从 `nomifun_db` 导出；用例改为**读同一张表**判定例外（`pk == 0` 与非 `_row_id` 的断言原样保留） |
| ③ | `provider_platform_migration::exact_legacy_preset_roots_gain_provider_identity_and_retired_roots_move`（`:61`） | **用例陈旧**：旧平台/URL 的重分类在**迁移 033**（`033_classify_legacy_provider_presets.sql`，`WHERE platform = 'custom'`，ppio/ctyun 还会把 URL 归一到新根），而用例的迁移目标是 `30`——**030 是 `ssh_hosts`**，于是 033 根本没跑 | `migrate_to(&pool, 30)` → **33**；fixture 与期望逐条对过 033 的 CASE（含 `new-api` 行**有意不动**）后保持原样 |

**验证（真实输出）**

| 命令 | 结果 |
| --- | --- |
| `cargo test -p nomifun-db --test id_schema_contract --no-fail-fast` | 修前 **20 passed / 2 failed**（`left: 114 / right: 110`、`left: "INTEGER" / right: "TEXT"`）→ 修后 **22 passed / 0 failed** |
| `cargo test -p nomifun-db --test provider_platform_migration --no-fail-fast` | 修前 **0 passed / 1 failed** → 修后 **1 passed / 0 failed** |

**未做（如实登记）**：整包 `cargo test -p nomifun-db`（含 lib，单次约 25 分钟）本轮**未重跑**——lib 与其余集成目标在上轮已全绿，本项只碰两个测试文件与一处导出；如需终局确认，跑一次整包即可。**未改迁移、未改 src 语义**（`pub` 只是可见性）。

#### R22 落地记录（2026-09-11 · MCP 连接器凭据明文，`17` P1 收口）

> 真实读数，命令与计数照抄。本项只做一件事：把 MCP 连接器 `env` 的**值**从快照里拿掉、改为**引用**，并在 MCP 真正启动时按引用注入真值（`17` §6 / `21` D5=C）。两步**同批**——只删值不补注入＝导入后连接器静默不可用（原始卡点）。

**① 引用语法与解析原语（`nomifun-common/src/secret_ref.rs`，新文件）**：`secret:<NAME>`（沿用浏览器引擎既有 `secret:NAME` 约定，全仓一个语法）；`parse_secret_ref` 纯函数（裸 `secret:` 不算引用）；`set_credentials` / `credentials` 进程级凭据表（`RwLock`，宿主启动写一次）；`resolve_env` / `resolve_env_with`——普通值**原样透传**（非敏感 env 不受影响）、引用从凭据表解析（**config 优先于进程 env**）、**解析不到即省略该变量**（fail-closed，绝不注入 `secret:` 字面量），并在 `missing` 里回报**键名**（键名不是秘密）。

**② 导入侧改写（`nomifun-importer/src/import.rs`）**：新增 `rewrite_secret_env`——`mcp.json` 的 `env` 中**敏感键**（`looks_sensitive_key`：键名含 `api` / `token` / `secret` / `password` / `apikey`，与 `userConfig` 的判定**同一谓词**，抽出后两处共用）的值改写为 `secret:<KEY>` 并 `builder.warn`；非敏感键、非字符串、已是引用的值一律原样。快照与 DB 行从此**只可能含引用**。

**③ 装配期注入（3 条 spawn 路径）**：`nomifun-mcp/src/connection_test/mod.rs`（连接测试）、`nomifun-ai-agent/src/factory/nomi.rs`（`row_to_mcp_server_config` + `session_server_to_mcp_server_config`）、`nomifun-ai-agent/src/factory/acp.rs`（`row_to_sdk_mcp_server` + `session_server_to_sdk_mcp_server`，经新增 `resolve_env_entries`）——都在把 env 交给子进程 / SDK 之前解析，缺凭据即省略并 `warn`（只记键名）。

**④ 值侧与填写入口（`21` D5=C）**：`AgentStoreConfig` 新增 `[credentials]`（手填表）；宿主 `apps/agent-store/src/main.rs` 启动时 `set_credentials(config.credentials)`（与既有 `set_distill_host_override` 同构，config 只读一次）。该表**不入** `config/get`（视图是显式投影 `config_view`，只 exist / default_model / providers / memory）也**不入** `config/set`（`AgentStoreConfigPatch` 的 `deny_unknown_fields` 直接拒绝）——协议面读不到也写不进凭据。

**验证（真实读数）**：`nomifun-common --lib secret_ref` **6 passed**；`nomifun-importer` **38 passed**（`mcp_connector_import_preserves_stdio_args_and_env` 改为断言快照只含 `secret:DEMO_TOKEN` 且不含明文，状态 `completed-with-warnings`）；`nomifun-ai-agent --lib secret_ref_env` **2 passed**；`nomifun-mcp --lib` **251 passed** 且 `--test connection_test_integration` **10 passed**；`nomifun-app-server --lib` agent_store **17 passed** / config **11 passed**；`nomifun-app --test importer_e2e` **10 passed**；`rustfmt --check` 与 `clippy`（本次文件）无新增。**迁移**：无（`[credentials]` 是 `config.toml` 手填表，不落库、不进 DB schema）。

**已知边界（有意）**：① 敏感判定按**键名**（与 `userConfig` 同口径）——键名不含敏感词的凭据不会自动脱敏，如需强制属后续增强；② 只覆盖 MCP 连接器 `env`（`17` P1 的范围），CLI 连接器 / 其它组件不走此路径；③ 凭据**不进**设置面（无 UI 填写入口，手编 `config.toml`），与「secret 类配置集中、不经协议读写」的口径一致。

#### R24 落地记录（2026-09-11，批 9 · 市场条目 `strict` 阻断）

> 真实读数，命令与计数照抄。本项只做 **`strict=true`** 那一半（`17` §7 / `02` §8 / §11.1）：来源缺 `plugin.json` 时**可见地拒绝**。`strict=false` 的「条目可补充或代替清单」**有意拆出**（用户 2026-09-11 拍板）→ `17` §10 **P5**，不属本项范围。

**归属订正（本轮定案）**：`strict` 是**市场条目字段**——宿主是 `.codebuddy-plugin/marketplace.json` 的 `plugins[]` 行（`02` §8 原文即把它与 `name` / `source` / `commands` / `agents`… 并列），而 `plugin.json` 是它**检查的对象**。`17` §3 原将它列在「插件清单字段」表内属**表位置错误**，本轮已挪出并加注；`02` §11.1 也补上了这条阻断（原先 `17` §7 的引用是**悬空**的）。

| 层 | 改动 |
| --- | --- |
| 判定（纯函数） | `nomifun-app/src/app_server_marketplace.rs` 新增 `strict_entry_block(strict, plugin_json_present) -> Option<String>`：只有 `strict && !plugin_json_present` 才给原因；`strict=false` 一律 `None`（缺清单**不是**错误，只是 P5 未实现）。原因文案**不含文件路径**（`02` §9：来源路径仅内部追溯） |
| 探测 | `probe_plugin_market` 读 `plugins[].strict`（默认 `false`）；`strict=true` 且缺清单的行**保留并标记** `blocked_reason`，不再 `continue` 静默消失；**非 strict 缺清单仍照旧跳过**（P5 行为不变）。`market_fetch.rs` 的 URL 内联清单同口径 |
| 导入闸门 | `import_entry` 在解析出的源目录上**重新判定**（目录市场是 live view，不能只信存下来的投影）→ 命中即返回 `nomifun_importer::blocked_import_result(...)`（本轮新导出，与 importer 内部阻断**同一结果契约**）→ `status=blocked`、**不产生快照行** |
| 线协议（additive） | `MarketplaceEntry.strict` / `blocked_reason`（DB 侧是 `plugin_marketplaces.entries_json`，**serde additive、无需迁移**，旧行反序列化为 `false` / `None`）→ `AppServerMarketplaceEntry` → `AppServerStoreItem.blocked_reason`；`@flowy-agent-store/protocol` 同步；三处均 `skip_serializing_if`，未声明时**线格式逐字不变** |
| 一键安装 | `app_server_store.rs` 的 `install_entry`：导入结果为 `blocked` 时**不再拿那个从未落库的 `snapshot_id` 去安装**（原先会去注册一个幽灵 id），改为原样回报 `errors` |
| Web | 商店卡片 / 抽屉 / 市场详情条目行：有 `blocked_reason` → 标「不可安装」（`catalog.storeBlocked`，中英各一份）、撤掉安装与导入动作、原因挂 `title`，抽屉内**整句显示**（不靠 hover）；样式只用既有主题变量 |

**验证（真实输出）**

| 命令 | 结果 |
| --- | --- |
| `cargo check -p nomifun-db -p nomifun-api-types -p nomifun-importer -p nomifun-app -p nomifun-app-server --tests` | **exit 0**（无 `^error`） |
| `cargo test -p nomifun-app --test importer_e2e --no-fail-fast` | **10 passed / 0 failed**（+1 例 `importer_market_strict_entry_is_listed_but_refused`：真 app 全链路——条目可见 + 带原因 → store 标记 → 导入 `blocked` 且**历史为空** → 一键安装 `installed_count=0` 且不 500） |
| `cargo test -p nomifun-app --lib marketplace --no-fail-fast` | **14 passed / 0 failed**（+3 例：判定只在 strict + 缺清单时触发）/ 线协议未声明时**两个键都不出现** / 探测保留被阻断行而其余照旧跳过） |
| `cargo test -p nomifun-db --lib marketplace` · `cargo test -p nomifun-app-server --lib catalog` | 各 **10 passed / 0 failed** |
| `cargo clippy -p nomifun-app -p nomifun-importer -p nomifun-db -p nomifun-app-server --all-targets` | 本项改动文件 **0 告警**（其余为既有告警） |
| `rustfmt --edition 2024 --check`（10 个改动文件） | 干净 |
| `cd web && bun run typecheck` · `bun run test` · `bun run build` | **exit 0** / **385 passed / 1 skipped（386 例、51 文件）** / **✓ built in 2.42s** |
| `bun run check:theme` · `check:icons` · `check:docs-sync` | 6 主题全 ✓ / clean / 0 drift |

**既有红（与本项无关，沿用上轮结论）**：整仓 `bun run check` 在 `typecheck --filter=./ui`（旧桌面 renderer）与 `check:i18n`（只扫 `ui/.../locales`，报 `videoGeneration.*` 漂移）就红；`check:dead-css`、`check:agent-vocabulary` 亦有既有失败项（已核：均**不含**本项新增的类名 / 文件）。本项改动全部落在 `crates/backend/**`、`web/**`、`docs/agent-store/**`。

**本项未做（已登记）**：① `strict=false` 的「条目代替清单」→ `17` §10 **P5**；② **导入层的依赖 SemVer 阻断**（`nomifun-importer` 只登记不决策）仍属 R23 的另一半，不在本项范围。

#### R23 落地记录（2026-09-11，批 8 · extension 层依赖阻断）

> 真实读数，命令与计数照抄。本项只做一件事：把「依赖不可满足」从只 `warn!` 变成可开启的**阻断**，且**默认关**（用户 2026-09-11 拍板），保证不开启时行为逐字不变。导入层（R24）不在本项范围。

| 层 | 改动 |
| --- | --- |
| 判定（纯函数） | `nomifun-extension/src/registry_helpers.rs` 新增 `blocked_dependents(&DependencyValidationResult) -> BTreeSet<String>`：只收集 `Missing` / `VersionMismatch` 的**依赖方**名；`Circular` **有意排除**（`dependency.rs` 明确要求环上扩展仍尽力加载） |
| 注册表 | `nomifun-extension/src/registry.rs`：新增字段 `strict_dependencies`（`new()` 置 `false`，签名不变 → 既有调用方零改动）＋ builder `with_strict_dependencies(bool)` ＋ `apply_dependency_policy`；`initialize_with_scan_paths` 与 `hot_reload` **同口径**接入（后者原先连 `dep_result` 都丢弃） |
| 逃生口 | `nomifun-app-server/src/agent_store.rs`：新增 `[import]` 段（`AgentStoreImport { strict_dependencies: Option<bool> }`）＋ `AgentStoreConfig::strict_dependencies()`（默认 `false`）。**有意不进 `config/set` 白名单、不进设置界面**：注册表在构造时读一次，做成界面开关会暗示「改了立即生效」这一并不存在的能力 |
| 宿主接线 | `nomifun-app/src/router/state.rs` 的 `build_extension_states`：读 `~/.agent-store/config.toml`（与 App Server 同一 `default_path`）→ `.with_strict_dependencies(…)`；文件缺失 / 解析失败 → 关（历史默认） |

**验证（真实输出）**

| 命令 | 结果 |
| --- | --- |
| `cargo check -p nomifun-extension -p nomifun-app-server -p nomifun-app --tests` | **Finished**（无 `^error`；仅既有 warning） |
| `cargo test -p nomifun-extension --lib --test registry_test --no-fail-fast` | lib **447 passed / 0 failed / 5 ignored**（+3 例 `blocked_dependents_*`）；`registry_test` **15 passed / 0 failed**（+3 例：默认关保留旧行为 / 开启后只拒依赖方 / healthy 图不受影响） |
| `cargo test -p nomifun-app-server --lib agent_store --no-fail-fast` | **16 passed / 0 failed**（+1 例 `import_strict_dependencies_is_an_explicit_opt_in`：无表 / 空表 / `false` 全为关，仅 `true` 为开） |

**口径订正（§5.2 R23 行原写「阻断默认开」）**：以用户 2026-09-11 的决定为准——**默认关**，完全保留现状行为；触发条件（第一个外部发布者挑战）出现时只翻默认值 + 走已建好的 changelog 公告面。

**本项未做（已登记）**：① **导入层半边已于批 10 落地**（见下方「R23 导入层半边 · 落地记录」）；② 被拒扩展**没有对外可观测面**（只记 `warn!` 日志）——`capabilities` / 目录页尚未暴露「因依赖被拒」这一状态，需要时属独立小项。

#### R23 导入层半边 · 落地记录（2026-09-11，批 10 · 依赖 SemVer 阻断）

> 真实读数，命令与计数照抄。补上 `17` §10 P3 的**导入层**：`nomifun-importer` 此前只把依赖**登记**成组件、从不参与任何决策。与批 8（extension 层）共用**同一个逃生口** `[import].strict_dependencies`，**默认关** ⇒ 不开启时行为逐字不变。

**「强依赖」的判定（本轮定案，写进 `02` §11.1）**：规范原文是「必需依赖无法解析，**且**来源声明为强依赖」，但 `02` §8 的声明形态只有 `{name, version?, marketplace?}`、**没有 `required` 标记**。故以**是否声明了 `version` 范围**为准——声明了即视为来源提出了要求（可阻断），只写名字则只是备注（仅登记、永不阻断）。

| 层 | 改动 |
| --- | --- |
| 判定（纯函数，新文件） | `nomifun-importer/src/dependency.rs`：`DeclaredDependency`（读归一化后的 `{name, version?, group?}`）＋ `CatalogIndex`（名称 → 可信版本；**空版本列表＝已知但不可比**）＋ `check_dependencies` → `DependencyProblem`（`MissingName` / `UnparsableRange` / `Missing` / `Unsatisfied`，每条自带一句可直接进 `errors` 的中文说明）。版本语义**逐字对齐 extension 层**（裸版本＝精确、`^`/`~` 同 semver），两层不会对同一条声明给出不同答案 |
| 目录投影 | `ImporterService::dependency_catalog_index()`：**快照行**贡献名称**与版本**（`plugins` 组依赖指向插件，这个版本是真的）；**组件行只贡献名称**——组件 payload 里的 `version` 是它**宿主插件**的版本，拿来比区间等于猜。上限 `CATALOG_INDEX_LIMIT = 500` |
| 闸门位置 | 放在**幂等判定之后、`insert_snapshot_with_components` 之前**：① 已存在的快照**不因重导被收回**（`Reuse` 早返回，什么都没被拿走）；② 新导入命中即 `remove_dir_all(materialized)` + `blocked_import_result(...)`，**不落库** |
| 接线 | `nomifun-app/src/router/state.rs` 新增 `strict_dependencies_enabled(services)`（真值源＝`AppServices::agent_store_config_path`，**不是** `~/.agent-store/config.toml` 约定——测试注入 `None` 就不该继承开发者个人配置），extension 注册表与 importer **都改用它**；`routes.rs` 只**构建一次**带策略的 `ImporterService`，四条导入路径（imports / markets / store 内部 / store 参数）全部 `.clone()` 复用，不会各自漂移 |

**两处有意保守（避免误伤，已在 `17` §10 P3 与代码注释写明）**：① **组件名只判存在性**——组件行没有可信的自有版本，因此对技能/连接器依赖的范围**不做判定**（宁可漏挡也不误挡）；② **目录之外满足的依赖不可见**（内置技能、手工配置的 MCP 连接器）⇒ 不阻断。再加逃生口默认关，误伤面被限制在「主动开启者 + 强依赖 + 目录内确实缺失」。
...
| --- | --- |
| `cargo check --tests --workspace` | **exit 0** |
| `cargo test -p nomifun-importer --lib --test importer_tests --no-fail-fast` | lib **38 passed / 0 failed**（+13 例：弱依赖不阻断 / 空目录 Missing / 可满足 / 裸版本=精确 / `^`·`~` 边界 / 范围不可解析 / 无名强依赖按位报错 / 大小写与 `v` 前缀 / 多候选取并集 / 已知但不可比不阻断 / 文案带依赖名且不含路径 / 索引空值处理 / 声明读取）；`importer_tests` **21 passed / 0 failed**（+4 例：默认关下缺依赖仍可导入（含 `software-company` 真实 fixture 的 `shared-helpers@1.0.0`）/ 开启后缺失 → `blocked` 且不落库、文案给出路且**不含来源路径** / 开启后「可满足的插件范围 + 已知组件名」通过 / 开启后范围不满足 → `blocked` 并报出目录里的实际版本） |
| `cargo test -p nomifun-app --test importer_e2e --no-fail-fast` | **10 passed / 0 failed**（宿主四条导入路径改为共用同一 importer 后无回归） |
| `cargo test -p nomifun-extension --lib --test registry_test` · `cargo test -p nomifun-app --lib agent_store` | lib **447 passed / 5 ignored**、registry_test **15 passed**；`agent_store` 配置用例通过 |
| `cargo clippy -p nomifun-importer -p nomifun-app --all-targets` · `rustfmt --edition 2024 --check`（7 文件） | 改动文件 **0 告警**（其余为既有告警） / **干净** |

**口径订正（遗留）**：本轮把 `state.rs` 的配置读取从 `AgentStoreConfig::default_path()` 改为 `AppServices::agent_store_config_path`——与 App Server 的其余读取（models 目录等）同一真值源，测试注入 `None` 时不再可能读到开发机上的个人配置。

**本项未做（已登记）**：见上一条 ②——被依赖阻断的**导入**（而不是扩展）同样只在 `errors` 里可见，没有独立的对外状态位。

#### R15 落地记录（2026-09-11 · 附件 / 图片输入）

> 真实读数，命令与计数照抄。载体＝**路径引用**（用户 2026-09-11 拍板）；本项只做「让附件真的到模型」，不做任何字节面上传。

**契约（`05` §12.1 已同步）**：`conversation/send` 新增可选 `attachments: string[]`，每一项是**本会话工作区内**真实文件的绝对路径。宿主按 owner 作用域解析会话工作区后逐个 canonicalize，越界（`..` / 指向外部的符号链接 / 别的盘）、相对路径、URL、不存在一律拒绝（`invalid_request` / `workspace_denied`），单次上限 **10**、重复路径只算一条；字段纯加法（不传＝无附件），请求体仍 `deny_unknown_fields`。

**为什么边界必须放在宿主**：运行时对 App Server 会话的 `image_read_root` 取 `NomiBuildExtra.write_root`，而宿主没有设置它（`None` = 不受限）——引擎不会替我们收紧。少了这一层，一个远程 WebUI 客户端就能把宿主上任意文件读进模型上下文。

| 层 | 改动 |
| --- | --- |
| 协议 | `ConversationSendRequest.attachments`（HTTP 与 WS 共用）+ `WsConversationSend.attachments`（`#[serde(default)]`）；WS 臂透传 |
| 宿主准入 | `resolve_conversation_attachments` + `validate_attachment_path`（`MAX_CONVERSATION_ATTACHMENTS = 10`）；返回**客户端给的字符串**（Windows 的 canonicalize 会带 `\\?\` 前缀，运行时不吃），canonical 只用于判界与去重 |
| SDK 包 | `conversations.send(..., attachments = [])`（缺省不改变 wire 形状）+ `http-transport` 路由表 body 加 `attachments` |
| 客户端前置校验 | `web/src/lib/attachments.ts`：与运行时 `image_attachments.rs` 的 `classify_extension` 逐条对齐（只支持 png/jpg/jpeg/webp；gif/bmp/tif/tiff/ico/avif/heic/heif/svg 单独一类并说明原因；**非图片不提供**——运行时直接忽略它，等于模型看不到） |
| store | `composerAttachments` + 增/删/清（去重、按上限截断、只收支持类型）；`submitTurn` 透传；**拿到回执才清空**（失败保留，重发复用原幂等键）；`@` 专家起 Run 时带附件**明确拒绝**（`agent/run` 没有附件载体），不静默丢 |
| UI | `AttachmentPicker.tsx`（只列会话工作区文件，不可附的照样列出并说明原因）+ composer 的「添加文件」菜单项（原先是空占位）打开它 + 输入框上方 chips（可移除）+ 双语文案 + `.composer-attach*` 样式（只用既有主题变量） |

**验证（真实输出）**

| 命令 | 结果 |
| --- | --- |
| `cargo test -p nomifun-app-server --lib --no-fail-fast` | **99 passed / 0 failed**（+2：`attachment_paths_must_be_absolute_files_inside_the_workspace` 覆盖「工作区内文件接受 / 相对路径 / 空 / 不存在 / 目录 / 工作区外 / `..` 逃逸」；`ws_conversation_send_takes_attachments_additively` 钉住加法与 `deny_unknown_fields`） |
| `cargo check --tests --workspace` | **exit 0**（Finished） |
| `rustfmt --edition 2024 --check crates/backend/nomifun-app-server/src/lib.rs` | 干净 |
| `cd web && bun run typecheck` | **exit 0** |
| `cd web && bun run test` | **385 passed / 1 skipped（386 例、51 文件）**（改动前 373/1、49 文件；新增 12 例＝7 例 `attachments.test.ts` + 5 例 `appStore.attachments.test.ts`） |
| `cd web && bun run build` | **exit 0**（`✓ built in 2.55s`） |
| `bun run check:theme` | 6 个主题 **全部 ✓** |

**后续（2026-09-11，用户指令「加一条宿主上传落点 + 前端粘贴/拖拽」）——本段登记项已落地**：① **落点改为会话工作区**：`/api/fs/upload` 新增可选 multipart 字段 `workspace`，带上时字节写进 `<workspace>/<file_name>`（`file_service.write_file`，复用同一套 allowed-roots 准入），**不再是** tmp；同名冲突按 `(n)` 后缀避让（`unique_upload_target`），不覆盖既有文件。② **前端**：`client.uploadFileToWorkspace(workspace, file)`（multipart POST 到宿主**根**路径）+ Composer 的**粘贴**（`onPaste`）与**拖拽**（`onDragOver` / `onDrop`）——先落工作区拿回绝对路径，再走既有 `addComposerAttachments`（路径引用）。③ 无工作区 / 非支持类型 / 上传失败均**明确 toast**，不静默丢弃。见下「R15 字节通道 · 落地记录」。

#### R15 字节通道 · 落地记录（2026-09-11 · 拖拽 / 粘贴本地文件）

> 真实读数。上一段登记的「解锁条件＝明确要不要开这条字节通道」已由用户拍板开启；本项把落点做进**会话工作区**，从而不引入「第二条路径权威」。

| 层 | 改动 |
| --- | --- |
| 上传落点（后端，`nomifun-file`） | `/api/fs/upload` 的 multipart 新增可选字段 `workspace`：带上时目标＝`<workspace>/<file_name>`，经 `FileService::write_file` 落盘——该方法用**同一套 allowed-roots 准入**（`validate_path_for_write_authority`），并照常发 `fileStream.contentUpdate`。不带 `workspace` 时行为逐字不变（仍落 tmp，向后兼容）。同名冲突由 `unique_upload_target` 加 `(n)` 后缀避让，不覆盖既有文件 |
| 客户端 | `web/src/lib/client.ts`：`uploadFileToWorkspace(workspace, file)`（FormData：`file` / `file_name` / `workspace`）+ 私有 `httpPostRootForm`（宿主根路径的 multipart POST，`Content-Type` 交给浏览器以便正确带 boundary） |
| Composer | `onPaste`（剪贴板含文件即接管，否则不干预文本粘贴）与 `onDragOver` / `onDrop`（仅带 `Files` 的 drag 才 `preventDefault`）；`uploadLocalFiles` 逐个校验类型（与既有 `classifyAttachment` 同口径）→ 上传 →`addComposerAttachments(路径)`；无工作区 / 类型不符 / 上传失败分别 toast（`composer.attachNoWorkspace` / `attachRejectedFile` / `attachUploadFailed`） |
| 不做 | 不做内联 base64 载体（维持路径引用）；不改 tmp 上传分支的行为（其它调用方不受影响） |

**验证（真实输出）**：`cargo test -p nomifun-file --lib` → **187 passed / 0 failed**（+1 例 `unique_upload_target_avoids_clobbering`）；`rustfmt --edition 2024 --check crates/backend/nomifun-file/src/routes.rs` → 干净；`cargo clippy -p nomifun-file --all-targets` → 本次改动 0 告警（仅有既有 `unused variable: link`）；`cd web && bun run typecheck` → **exit 0**；`cd web && bun run test` → **399 passed / 1 skipped（400 例、53 文件）**（+1 例 `lib/client.upload.test.ts`：mock fetch 走完整握手，断言打在宿主根路径 `/api/fs/upload`、multipart 带 `workspace` 与文件）；`cd web && bun run build` → **exit 0**。**迁移**：无。

**已知边界（有意）**：① 落点固定在**工作区根**（不选子目录）——文件名由浏览器给出（`File.name`），冲突自动加后缀；② 粘贴 / 拖拽**只收图片**（与附件载体同口径：非图片运行时看不到）；③ 未做「拖到具体目录」与进度条（本地宿主，收益低）；④ 同名冲突的「查存在 → 写」之间有竞态，用户主动操作可接受（无后缀才是静默丢数据）。

**已知边界（有意）**：非图片附件不提供（运行时忽略它，模型看不到）；一次最多 10 个；附件只随普通消息（`@` 专家起 Run 不支持）；重试 / 重新生成 / 编辑重发**不重放附件**（附件未按轮持久化；引擎自己的截断续写走回执里的 `files`，不受影响）。

#### R20a 落地记录（2026-09-11 · 产物「按 Run 归属」）

> 真实读数，命令与计数照抄。本项只做一件事：让产物列表能说清「这份文件是哪个 Run 的哪一步产出的」，并能跳过去。

**落地路径订正（与原 R20 行建议不同，理由在此）**：原行写「宿主面文件服务（`/api/fs/list`）只需补一层映射」。**未采纳**——那会把一个**通用文件端点**（`nomifun-file` 的 `list_workspace_files`，只吃 `root`）耦合到 agent execution 的 DB 查询上，且需要新开一层「workspace → runs」的解析；收益只是省一次客户端投影。改为**客户端纯投影**：归属的唯一数据源就是**已经拿到的 `run/plan` 快照**（`attempt.output_files`），零协议增量、零新端点、零新权限面，也不动已有文件服务的行为。

| 层 | 改动 |
| --- | --- |
| 投影（纯函数，新文件） | `web/src/lib/artifact-owners.ts`：`normalizeArtifactKey`（去空白 / `./` / 前导分隔符、`\`→`/`；**不折叠大小写**——宁可漏认也不误认；空路径 → `null`）＋ `collectArtifactOwners(plan, runId)`（同路径**保留最先出现**的那次尝试：产物是它先产出的，覆盖成最后一次会把「谁产出的」变成「谁最后碰过」） |
| store | `artifactOwners: ArtifactOwnerIndex`（**按会话**：`conversationId` + `byPath`）；`loadRunPlan` 成功时并入**仅当** `runConversationId === selectedConversationId`（否则会把别处的归属贴到当前会话）；新增 `focusArtifactOwner` |
| 跳转 | `focusArtifactOwner`：关抽屉（含预览）→ 必要时 `followRun`（归属可能来自已不是当前跟随的 Run）→ 滚到 `plan-step-*`；锚点要等 `run/plan` 落库后 `RunDetail` 才画出来，故短重试（12 × 40ms），**无真实 DOM 时直接返回**（不假装滚过、不空转） |
| 复用（去重） | 「滚到锚点 + 高亮一瞬」抽到 `lib/run-plan.ts` 的 `scrollToAnchor`（返回是否命中），`RunDetail` 原有的私有 `jumpTo` 改为调用它——两处共用同一套锚点语义与无 DOM 降级 |
| UI | `ArtifactPanel`：有归属才渲染 `步骤 {{step}}` 药丸（无归属不摆空标签），药丸是行的**兄弟节点**（不嵌套 button）；`title` 带 run id 与尝试序号；点击走 `focusArtifactOwner`。两语言加 `artifact.ownerLabel` / `artifact.ownerTitle`；样式 `.artifact-item` / `.artifact-row-owner` 只用既有主题变量（`check:theme` 通过） |

**已知边界（有意，不猜）**：只能归属**本会话里加载过计划快照的那些 Run**——没看过计划的 Run 的产物显示为「无归属」；同路径在多次加载的快照间由**更新的快照**覆盖（会话内累积）。
...
| --- | --- |
| `cd web && bun run typecheck` | **exit 0** |
| `cd web && bun run test` | **373 passed / 1 skipped（374 例、49 文件）**（改动前 362 / 1、47 文件；新增 11 例＝6 例 `artifact-owners.test.ts` + 5 例 `appStore.artifact-owners.test.ts`） |
| `bun x vitest run src/lib/artifact-owners.test.ts src/store/appStore.artifact-owners.test.ts src/lib/run-plan.test.ts src/components/RunDetail.render.test.tsx src/store/appStore.artifact-meta.test.ts` | **30 passed（5 文件）** |
| `bun run check:theme` | **全部 6 个主题 ✓** |

**门禁说明（既有、与本项无关）**：整仓 `bun run check` 在第一步 `typecheck`（`--filter=./ui`，即旧桌面 renderer 工作区）就失败，报错文件全在 `ui/src/renderer/**`（`editResubmitRecovery.test.ts` 的 `result_error_code` 可选性、`VideoHomeComposer` 的 `Popover` props 等）；`check:i18n` 同理只扫 `ui/src/renderer/services/i18n`（报 `videoGeneration.*` 漂移），**不覆盖 `web/src/i18n`**。本项改动全部落在 `web/`、`crates/`、`docs/agent-store/`（`git status` 可核，无 `ui/` 文件）——这两处红是工作树既有状态，已按「不是自己引入的不顺手改」处理。

**后续（2026-09-11）**：原「本项未做」的 R20b「接受 / 回退」已落地——见下条「R20b 落地记录」；落点**不是** `capabilities.artifacts` 写面，而是宿主已有的 git 基线快照服务。

#### R20b 落地记录（2026-09-11 · 产物「接受 / 回退」）

> 真实读数，命令与计数照抄。本项只做一件事：让**工作区变更**能逐条**接受或回退**。口径订正：原行写「等 Artifact Phase（需 `capabilities.artifacts` 写面）」——**不需要**。`nomifun-file` 已有 **git 基线快照服务**（`SnapshotService`），宿主文件服务已把它的 `compare` / `stage` / `discard` 暴露为 `/api/fs/snapshot/*`；「接受 / 回退」就是 `stage` / `discard`，前端只做封装 + UI（用户 2026-09-11 指令）。

**原语（既有，本项未改后端）**：`crates/backend/nomifun-file/src/snapshot_service/`——`init` 建基线（`git-repo` 直接用已有 `.git`；否则建临时 `nomifun-snapshot-*` repo，`core.worktree` 指向工作区）；`compare` 出 `staged` / `unstaged`；`stage_file` / `stage_all`（接受）、`unstage_file` / `unstage_all`（撤销接受）、`discard_file(operation)`（回退：`create` 删新文件、`modify` / `delete` 从基线 checkout 恢复）、`reset_file`（unstage + 回退）。路由在 `nomifun-file/src/routes.rs` 的 `E. Workspace snapshot` 段；**可达性已核**：`agent-store` 宿主经 `nomifun_app::create_router` 挂 `file_routes`（`crates/backend/nomifun-app/src/router/routes.rs`），`build_file_state` 传入与对话服务共享的 `SnapshotService`（`router/state.rs`）——webui 已在用同一前缀的 `/api/fs/list`，故快照端点同样可达。

| 层 | 改动 |
| --- | --- |
| 纯层（新文件） | `web/src/lib/artifact-changes.ts`：wire 类型（`FileChangeOperation` / `SnapshotMode` / `FileChangeInfo` / `SnapshotCompare` / `SnapshotInfo`）+ `EMPTY_SNAPSHOT_COMPARE`（引用稳定的空态）+ `changeKey`（复用 `artifact-owners` 的归一化）+ `sortChanges`（大小写不敏感字母序，不改入参）+ `changeCount` |
| client | `web/src/lib/client.ts` 新增 6 个封装：`snapshotInit` / `snapshotCompare` / `snapshotStageFile` / `snapshotStageAll` / `snapshotUnstageFile` / `snapshotDiscardFile`——一律走 `httpPostRoot`（宿主根路径），`file_path` 传**相对路径**（服务端 `workdir.join(rel_path)`，传绝对路径会拼错） |
| store | 新切片：`artifactChanges` / `artifactChangesLoading` / `artifactChangesError` / `artifactSnapshot`（`disabled` 时带 `reason`）/ `artifactChangeBusy` + `refreshArtifactChanges` / `acceptArtifactChange` / `revertArtifactChange` / `acceptAllArtifactChanges` / `unstageArtifactChange`。**读取顺序**：先直接 `compare`（已跟踪时一步到位，**不**反复 `init` 推高引用计数）；仅当它抛「未初始化」才 `init` 一次建立基线再 `compare`；`init` 返回 `disabled` 则保留 reason 空态呈现（不做假保护）。动作成功后一律**重读 compare**，以后端为准 |
| UI | `ArtifactPanel` 增「文件 / 变更（n）」页签；变更页分「待处理（N）」（每行 新增/修改/删除 徽标 + 相对路径 + 接受 + 回退，头部「全部接受」）与「已接受（N）」（每行 + 撤销接受）；`disabled` 显示后端给的 reason；`changeBusy` 期间按钮禁用防重入。i18n 两语言 14 键；样式 `.artifact-tabs` / `.artifact-change-*` 只用既有主题变量 |
| 不做 | **未**引入 Artifact 协议（`artifact/list` / `artifact/get`、`capabilities.artifacts` 仍恒 `false`）；**未**做批量「回退全部」（后端无对应端点，且回退会改文件，逐条确认更稳） |

**验证（真实输出）**：`cd web && bun run typecheck` → **exit 0**；`cd web && bun run test` → **398 passed / 1 skipped（399 例、52 文件）**（改动前 385 / 1；新增 13 例＝3 例 `lib/artifact-changes.test.ts` + 10 例 `store/appStore.artifact-changes.test.ts`）；`cd web && bun run build` → **通过（exit 0）**。**迁移**：无（复用既有快照服务，不落库）。

**已知边界（有意）**：① **基线必须早于改动**——`compare` / `discard` 参照基线（workspace init 时的 HEAD / 初始提交），若首次 `init` 发生在 agent 改动**之后**，那次改动不可回退（正常路径下会话服务在回合开始已建 turn checkpoint → 已 init，故通常成立）；② 「接受」= git `stage`（登记进基线索引），不写 commit、不动 git 历史，与 `19` §3 W5「不做 Git 集成」一致；③ 快照被安全守卫拒绝（盘符根 / 系统目录等）的工作区没有变更审查，界面显示后端给出的 reason。


#### R14 逐轮 usage 持久化 · 落地记录（2026-09-11，本会话）

> 真实读数，命令与计数照抄。本项只做一件事：让「按 turn 的 token 与金额」在**重载后仍然显示**；不属本项的边界（逐轮历史回放 / 金额入库）明确不做，且写进迁移注释。

| 层 | 改动 |
| --- | --- |
| 迁移 | `crates/backend/nomifun-db/migrations/064_app_server_context_usage_last_turn.sql`（**新**）：`ALTER TABLE app_server_context_usage ADD COLUMN last_turn_input_tokens INTEGER CHECK (last_turn_input_tokens >= 0)` + 同形 `last_turn_output_tokens`；**可空、无默认值**；头注释写死三条边界（只承载最近一轮 / 金额不入库 / 不是逐轮审计）与「未上报 = SQL NULL ≠ 0」。**未改 001–057**。 |
| DB 模型 | `crates/backend/nomifun-db/src/models/app_server_context_usage.rs`：`AppServerContextUsageRow` 加两个 `Option<i64>` 字段（文档写明 NULL 与 0 的区别）。 |
| 仓储 | `src/repository/conversation.rs`（trait 签名 + 默认实现）、`src/repository/sqlite_conversation.rs`（INSERT 补两列、`ON CONFLICT DO UPDATE` 一并覆盖、上报负数 → `DbError::Conflict`）。 |
| 写入点 | `crates/backend/nomifun-conversation/src/stream_relay.rs`：新增 `reported_turn_tokens`（两侧皆 0 → `None`）+ `persist_app_server_context_usage` 落库两列；调用点守卫**保持原样**（「占用或窗口有测量值」才写行） |
| 读回投影 | `crates/backend/nomifun-app-server/src/lib.rs`：`ContextUsageView` 两字段（`skip_serializing_if`，缺席即不上 wire）+ `context_usage_view(row)` 直透（`conversation/get` 既有路径带出）。 |
| 前端 | `web/packages/protocol/src/protocol.ts`（类型 additive）、`web/src/lib/conversation-events.ts`（`persistedTurnUsage` + `restoreTurnUsage` 动作）、`web/src/store/appStore.ts`（`loadConversation` / `refreshConversation` 于 `reset` 后回填，`is_processing` 时不填）。 |
| 测试 | `crates/backend/nomifun-db/tests/app_server_context_usage_repository.rs`（+1 例 NULL 语义、扩 2 例）、`nomifun-app-server/src/lib.rs` 内嵌测试（+2 例）、`web/src/lib/conversation-events.test.ts`（**+9 例**：6 例 `persistedTurnUsage` 口径 + 3 例 `restoreTurnUsage` 动作）、`web/src/store/appStore.turn-usage-restore.test.ts`（**新，6 例**，走真实 `loadConversation`：回填带模型键 / 显式选模型优先 / 无持久化值不给 0 / 只持久化一侧也不填 / 回合在跑不回填 / 重连 `refreshConversation` 同口径）。**web 全量 347 → 362 passed / 1 skipped（46 → 47 文件）**，与新增例数（9+6=15）相符。 |

**验证（真实输出）**

| 命令 | 结果 |
| --- | --- |
| `cargo test -p nomifun-db --test app_server_context_usage_repository --no-fail-fast` | **5 passed, 0 failed**（exit 0；含本项三例） |
| `cargo test -p nomifun-app-server -p nomifun-extension --lib --no-fail-fast` | app-server **96 passed, 0 failed**、extension **444 passed, 0 failed, 5 ignored**（exit 0） |
| `cargo check --tests --workspace` | **exit 0**（3m37s；日志 `^error` 计数 0） |
| `cargo test -p nomifun-db --no-fail-fast` | 本项相关测试目标全绿；既有目标 `tests/id_schema_contract.rs` **2 例既有失败**（与 `058` 无关，已证伪，见下） |
| `cd web && bun run test` | **362 passed, 1 skipped**（47 文件；exit 0） |
| `cd web && bun run typecheck` / `bun run build` | 均 **exit 0** |
| `bun x vitest run packages/client/src/http-transport.test.ts packages/client/src/docs-drift.test.ts` | **19 passed**（2 文件；exit 0；未新增方法 → 协议计数与站点 §7.3 无需动） |

**基线红（既有、非本项引入，已证伪归属）**：`cargo test -p nomifun-db`（整包）在既有目标 `crates/backend/nomifun-db/tests/id_schema_contract.rs` 上失败 2 例——① `every_product_table_has_one_integer_autoincrement_row_primary_key`（`:131`）硬编码 `tables.len() == 110`、实际 **114**（`052`/`053`/`055` 各加表共 +4，期望未更新）；② `all_nontechnical_id_columns_are_text_and_only_id_is_a_technical_key`（`:257`）要求 `*_id` 列为 TEXT，而 `058_oauth_client_registrations.sql:42` 明写 `ALTER TABLE oauth_tokens ADD COLUMN registration_id INTEGER`（logical link）。**证伪方式**＝把 `058` 移出迁移目录（当时 57 个文件）重跑该目标 → **同样 20 passed / 2 failed**（`left: 114 / right: 110`、`left: "INTEGER" / right: "TEXT"`），与 `app_server_context_usage` 无关。**未修**：修它需要改既有迁移或放宽 ID 契约（两者都超出本项与硬边界），登记交归属方。**同轮又命中第三例（同一整包门、另一次运行，已独立证伪）**：`crates/backend/nomifun-db/tests/provider_platform_migration.rs:61` 的 `exact_legacy_preset_roots_gain_provider_identity_and_retired_roots_move`（**0 passed / 1 failed**）——迁移 `030` 的平台/URL 回填期望与 fixture 不一致（期望 `openai` / `xai` / `ppio` / `wishub-x6`…，实际得到 `custom` 平台与原 URL）。**该用例的路径是 `migrate_to(pool, 29)` → 插 fixture → `migrate_to(pool, 30)`**，`max_version` 过滤使 `058` **在整条路径上根本不会被应用**，故与 `058` 无关（比「移出目录」更直接）；同样**未修**。**整包口径（本会话两次运行合计覆盖 lib + 32/32 个集成目标）**：lib **448 passed / 0 failed**；**30 个集成目标全绿**；**2 个既有红目标**＝`id_schema_contract`（2 例）+ `provider_platform_migration`（1 例）。

> **✅ 三例全部收口（2026-09-11）**：① + ② 按「**契约的单一事实源**」修，**不动迁移**——`NON_REFERENCE_ID_COLUMNS` 由私有改为 `pub` 并从 `nomifun_db` 导出，`id_schema_contract.rs` 的用例改为**读这张登记表**判定例外（原先自带副本，迁移 `052` 加上 `oauth_tokens.registration_id` 之后两份定义就开始打架），表数量期望 `110` → **114**；③ 是**用例陈旧**而非迁移回归：重分类在**迁移 033**（`033_classify_legacy_provider_presets.sql`），用例的迁移目标原写成 `30`（**030 是 `ssh_hosts`**），改成 `33` 后按预期重分类并归一 URL。**实测**：`cargo test -p nomifun-db --test id_schema_contract` → **22 passed / 0 failed**（原 20/2）；`--test provider_platform_migration` → **1 passed / 0 failed**（原 0/1）。改动只落在两个测试文件 + 一处可见性导出，**未改任何迁移**。记录见「基线红测试收口记录」。
...
- WebSocket（stdio JSONL 为 V2/deferred、V1 不纳入，见 §7 决策 2）；
- 结构化错误和幂等键。
...
> 注（2026-09-09）：本节原文档清单仅覆盖 `00`~`08` + roadmap + test-cases，已过期；`09`~`15`、证据页与设计记录未列入，以上方指针为准。（证据页与设计记录已于 2026-09-11 整体并入编号文档。）
...
- App Server WebSocket 传输的稳定性（V1 不含 stdio）；
...
2. **协议词汇与概念对齐（thread/turn/item；原题「协议 vNext」），stdio 不纳入**（2026-09-09）：V1 之后破坏性升级公共协议为 v2，概念模型对齐 Codex app-server（`run/conversation → thread`、`agent/run → turn*`、事件项归并为 `item`）；`initialize` 版本协商与 `dispatch_connection_request` 唯一分发保留；stdio 维持排除（SDK 仍走 spawn + 回环 WS）。该决策将重构 webui 事件层（`conversation-events` / `RunHandle` 等）与 SDK 方法面，需在词汇对齐立项前先产出 Codex app-server spec diff（方法/事件/概念映射表）再动工；V1 冻结版本文档（`05`、`07`）标为 v1 基线。（**注**：其中「升级为 v2 / 冻结版本 / v1 基线」的**版本框架表述已被本节决策 4 取代**——该对齐属协议 v1 内部调整，不设下一版；概念对齐目标不变。）
...
3. **`web` typecheck 归零（指令「修 web 那条既有 typecheck 错」）**。既有红是
   ...
   `{ name: "flowy-agent-store-chat", version: "0.3.0" }`——否则宿主的版本字面量会被钉在第二个地方，
