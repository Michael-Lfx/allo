...
import { canvasNodeCreatedAt, canvasNodeUpdatedAt } from "@oc/lib/canvas/canvas-node-timestamps";
...
export function canvasNodeSearchTimes(node: CanvasNodeData) {
    ...
    return {
        createdAt,
        updatedAt,
        createdLabel: createdAt ? searchTimeFormatter.format(new Date(createdAt)) : canvasT("videoCanvas.search.unrecorded", "未记录"),
        updatedLabel: updatedAt ? searchTimeFormatter.format(new Date(updatedAt)) : canvasT("videoCanvas.search.unrecorded", "未记录"),
    };
}
...
function canvasNodeSearchTerms(node: CanvasNodeData) {
    return [
        ...
        typeof node.metadata?.shotIndex === "number" ? canvasT("videoCanvas.search.shot", "镜头 {{n}}", { n: node.metadata.shotIndex + 1 }) : "",
...
function formatDuration(durationMs: number) {
    ...
    if (seconds < 60) return canvasT("videoCanvas.search.seconds", "{{n}}秒", { n: seconds });
    return canvasT("videoCanvas.search.minutesSeconds", "{{m}}分{{s}}秒", { m: Math.floor(seconds / 60), s: seconds % 60 });
}
