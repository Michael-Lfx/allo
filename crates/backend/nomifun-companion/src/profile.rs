...
impl CompanionProfileConfig {
    ...
    pub fn load(dir: &Path) -> Result<Option<Self>, nomifun_common::AppError> {
        ...
        for (label, model) in [
            ("fallback_model", profile.fallback_model.as_ref()),
            ("vision_model", profile.vision_model.as_ref()),
            ("voice.asr", profile.voice.asr.as_ref()),
        ] {
            validate_persisted_model(model).map_err(|error| {
                nomifun_common::AppError::Internal(format!(
                    "companion profile {} has invalid {label}: {error}",
                    path.display()
                ))
            })?;
        }
        if let Some(tts) = profile.voice.tts.as_ref() {
            validate_persisted_model(Some(&tts.as_provider_model())).map_err(|error| {
                nomifun_common::AppError::Internal(format!(
                    "companion profile {} has invalid voice.tts: {error}",
                    path.display()
                ))
            })?;
        }
        profile.voice.vad.validate().map_err(|error| {
            nomifun_common::AppError::Internal(format!(
                "companion profile {} has invalid voice.vad: {error}",
                path.display()
            ))
        })?;
