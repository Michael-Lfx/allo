...
## 追加决策：内容契约唯一事实源（实施期第三轮）

评审发现同一契约数值在两个 crate 里有 4–6 处手写拼写，且已发生**活体数
值漂移与自相矛盾**：agent 提示词说概念/例题每节 400–700 字（档位无关）、
fallback 提示词按档位 150–400 字、质检门要求全字符 ≥300——低档课时照
提示词写（150 字）会被门拒（150 < 300），demo 节旁注 200–400 也可能被
门拒。agent 提示词的 visual 菜单还漏了「无」并把「文字」当合法值，与
枚举/门禁不一致。

**决策**：

- **提示词仍是 const 模板**（本 ADR 的原决策不翻案），但一切契约**数
  值**——节配比（低 1-3/中 3-5/高 4-6、上限 8）、visual 七值菜单、
  每节文字预算（低 150/中 250/高 400）——由 owner（`nomifun-learning::
  models` 的 `ComplexityTier` / `SectionKind` / `VISUAL_OPTIONS`）渲染成
  规则片段，注入 agent 提示词（LazyLock 静态）、工具描述与 schema 枚举、
  fallback 提示词。模板管措辞，表格管数字；新增拼写点必须调用渲染函数，
  禁止手写字面量（反漂移钉子测试锁定渲染产物）。
- **门槛与预算同口径同源**：可视化为主三节型（概念/例题/演示）的质检门
  下限从「全字符 300」改为「纯文字 = 低档预算（150）」——计量口径与文
  字预算一致（公式/图表不占），任何档位照预算写必然过门。小结 120/练习
  40 不变。这是对旧门的有意放宽：可视化为主哲学下，内容质量由 visual 承
  诺兑现门承担，文字门只拦空薄。
- **fallback 提示词去数值化**：风格提示词不再复述档位数值区间（"the
  section task states your exact prose budget"），逐节任务的「约 N 字」
  本就由 `prose_budget` 生成。
